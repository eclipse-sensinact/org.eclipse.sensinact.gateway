...
public interface ResourceBuilder<T> {
	...
	/**
	 * The type of the resource. If not set then it is inferred from the type
	 * of the initial value. If neither is set then String is used.
	 * @param <R>
	 * @param type
	 * @return
	 */
	...
	/**
	 * The type of the value - must be consistent with the built model, e.g.
	 * if {@link #withSetter(Consumer)} is called then the valueType must be
	...
	/**
	 * The resource type - must be consistent with the built model, e.g.
	 * if {@link #withAction(Function, Class...)} is called then the resource
	 * type must be {@link ResourceType#ACTION}
	 * @param resourceType
	 * @return
	 */
	...
	/**
	 * Set an action function to be called, including the types of any arguments that should be passed
	 * @param action
	 * @param arguments
	 * @return
	 */
	 ...
	 * Build the resource
	 * @return
	 * @throws IllegalArgumentException if an invalid model is defined, e.g. having a getter and an action.
	 */
	Resource build();
}
