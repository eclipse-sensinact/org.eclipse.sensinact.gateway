...
public class SensiNactSessionImpl implements SensiNactSession {
	...
	private List<SessionListenerRegistration> removeSubscription(String subscriptionId, List<SessionListenerRegistration> regs) {
		List<SessionListenerRegistration> list = regs.stream()
	...
	public void notify(String topic, AbstractResourceNotification event) {
		...
		synchronized (lock) {
			if(!isExpired()) {
				...
				for(Entry<String, List<SessionListenerRegistration>> e : 
					listenersByWildcardTopic.headMap(topic, true).descendingMap().entrySet()) {
