...
public class AnnotationMapping {
	...
	private static Function<Object, Instant> getTimestampMapping(Class<?> clazz) {
		Field timestamp = Arrays.stream(clazz.getFields())
		...
		if(timestamp == null) {
			...
			Function<Object, Long> read = o -> {
				...
				if (t instanceof String) return Long.parseLong(t.toString());
	...
	private static Object getValueFromField(String fieldName, Object update) {
		try {
			...
			throw new IllegalArgumentException(
					String.format("Failed to read the field %s for the class %s", 
							fieldName, update.getClass()), e);
		}	
	}
	...
	/**
	 * This method must be careful not to capture the class or field in the returned function as
	 * it will cause memory leaks if they are referenced in the value of the weak cache
	 * @param clazz
	 * @param f
	...
	private static String getAnnotationValue(AnnotatedElement f, Class<? extends Annotation> annotationType, Method valueMethod) {
		String resourceName;
		try {
	...
	/**
	 * This method must be careful not to capture any types that hold the classloader
	 * @param <T>
	 * @param fieldName
	 * @param update
	 * @param annotationType
		...
		try {
			...
			throw new IllegalArgumentException(
					String.format("Failed to read the %s annotated field %s for the class %s", 
							annotationType.getSimpleName(), fieldName, update.getClass()), e);
		}	
	}
}
