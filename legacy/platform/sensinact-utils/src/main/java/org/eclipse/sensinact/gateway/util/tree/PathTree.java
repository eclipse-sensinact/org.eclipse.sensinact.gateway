...
public class PathTree<P extends PathNode<P>> {
    ...
    public P add(String uri, String uriElement, boolean isPattern) {
        ...
        if (current != null) {
            current = current.add(isPattern ? factory.createPatternNode(uriElement) : factory.createPathNode(uriElement));
        }
        return current;
    }
    ...
    public <N extends ImmutablePathNode<N>, T extends ImmutablePathTree<N>> T immutable(Class<T> tc, Class<N> ic) {
        try {
            return tc.getConstructor(new Class<?>[]{ic}).newInstance(this.root.<N>immutable(ic, null));
