...
> schema, Timescale optional); M1–M6 done (M4: service binding, helper rewrite, +1ms removal,
...
3. **M3 — Chosen backend on the SPI** *(decision made 2026-08-12: stay on Postgres + TimescaleDB, unified schema per ADR)*: rewrite timescale-provider as a `HistoryStorage` (unified single-table schema, hypertable + `time_bucket` when the extension is present, `date_bin` otherwise) with a composable query builder (one `if` per dimension, no template matrix), efficient `prune`, and a one-time migration of legacy 3-table data. Gate *(refined — the original "unmodified TimescaleHistoryTest" is impossible under a schema change)*: the **query-behavior tests of `TimescaleHistoryTest` stay green with unmodified assertions** (ACT single/range/count semantics; only the table-based synchronization helpers may be adapted), the schema-assertion tests (`InsertTests` row-per-table counts, `IndexesTests`) are replaced by the contract suite + new schema tests, and the M0 golden-response ITs stay green. GeoJSON losslessness lands here; legacy rows stay readable via migration. **Docs (`docs/source/southbound/history/timescale.md`) updated in this milestone**: unified schema, `-oss` image recommendation, unchanged config keys.
