...
Branch `feature/history_provider_rework` is even with master — clean starting point.
...
- **Internal shape**: each config instance is registered by the factory component as a `HistoryIngestFilter` service (name, targets, include/exclude `ICriterion`); the manager consumes them via MULTIPLE/DYNAMIC reference and recomputes the union of `dataTopics()` per provider so unneeded events are never delivered, not merely dropped.
