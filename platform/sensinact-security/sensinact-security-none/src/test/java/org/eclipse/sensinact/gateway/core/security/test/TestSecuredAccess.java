...
public class TestSecuredAccess
{
    ...
    private final ServiceReference referenceHandler = 
			Mockito.mock(ServiceReference.class);
    private final ServiceReference referenceAgent = 
			Mockito.mock(ServiceReference.class);
	private final ServiceRegistration registrationHandler = 
	...
	public void init() throws InvalidServiceProviderException, InvalidSyntaxException 
	{
		...
		Mockito.when(context.createFilter(MESSAGE_HANDLER_FILTER)
				).thenReturn(filterHandler);
		Mockito.when(filterHandler.toString()).thenReturn(
				MESSAGE_HANDLER_FILTER);
...
//					}
		...
		Mockito.when(context.registerService(
			Mockito.eq(MessageHandler.class.getCanonicalName()), 
...
//						(SnaMessageHandler) invocation.getArguments()[1]);
		...
		Mockito.when(context.registerService(
				Mockito.eq(SnaAgent.class.getCanonicalName()), 
				Mockito.any(SnaAgentImpl.class), 
				Mockito.any(Dictionary.class))).thenAnswer(
					new Answer<ServiceRegistration>()
					{
						@Override
...
//							(SnaAgent) invocation.getArguments()[1]);
		...
		Mockito.when(context.registerService(
			Mockito.eq(SensiNactResourceModel.class.getCanonicalName()), 
			Mockito.any(ModelElement.class),
			Mockito.any(Dictionary.class))).thenAnswer(
				new Answer<ServiceRegistration>()
				{
					@Override
