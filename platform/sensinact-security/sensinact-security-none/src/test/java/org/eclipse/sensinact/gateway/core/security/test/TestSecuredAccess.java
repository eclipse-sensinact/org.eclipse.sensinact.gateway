...
public class TestSecuredAccess {
    ...
    private static final String MESSAGE_HANDLER_FILTER = "(&(" + Constants.OBJECTCLASS + "=" + MessageHandler.class.getCanonicalName() + ")(uri=/serviceProvider)";
    ...
    public void init() throws InvalidServiceProviderException, InvalidSyntaxException {
        ...
        Mockito.when(context.getServiceReferences(Mockito.anyString(), Mockito.anyString())).then(new Answer<ServiceReference[]>() {
            ...
            public ServiceReference[] answer(InvocationOnMock invocation) throws Throwable {
                ...
                if (arguments[0] != null && arguments[0].equals(MessageHandler.class.getCanonicalName()) && arguments[1] != null && arguments[1].equals("(uri=/serviceProvider)")) {
                    if (handler == null) {
                        return null;
                    }
                ...
                } else if (arguments[0] != null && arguments[0].equals(SensiNactResourceModel.class.getCanonicalName()) && arguments[1] != null && arguments[1].equals("(uri=/serviceProvider)")) {
                    return new ServiceReference[]{referenceProvider};

                } else if ((arguments[0] != null && arguments[0].equals(AuthorizationService.class.getCanonicalName()) && arguments[1] == null) || (arguments[0] == null && arguments[1].equals(AUTHORIZATION_FILTER))) {
...
//					}
        ...
        Mockito.when(context.registerService(Mockito.eq(MessageHandler.class.getCanonicalName()), Mockito.any(MessageHandler.class), Mockito.any(Dictionary.class))).thenAnswer(new Answer<ServiceRegistration>() {
            @Override
...
//						(SnaMessageHandler) invocation.getArguments()[1]);
        ...
        Mockito.when(context.registerService(Mockito.eq(SnaAgent.class.getCanonicalName()), Mockito.any(SnaAgentImpl.class), Mockito.any(Dictionary.class))).thenAnswer(new Answer<ServiceRegistration>() {
            @Override
            public ServiceRegistration answer(InvocationOnMock invocation) throws Throwable {
...
//							(SnaAgent) invocation.getArguments()[1]);
        ...
        Mockito.when(context.registerService(Mockito.eq(SensiNactResourceModel.class.getCanonicalName()), Mockito.any(ModelElement.class), Mockito.any(Dictionary.class))).thenAnswer(new Answer<ServiceRegistration>() {
            @Override
            public ServiceRegistration answer(InvocationOnMock invocation) throws Throwable {
