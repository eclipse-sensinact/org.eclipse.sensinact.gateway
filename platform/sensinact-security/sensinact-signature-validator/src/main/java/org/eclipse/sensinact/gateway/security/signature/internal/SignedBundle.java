...
public class SignedBundle
{
	 ...
	 * @throws IOException
	 */
	public SignedBundle(Mediator mediator, final Bundle bundle,
	        final CryptographicUtils cryptoUtils) throws IOException
	{
	...
	{
	   List<URL> listEntries = Collections.<URL>list(
			 this.bundle.findEntries("/", "*", true));
	   return listEntries;
	}
	...
	public Manifest getManifest()
	{
		...
		try
		{
			manifest.read(this.getEntry(METADATA_DIR + MF_FILE
					).openStream());
		}
		catch (IOException e)
		{
	 ...
	 * @return HashMap
	 */
	private Map<String, SignatureFile> getSignatureFiles()
			throws IOException
	{
		if (signatureFiles == null)
		{
			...
			Enumeration<URL> entries = this.bundle.findEntries(
					METADATA_DIR, "*.SF", true);
		...
		else
		{
			...
			{
				this.mediator.debug(
				"Better performance, data found (signatureFiles)");
			}
		}
	 ...
	 * @return HashMap
	 */
	public SignatureFile getSignatureFile(final String givenSigner)
	        throws IOException
	{
		...
		{
			this.mediator.debug(
					"SignedJarFile.getSignatureFile, signer: " + signer);
		}
		...
		if (file == null)
		{
			...
			{
				this.mediator.error("SignedJarFile.getSignatureFile: "
		...
		else
		{
			...
			{
				this.mediator.debug(
				"Better performance, data found (signatureFile)");
			}
		}
		return file;
	}
	 ...
	 * @return Certificate.
	 */
	public Certificate getCertificate(final String signer)
	        throws IOException, CMSException, GeneralSecurityException
	{
		...
		else
		{
			...
			{
				this.mediator.debug(
						"Better performance, data found (cert)");
			}
		}
		return cert;
	}
	 ...
	 * @throws Exception
	 */
	public byte[] getSignatureFromBlock(final String signer)
	...
	public SignatureBlock getSignatureBlock(final String signer)
	        throws IOException, CMSException
	{
		...
		else
		{
			...
			{
				this.mediator.debug(
						 "Better performance, data found (block)");
			}
		}
	...
	public CMSSignedData getSignatureBlockAsCMS(final String signer)
	        throws IOException, CMSException
	{
	...
	public Map<String, Certificate> getCertificates()
	        throws CMSException, IOException, GeneralSecurityException
	{
	 ...
	 * @return List<Certificate>
	 */
	private Map<String, Certificate> extractValidCertificates(
	        final String passwd) throws CMSException, IOException, GeneralSecurityException
	{
	 ...
	 * is given
	 */
	private boolean isCertValid(final Certificate currentCert,
	        final String passwd) throws KeyStoreException, CertificateException,
	 ...
	 */
	public Map<String, Certificate> getValidCertificates(final String passwd)
	        throws IOException, CMSException, GeneralSecurityException
	{
	 ...
	 * @return boolean
	 */
	public boolean checkCoherence(final String signer, 
			final Certificate cert, String algo)
	        throws Exception
	{
		return new SignedBundleChecker(mediator).checkCoherence(this, 
				signer, cert, cryptoUtils, algo);
	}
