...
public class SignatureFileChecker
{
	...
	private static boolean checkAbsenceOfModification(
	        final SignedBundle signedJar, final SignatureFile pretendedSigFile)
	        throws IOException
	{
	...
	private static boolean checkHashValuesValid(Mediator mediator, 
			SignedBundle signedBundle, SignatureFile pretendedSigFile, 
			CryptographicUtils cryptoUtils)
	        throws FileNotFoundException, IOException, NoSuchAlgorithmException
	{
		boolean manifestEntriesValid = true;

		final Attributes pretendedMainAttributes = 
				pretendedSigFile.getMainAttributes();
		
		final Map pretendedEntries = pretendedSigFile.getEntries();
		
		final Map<String, String> dataMap = ManifestChecker.extractEntryHashes(
		    mediator, signedBundle.getEntry("/META-INF/MANIFEST.MF").openStream(),
		        cryptoUtils, pretendedSigFile.getHashAlgo());
		...
		for (final Iterator iter = dataMap.entrySet().iterator(); iter.hasNext()
		        && manifestEntriesValid;)
		{
			...
			{
				pretendedHash = (String) pretendedMainAttributes.getValue(
						entryName);
			}
			...
			{
				pretendedHash = ManifestChecker.getEntryHash(
				        (Attributes) pretendedEntries.get(entryName));
			}
	...
	protected static boolean checkEntriesValidity(Mediator mediator, 
		SignedBundle signedJar, SignatureFile pretendedSigFile, 
		CryptographicUtils cryptoUtils) throws FileNotFoundException, 
			IOException, NoSuchAlgorithmException
	{
		// logger.log(Level.DEBUG, "SignatureFileChecker.checkEntriesValidity");

		final boolean absenceOfModifications = 
			SignatureFileChecker.checkAbsenceOfModification(
					signedJar, pretendedSigFile);

		// logger.log(Level.DEBUG,
		// "SignatureFileChecker: absence of modifications, "
		// +absenceOfModifications);
		// check that the hash values are valid
		final boolean hashesValid = 
				SignatureFileChecker.checkHashValuesValid(
					mediator, signedJar, pretendedSigFile, cryptoUtils);
		// logger.log(Level.DEBUG,
