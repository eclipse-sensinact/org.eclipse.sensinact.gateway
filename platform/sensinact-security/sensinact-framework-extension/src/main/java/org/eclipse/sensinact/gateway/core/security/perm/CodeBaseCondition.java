...
public class CodeBaseCondition implements Condition
{
	...
	private static final String CODEBASE_SOURCES_TYPE =
		"org.eclipse.sensinact.gateway.core.security.perm.CodeBaseCondition$1";
	...
	static abstract class CodeBaseSources
	{
		...
		CodeBaseSources(List<String> cache, 
				String conditionType,
				String codeBaseSourcesType)
		{
		...
		{
			if(!this.codeBaseSourcesType.equals(codeBaseSourcesType) 
			  ||!this.codeBaseSourcesType.equals(getClass().getName())
			  ||this.outerCount!= this.sources.size())
			{
	...
	static final Bundle findBundle(
			BundleContext context, String name, boolean location)
	{
		Bundle bundle = null;
	...
	public static Condition getCondition(final Bundle bundle, final ConditionInfo info) 
	{
		...
		{
			throw new IllegalArgumentException(
					"Illegal number of args: " + args.length);
		}
			...
			{											
				...
				try
				{
					if((bundle.adapt(BundleRevision.class).getTypes() 
						...
						&& bundle.getState() == Bundle.RESOLVED)
					{
						b = findBundle(bundle.getBundleContext(), bundle.getHeaders(
								).get(Constants.FRAGMENT_HOST), false);
					} else
					{
						b = bundle;
					}
				...
				int index = 0;
				int length =  stacktraceElements==null
						?0:stacktraceElements.length;
				...
				for(;index < length; index++)
				{
					...
					try
					{
						...
						char[] ccs = c.getProtectionDomain().getCodeSource(
							).getLocation().toString().toCharArray();
	...
	{
		return CodeBaseCondition.getCondition(bundle, info
				).isSatisfied();
	}
