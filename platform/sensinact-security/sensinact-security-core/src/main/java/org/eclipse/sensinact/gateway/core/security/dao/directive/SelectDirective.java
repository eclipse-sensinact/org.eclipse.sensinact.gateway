...
public class SelectDirective extends Directive
{
	 ...
	 * @return
	 */
	static <E extends SnaEntity> SelectDirective 
	getSelectDirective(Mediator mediator, Class<E> entityType,
		Map<Field, Column> fields)
	{	
		Table table = entityType.getAnnotation(Table.class);
		
		KeyDirective keyDirective = KeyDirective.createKeyDirective(
			mediator, table, entityType.getAnnotation(PrimaryKey.class
					), fields);
		
		SelectDirective selectDirective = new SelectDirective(mediator, 
				table.value(), keyDirective);
...
//			}
	 ...
	 * @return
	 */
	public static <E extends SnaEntity> SelectDirective 
	getSelectDirective(Mediator mediator, Class<E> entityType)
	{		
	 ...
	 * Constructor
	 */
	public SelectDirective(Mediator mediator, String table, 
	 ...
	 * to this one
	 * 
	 * @param selectDirective
	 * 		the SelectDirective to join to this one
	 */
	 ...
	 * 
	 * @param keyDirective
	 * 		the KeyDirective to join to this 
	 * 		SelectDirective
	 */
	...
	protected String getColumnName(String table, String column)
	{
		...
		if(columnElements.length == 2 && table.equals(
				columnElements[0]))
		{
			return column;
		}
	...
	public String toString()
	{
		...
		fromBuilder.append(SnaDAO.SPACE).append(SnaDAO.FROM_DIRECTIVE
				).append(SnaDAO.SPACE);

		Iterator<Map.Entry<String,List<String>>> iterator = 
				this.select.entrySet().iterator();
		...
		while(iterator.hasNext())
		{
			...
			{				
				...
				selectBuilder.append(this.getColumnName(
					entry.getKey(), list.get(selectIndex)));	
			}
			if(fromIndex++ > 0)
			{
		...
		Iterator<Map.Entry<String,Object>> conditionIterator = 
				this.where.entrySet().iterator();
		...
		{
			whereBuilder.append(SnaDAO.SPACE).append(
				SnaDAO.WHERE_DIRECTIVE).append(SnaDAO.SPACE);
			...
			for(;index < length; index++)
			{
				...
				{
					whereBuilder.append(SnaDAO.SPACE).append(
							SnaDAO.AND).append(SnaDAO.SPACE);					
				}
				whereBuilder.append(directives.get(index));
			}
		}
		String selectStatement =  selectBuilder.append(
				fromBuilder.toString()).append(
				whereBuilder.toString()).toString();
