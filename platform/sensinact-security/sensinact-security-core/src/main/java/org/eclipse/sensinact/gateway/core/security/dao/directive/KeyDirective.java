...
public class KeyDirective extends Directive
{
	...
	public class KeyEntry implements Nameable
	{
		...
		{
			return KeyDirective.this.getColumnName(
					this.column);
		}
	}
	...
	class ForeignKeyEntry extends KeyEntry
	{
		...
		public String getName() 
		{
			return new StringBuilder().append(
			 this.foreignTable).append(SnaDAO.DOT
				).append(this.foreignColumn
					).toString();
		}
	}
	 ...
	 *  
	 * @return
	 * 		a new {@link KeyDirective} instance for the 
	 * 		specified {@link SnaEntity} type
	 */
	public static <E extends SnaEntity> KeyDirective
	createKeyDirective(Mediator mediator, Class<E> entityType)
	{		
		return createKeyDirective(
			mediator, entityType.getAnnotation(Table.class), 
			entityType.getAnnotation(PrimaryKey.class), 
			SnaEntity.getFields(entityType));
	}
	 ...
	 */
	public static <E extends SnaEntity> KeyDirective
	createKeyDirective(Mediator mediator, Table table, 
	...
	{		
		...
		if(length > 0)
		{
			Iterator<Map.Entry<Field, Column>> iterator = 
			...
			while(iterator.hasNext())
			{
				...
				for(;index < length;index++)
				{
					...
					{
						keyDirective.addForeignKeyEntry(keys[index], 
	...
	{		
    	int index  = this.keyEntries.indexOf(
    			new Name<KeyEntry>(name));	    	
    	if(index == -1)
    	{
    		return;
    	}
	 ...
	 * @param foreignColumn
	 */
	public ForeignKeyEntry addForeignKeyEntry(String column, 
    ...
    public String getValueDirective()
    {
		...
		while(iterator.hasNext())
		{
			...
			if(entry.getValue() != null)
			{
				if(builder.length() > 0)
				{
					builder.append(SnaDAO.SPACE).append(SnaDAO.AND
						).append(SnaDAO.SPACE);
				}
				super.buildEqualityDirective(builder,entry.getColumn(), 
						entry.getValue());
			}
		}
	...
	public String toString()
	{
		...
		while(iterator.hasNext())
		{
			...
			if(ForeignKeyEntry.class.isAssignableFrom(entry.getClass()))
			{
				...
				builder.append(((ForeignKeyEntry)entry).getForeignTable()
					).append(SnaDAO.DOT).append(((ForeignKeyEntry)entry
						).getForeignColumn()).append(SnaDAO.EQUALS_OPERATOR
