...
public abstract class AbstractMutableSnaDAO<E extends SnaEntity> extends AbstractSnaDAO<E> {
     ...
     * @throws DAOException
     */
    AbstractMutableSnaDAO(Mediator mediator, Class<E> entityType) throws DAOException {
        super(mediator, entityType);
        if (this.entityType.getAnnotation(Immutable.class) != null) throw new DAOException("Mutable type expected");
    }
    ...
    void created(E entity, long identifier) {
        ...
        if (SnaEntity.setUniqueLongPrimaryKey(mediator, entity, identifier)) {
            ...
            mediator.debug("Unable to define the new entity '%s' identifier [%s]", entityType.getSimpleName(), identifier);
        }
    }
