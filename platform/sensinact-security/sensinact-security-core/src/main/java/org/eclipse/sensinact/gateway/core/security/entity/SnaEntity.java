...
public abstract class SnaEntity {
    ...
    public class Key implements Nameable {
        ...
        public Key(Field field) {
            ...
            this.localColumn = new StringBuilder().append(table.value()).append(SnaDAO.DOT).append(column.value()).toString();

            if (this.foreignKey) {
                this.foreignColumn = new StringBuilder().append(foreignKey.table()).append(SnaDAO.DOT).append(foreignKey.refer()).toString();
            }
            this.field = field;
        }
    ...
    private static final <E extends SnaEntity> java.lang.reflect.Field getUniqueFieldPrimaryKey(Mediator mediator, E entity) {
     ...
     * @param mediator the {@link Mediator} allowing to
     *                 interact with the OSGi host environment
     * @param row      the JSONObject describing this SnaEntity
     */
...
//		}
    ...
    private String getMethodName(String fieldName, String prefix) {
        ...
        String methodName = new StringBuilder().append(prefix).append(firstUpper).append(fieldName.substring(1)).toString();
        return methodName;
    }
}
