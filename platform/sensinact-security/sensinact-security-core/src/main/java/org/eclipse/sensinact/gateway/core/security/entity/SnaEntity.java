...
{	
	...
	public class Key implements Nameable
	{
		...
		public Key(Field field)
		{
			...
			this.localColumn = new StringBuilder().append(table.value()
					).append(SnaDAO.DOT).append(column.value()
							).toString();
			
			if(this.foreignKey)
			{
				this.foreignColumn =  new StringBuilder().append(
					foreignKey.table()).append(SnaDAO.DOT).append(
						foreignKey.refer()).toString();
			}
			this.field = field;
		}
		...
		public long value()
		{
			Object object = SnaEntity.this.getFieldValue(
					this.field);
			if(object == null)
			{
				return -1;
	 ...
	 */
	private final static Map<Class<? extends SnaEntity>, Map<Field,Column>> 
	FIELDS_MAP = new HashMap<Class<? extends SnaEntity>, Map<Field,Column>>();

	/**
	 * @param snaEntityType
	 * @return
	 */
	public static <E extends SnaEntity> Map<Field,Column> getFields(
			Class<E> entityType)
	{
	...
	private static final <E extends SnaEntity> java.lang.reflect.Field 
	getUniqueFieldPrimaryKey(Mediator mediator, E entity)
	{
		...
		Iterator<Map.Entry<java.lang.reflect.Field, Column>>
	 ...
	 * @return
	 */
	public static <E extends SnaEntity> long getUniqueLongPrimaryKey(
			Mediator mediator, E entity)
	{		
		java.lang.reflect.Field field = getUniqueFieldPrimaryKey(
				mediator,entity);
	 ...
	 * @return
	 */
	public static <E extends SnaEntity> boolean setUniqueLongPrimaryKey(
			Mediator mediator, E entity, long identifier)
	{
		java.lang.reflect.Field field = getUniqueFieldPrimaryKey(
				mediator,entity);
	...
	protected SnaEntity(Mediator mediator)
	{
		...
		this.keys = this.createKeyList(this.getClass().getAnnotation(
				PrimaryKey.class),fields);
	}
...
//			{
				...
				String column = new StringBuilder().append(table.value()
					).append(SnaDAO.DOT).append(rowColumn).toString();
				
				if(row.has(column))
				{
					setFieldValue(mediator.getClassLoader(), 
							entry.getKey(), row.get(column));
					
				} else if(row.has(rowColumn))
				{
					setFieldValue(mediator.getClassLoader(),
							entry.getKey(), row.get(rowColumn));
				}
...
//			}
	 ...
	 * @param entity
	 */
	protected List<Key> createKeyList(
			...
			Map<Field, Column> fields)
	{
		...
		Iterator<Map.Entry<Field, Column>>
		iterator = fields.entrySet().iterator();
	...
	public Object getFieldValue(String field)
	{
		...
		Iterator<Map.Entry<Field, Column>> iterator = fields.entrySet(
				).iterator();
		while(iterator.hasNext())
		{
			Map.Entry<Field, Column> entry = iterator.next();
...
//		}
	...
	protected void setFieldValue(ClassLoader classLoader, Field field, Object value)
	{
		...
		try 
		{
			if((method = this.getMethod(this.getMethodName(
				field.getName(), "set"), field.getType()
				))!=null)
			{
				method.invoke(this, CastUtils.cast(classLoader, 
						field.getType(), value));
				
			} else
			{
				field.setAccessible(true);
				field.set(this, CastUtils.cast(classLoader, 
						field.getType(), value));
			}
		} catch (Exception e) 
		{
	...
	private Method getMethod(String methodName, Class<?> fieldType)
	{
		try 
		{
			return this.getClass().getDeclaredMethod(
	...
	private String getMethodName(String fieldName, String prefix)
	{
		...
		String methodName = new StringBuilder().append(prefix
			).append(firstUpper).append(fieldName.substring(1)
					).toString();
		return methodName;
	}
}
