...
public class TestDAO
{
    ...
    private final ServiceReference referenceDataStoreService = 
	...
	UnableToConnectToDataStoreException, InvalidSyntaxException 
	{
		...
		Mockito.when(context.getService(Mockito.any(
				ServiceReference.class))).then(
					new Answer<Object>(){
		...
		Mockito.when(bundle.getResource(Mockito.anyString())).thenAnswer(
		new Answer<URL>()
		{
			@Override
					...
					throws Throwable 
			{
				...
				{
					return new File(
						"src/main/resources/script/getObjectFromPath.sql"
							).getAbsoluteFile().toURI().toURL();						
				}
				else if(arguments[0].equals("script/getMethodAccessibilities.sql"))
				{
					return new File(
						"src/main/resources/script/getMethodAccessibilities.sql"
							).getAbsoluteFile().toURI().toURL();						
				}
				return null;	
			}
		...
		dataStoreService = new SQLiteDataStoreService(mediator, 
				"src/test/resources/sensinact.sqlite");      
     }
	...
	public void testSelectExactObjectDAO() throws DAOException
	{
		...
		String[] resultingPaths = new String[]{
				"/slider/cursor/position",
				"/slider/[^/]+/location"
	...
	public void testFakePredefinedSelectStatement() throws DAOException
	{
		...
		new Answer<URL>()
		{
					...
					throws Throwable 
			{
				...
				{
					return new File(
						"src/main/resources/script/fake.sql"
							).getAbsoluteFile().toURI().toURL();						
				}
				else if(arguments[0].equals("script/getMethodAccessibilities.sql"))
				{
	...
	public void testDeleteParentObject() throws DAOException
	{
		...
		ObjectEntity entity = new ObjectEntity(
				mediator, 2l, 1l, "fake", 0, 0, 0, null);
		objectDAO.create(entity);
		
		final long identifier = entity.getIdentifier();
		assertTrue(identifier > 0l);
		
		ObjectEntity newEntity = new ObjectEntity(
				mediator, 2l, 2l, "admin", 0, 1, identifier, null);
		objectDAO.create(newEntity);
		
		final long newIdentifier = newEntity.getIdentifier();
		assertTrue(newIdentifier > 0l);

		ObjectEntity otherEntity = new ObjectEntity(
				mediator, 2l, 1l, "service", 0, 1, identifier, null);
		objectDAO.create(otherEntity);
		
		final long otherIdentifier = otherEntity.getIdentifier();
		assertTrue(otherIdentifier > 0l);

		ObjectEntity lastEntity = new ObjectEntity(
				mediator, 2l, 1l, "resource", 0, 1, otherIdentifier, null);
		...
		List<AuthenticatedEntity> entities = authenticatedDAO.select(
				new HashMap<String,Object>(){{
					this.put("OID", lastIdentifier);
		}});
		assertTrue(entities.isEmpty());
		entities = authenticatedDAO.select(
				new HashMap<String,Object>(){{
	...
	public void testDeleteBundle() throws DAOException
	{
		...
		BundleEntity bundleEntity = new BundleEntity(mediator,
				"fake-bundle", "fake-bundle", 0, 1);
		bundleDAO.create(bundleEntity);
		
		ObjectDAO objectDAO = new ObjectDAO(mediator);	
		
		ObjectEntity entity = new ObjectEntity(
				mediator, bundleEntity.getIdentifier(), 1l, "fake", 0, 0, 0, null);
		objectDAO.create(entity);
