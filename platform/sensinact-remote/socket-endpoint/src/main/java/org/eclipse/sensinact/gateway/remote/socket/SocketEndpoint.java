...
public class SocketEndpoint extends AbstractRemoteEndpoint {
    ...
    protected JSONObject incomingRequest(JSONObject request) {
        ...
        switch (length) {
            case 1:
                ...
                switch (subUriELements[0]) {
                    ...
                    case "callback":
                        ...
                        try {
                            for (; index < arrayLength; index++) {
                                messages[index] = AbstractSnaMessage.fromJSON(mediator, messagesArray.getJSONObject(index).toString());
                            }
                    ...
                    case "all":
                        ...
                        response = (subUriELements.length == 1) ? super.remoteCore.getAll(publicKey) : super.remoteCore.getAll(publicKey, subUriELements[1]);
                        break;
                    case "providers":
            ...
            case 7:
                switch (uriElements[6]) {
                    case "GET":
                        accessMethodResponse = super.remoteCore.get(publicKey, uriElements[1], uriElements[3], uriElements[5], null);
                        break;
                    case "SET":
                        ...
                        accessMethodResponse = super.remoteCore.set(publicKey, uriElements[1], uriElements[3], uriElements[5], attributeId, data);
                        break;
                    case "ACT":
                        ...
                        accessMethodResponse = super.remoteCore.act(publicKey, uriElements[1], uriElements[3], uriElements[5], args);
                        break;
                    case "SUBSCRIBE":
                        ...
                        accessMethodResponse = super.remoteCore.subscribe(publicKey, uriElements[1], uriElements[3], uriElements[5], conditions);
                        break;
                    case "UNSUBSCRIBE":
        ...
        return new JSONObject().put("uuid", uuid).put("response", response);
    }
        ...
        this.connectionTimer.schedule(new TimerTask() {
            ...
            public void run() {
                try {
                    ...
                    mediator.error(e);
                    if (SocketEndpoint.this.client != null && !SocketEndpoint.this.client.running()) {
    ...
    protected void serverStopped() {
        if (super.connected) {
            ...
            mediator.warn("An error occurred that impeded the server start");
        }
    }
    ...
    public void unregisterAgent(String identifier) {
        ...
        String response = this.client.request(new JSONObject().put("uri", uri));
        if (response != null) {
            mediator.debug(response.toString());
        }
    }
