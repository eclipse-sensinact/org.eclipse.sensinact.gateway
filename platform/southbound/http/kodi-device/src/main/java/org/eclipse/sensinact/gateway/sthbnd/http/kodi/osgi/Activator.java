...
@HttpTasks(tasks = {
	@SimpleHttpTask(
		commands={CommandType.GET,CommandType.ACT},
		configuration = @HttpTaskConfiguration(
			host = "@context[kodi.ip]",
			path ="/jsonrpc",
			contentType="application/json",
			httpMethod="POST",
			content= KodiTaskConfigurator.class), 
		profile = "")		
	}
)
public class Activator extends HttpActivator
{
     ...
     */
    @Override
	public void connect(ExtModelConfiguration configuration)
    		...
    		new Executable<HttpService,Void>()
			{
				...
				public Void execute(HttpService httpService) throws Exception
				{
					HttpContext hc = httpService.createDefaultHttpContext();
	              	...
	              	httpService.registerServlet(KODI_ENDPOINT,
	                new KodiRemoteControlHttpListener(Activator.super.endpoint,
	                (KodiServiceMediator) Activator.super.mediator),
	                new Hashtable(), hc);
	...
	public HttpTaskProcessingContextFactory getTaskProcessingContextFactory()
	{
		return new DefaultHttpTaskProcessingContextFactory(mediator)
		{
			...
			{
				return new KodiTaskProcessingContext(super.mediator, httpTaskConfigurator,
						endpointId, task);
			}
		};				
	}
	...
	extends DefaultHttpTaskProcessingContext
	{
		 ...
		 * @param task
		 */
		public KodiTaskProcessingContext(Mediator mediator,
				...
				final String endpointId, final HttpTask<?, ?> task)
		{
			...
			super.properties.put("kodi.ip", new Executable<Void,String>()
			{
				...
				public String execute(Void parameter) throws Exception
				{
					...
					return ((KodiServiceMediator)Activator.super.mediator
							).getKodiIP(serviceProvider);
				}			
			});
		}
	}
