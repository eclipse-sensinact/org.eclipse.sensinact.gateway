...
public class KodiTaskConfigurator implements HttpTaskConfigurator
{
	...
	public <T extends HttpTask<?, ?>> void configure(T task) throws Exception
	{
		...
		{		
			...
			{
		        Iterator<MethodDefinition> iterator = 
		        ((ExtResourceConfig) resourceConfig).iterator();
		        ...
		        while(iterator.hasNext())
		        {
			        ...
			        while(it.hasNext()) 
			        {
			        	List<ParameterDefinition> parametersName = 
			        			it.next().getParameters();
				        ...
				        {
				            params.put(parametersName.get(i).getName(), 
				            		parameters[i]);
				        }
