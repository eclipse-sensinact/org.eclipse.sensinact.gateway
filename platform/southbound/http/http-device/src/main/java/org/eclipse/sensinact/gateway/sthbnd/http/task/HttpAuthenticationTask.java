 ...
 */
public class HttpAuthenticationTask<RESPONSE extends HttpResponse,
...
{ 
    ...
    * by this HttpDiscoveryConnectionConfiguration
	 */
	public HttpAuthenticationTask(Mediator mediator, 
    ...
    private void registerAuthenticationHeader(String token)
    {
    	...
    	((HttpProtocolStackEndpoint)super.transmitter
				).registerPermanentHeader(key, token);
    }
    ...
    public void setResult(Object result, long timestamp)
    {
		...
		try
		{
			value = this.tokenExtractor==null?CastUtils.cast(
				super.mediator.getClassLoader(), String.class, result)
					:this.tokenExtractor.execute(result);
		}
		catch (Exception e)
		{
