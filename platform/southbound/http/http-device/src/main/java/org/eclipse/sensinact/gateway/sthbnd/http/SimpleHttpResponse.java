...
public class SimpleHttpResponse extends HttpResponse
{
	 ...
	 * @throws IOException 
	 */
    public SimpleHttpResponse(Mediator mediator, HttpURLConnection 
    	connection,  HttpConnectionConfiguration<? extends HttpResponse, 
    	? extends Request<? extends HttpResponse>> configuration)
    		throws IOException
    {
	...
	public HttpPacket createPacket()
	{
		...
		try
		{
			...
			{
				return packetType.getConstructor(
						HttpResponse.class).newInstance(this);
				
			} else
			{
				return  this.packetType.getConstructor(
					Map.class, byte[].class).newInstance(
					this.getHeaders(),this.content);
