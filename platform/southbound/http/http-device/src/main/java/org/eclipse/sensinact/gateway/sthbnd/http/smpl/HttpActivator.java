...
{	
	...
	{
		return (byte) (BuildPolicy.BUILD_COMPLETE_ON_DESCRIPTION.getPolicy()
	 ...
	 */
	public SimpleHttpProtocolStackEndpoint configureProtocolStackEndpoint() 
			throws Exception
	{
		SimpleHttpProtocolStackEndpoint endpoint =
			ReflectUtils.getInstance(getEndpointType(), new Object[]{
				mediator	
			});	
		...
		ChainedHttpTasks chainedTaskArray = this.getClass().getAnnotation(
				ChainedHttpTasks.class);
		...
		RecurrentChainedHttpTask[] recurrentChainedTasks = chainedTaskArray==null
				?null:chainedTaskArray.recurrences();		
		index = 0;
