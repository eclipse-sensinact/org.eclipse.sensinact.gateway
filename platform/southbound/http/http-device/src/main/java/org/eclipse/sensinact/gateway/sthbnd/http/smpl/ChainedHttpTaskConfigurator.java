...
{ 	
	 ...
	 */
	public ChainedHttpTaskConfigurator(SimpleHttpProtocolStackEndpoint endpoint,
		String profile, CommandType command, HttpTaskUrlConfigurator urlBuilder,
		HttpTaskConfiguration annotation, HttpChildTaskConfiguration[] chain)
	{	
		...
		{
			SimpleTaskConfigurator executable = new SimpleTaskConfigurator(
			endpoint, profile, command, urlBuilder, annotation,
			chain[index]);
		...
		try
		{
			...
			while(iterator.hasNext())
			{
				...
				HttpTaskProcessingContext context =
						super.endpoint.createChainedContext(
						link.configurator, chained, subTask);
				
				if(context != null)
				{
					super.endpoint.getMediator().registerProcessingContext(
						subTask, context);
				}
			}
		} catch(Exception e)
