...
HTTP_CHAINED_TASK extends HttpChainedTask<REQUEST>> extends 
HttpTask<SimpleHttpResponse,REQUEST>
{		
	 ...
	 * @return
	 */
	public abstract HTTP_CHAINED_TASK createChainedTask(
		String identifier, CommandType type,  String path, String profileId,
	 ...
	 * @param parameters
	 */
	public HttpChainedTasks(Mediator mediator, CommandType command, 
			TaskTranslator transmitter, Class<REQUEST> requestType, 
			String path, String profileId, ResourceConfig resourceConfig, 
			Object[] parameters) 
	{
		super(mediator, command, transmitter, requestType, path, 
	...
	public void execute() 
	{
		Iterator<HTTP_CHAINED_TASK> iterator = 
				this.chain.iterator();
		...
		Class<? extends HttpPacket> packetType =
			(this.packetType!=null)?this.packetType:HttpPacket.class;
		try
		{
			packet = packetType.getConstructor(
			Map.class, byte[].class).newInstance(
    ...
    {
		return this.addChainedTask(this.createChainedTask(
			identifier, this.getCommand(), super.getPath(), 
			super.getProfile(), super.getResourceConfig(),
			super.getParameters()));
    }
