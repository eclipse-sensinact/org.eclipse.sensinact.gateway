...
public class SimpleHttpProtocolStackEndpoint extends HttpProtocolStackEndpoint
{
	...
	public static final Class<? extends HttpTask> 
	GET_TASK = HttpTask.class;

	public static final Class<? extends HttpTask> 
	SET_TASK = HttpTask.class;

	public static final Class<? extends HttpTask> 
	ACT_TASK = HttpTask.class;

	public static final Class<? extends HttpTask> 
	SUBSCRIBE_TASK = HttpTask.class;

	public static final Class<? extends HttpTask> 
	UNSUBSCRIBE_TASK = HttpTask.class;

	public static final Class<? extends HttpTask> 
	SERVICES_ENUMERATION_TASK = HttpTask.class;
	 ...
	 * @throws IOException
	 */
	public SimpleHttpProtocolStackEndpoint(HttpMediator mediator)
	        throws ParserConfigurationException, SAXException, IOException
	{
        ...
        ServiceLoader<HttpTaskUrlConfigurator> loader = ServiceLoader.load(
        		HttpTaskUrlConfigurator.class, mediator.getClassLoader());
	...
	public void registerAdapter(ChainedHttpTask chainedHttpTask)
	{
		...
		for(;index < length; index++)
		{
			ChainedHttpTaskConfigurator executor = new 
				ChainedHttpTaskConfigurator(
				this, chainedHttpTask.profile(),
				commands[index], 
				this.builders.get(commands[index]),
				chainedHttpTask.configuration(),
				chainedHttpTask.chain());
	...
	{
		RecurrentHttpTaskConfigurator executor =
			new RecurrentChainedTaskConfigurator(
			this,
			chainedHttpTask.command(),
		    this.builders.get(chainedHttpTask.command()),
		    chainedHttpTask.chaining(),
		    chainedHttpTask.period(),
		    chainedHttpTask.delay(), 
		    chainedHttpTask.timeout(),
		    chainedHttpTask.configuration(),
		    chainedHttpTask.chain());
	...
	{		
		...
		for(;index < length; index++)
		{
		    SimpleTaskConfigurator executor = 
			new SimpleTaskConfigurator(
				this, 
				httpTaskAnnotation.profile(), 
				commands[index], 
				this.builders.get(commands[index]),
				httpTaskAnnotation.configuration());
			this.adapters.put(commands[index], executor);
		}
	}	
	...
	{
		RecurrentTaskConfigurator executor =
		new RecurrentTaskConfigurator( 
			this, 
			httpTaskAnnotation.command(),
		    this.builders.get(httpTaskAnnotation.command()),
		    this.getTaskType(httpTaskAnnotation.command()),
		    httpTaskAnnotation.period(),
		    httpTaskAnnotation.delay(), httpTaskAnnotation.timeout(),
		    httpTaskAnnotation.recurrence());
		this.recurrences.add(executor);
	}
			...
			throws InvalidProtocolStackException
	{
		...
		Iterator<RecurrentHttpTaskConfigurator> iterator = 
				this.recurrences.iterator();
			...
			TimerTask timerTask = new TimerTask()
			{
				private long timeout = 0;
				 ...
				 public void run()
				 {
					if(timeout == 0)
					{
						timeout = executable.getTimeout()==-1
						?-1:(System.currentTimeMillis()+executable.getTimeout());
					}
					...
					HttpTask<?,?> task = ReflectUtils.getInstance(executable.getTaskType(), 
					new Object[]{ mediator, executable.handled(), SimpleHttpProtocolStackEndpoint.this,
					SimpleHttpRequest.class, UriUtils.ROOT, null, null, null});					
					try
					{
						if(ChainedHttpTaskConfigurator.class.isAssignableFrom(
								executable.getClass()))
						{
						...
						{
							HttpTaskProcessingContext context = 
							    SimpleHttpProtocolStackEndpoint.this.createContext(
									executable, task);
							
							if(context != null)
							{
								((HttpMediator)mediator).registerProcessingContext(
										task, context);
							}
						}
						task.execute();
	...
	public void send(Task task)
	{
		...
		{
			((HttpMediator)mediator).unregisterProcessingContext(
					(HttpTask<?, ?>) task);
		}
	}
	...
	public Task createTask(Mediator mediator, CommandType command, String path,
	        String profileId, ResourceConfig resourceConfig,
	        Object[] parameters)
	{
		...
		HttpTask<?,?> task = ReflectUtils.getInstance(this.getTaskType(command), 
		new Object[]{ mediator, command, this, SimpleHttpRequest.class,  path, 
				profileId, resourceConfig, parameters});
		try
		{
	 ...
	 * processing context
	 */
	protected HttpTaskProcessingContext createContext( 
			HttpTaskConfigurator httpTaskConfigurator, HttpTask<?, ?> task)
	{
	 ...
	 * processing context
	 */
	protected <CHAINED extends HttpChainedTask<?>> HttpTaskProcessingContext
	createChainedContext(HttpTaskConfigurator httpTaskConfigurator,
						 HttpChainedTasks<?,CHAINED> tasks, CHAINED task)
	{
		HttpChainedTaskProcessingContextFactory factory = null;
		
		if((factory = ((HttpMediator)this.mediator
				).getChainedTaskProcessingContextFactory()) != null)
		{
			return factory.newInstance(httpTaskConfigurator, 
					this.endpointId, tasks, task);
		}
		return null;
	}
	 ...
	 */
	public  void setServicesEnumerationTaskType(
			Class<? extends HttpTask> servicesEnumerationTaskClass)
	{
