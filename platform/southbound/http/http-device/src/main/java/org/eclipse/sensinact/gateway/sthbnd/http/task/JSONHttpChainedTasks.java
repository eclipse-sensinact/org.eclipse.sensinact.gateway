 ...
 * 
 */
public class JSONHttpChainedTasks<REQUEST extends Request<SimpleHttpResponse>>
extends HttpChainedTasks<REQUEST, JSONHttpChainedTask<REQUEST>>
{
	/**
	 * @param mediator
	 * @param transmitter
	 ...
	 * @param parameters
	 */
    public JSONHttpChainedTasks(Mediator mediator, CommandType command, 
    		TaskTranslator transmitter, Class<REQUEST> requestType, 
    		String path, String profileId, ResourceConfig resourceConfig, 
    		Object[] parameters)
    {
	    super(mediator, command, transmitter, requestType, 
	    		path, profileId, resourceConfig, parameters);
    }
    ...
    {
	   JSONHttpChainedTask<REQUEST> task = this.createChainedTask(
	    identifier, this.getCommand(),  super.getPath(), super.getProfile(), 
		super.getResourceConfig(), super.getParameters());	
	 ...
	 */
    @Override
	public JSONHttpChainedTask<REQUEST> createChainedTask(
			String identifier, CommandType command, String path, String profileId, 
            ResourceConfig resourceConfig, Object[] parameters)
    {
	    JSONHttpChainedTask<REQUEST> chain =  new JSONHttpChainedTask<REQUEST>(
	    mediator, command, transmitter, requestType, path, profileId, 
	...
	protected byte[] getResultBytes() 
	{
		...
		if((result = super.getIntermediateResult()) !=null
				&& result!= AccessMethod.EMPTY)
		{
