 ...
 */
public class FreeSmsProtocolStackEndpoint extends 
SimpleHttpProtocolStackEndpoint implements ManagedConfigurationListener
{
	 ...
	 * @throws IOException
	 */
	public FreeSmsProtocolStackEndpoint(HttpMediator mediator)
	...
	{
		Map<String, FreeSmsResourceConfig> propertiesResources = 
			new HashMap<String, FreeSmsResourceConfig>();
		...
		while(enumeration.hasMoreElements())
		{
			...
			if(propertyKeyParts.length == 2) 								
			{
				FreeSmsResourceConfig fsrc = 
					propertiesResources.get(propertyKeyParts[0]);
			...
			super.mediator.warn(String.format(
			"unrecognized property '%s' \n '<name>#(user|pass)' key format expected ",
			propertyKey));			
		}		
		
		Iterator<Map.Entry<String,FreeSmsResourceConfig>> iterator =
				configuredResources.entrySet().iterator();
		...
		while(iterator.hasNext())
		{
			...
			if(!propertiesResources.containsKey(entry.getKey()))
			{
				try
				{
					super.process(new FreeSmsPacket(entry.getKey(),
							null, null, false));					
					iterator.remove();					
				}
		...
		while(!toBeAdded.isEmpty())
		{
			...
			if(fsrc == null || (user = fsrc.getUser())==null 
					|| (pass = fsrc.getPass())==null)
			{
				mediator.warn(String.format(
					"%s : user and pass properties are needed",key));	
				continue;
			}
			try
			{				
