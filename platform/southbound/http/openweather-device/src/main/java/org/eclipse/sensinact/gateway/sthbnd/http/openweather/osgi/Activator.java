...
@ChainedHttpTasks(recurrences = {
	@RecurrentChainedHttpTask(
		...
		configuration = @HttpTaskConfiguration(
			host = HttpChildTaskConfiguration.DEFAULT_HOST),
		chain = { 
			@HttpChildTaskConfiguration(
				...
				identifier="weather",
				query = {
					@KeyValuePair(key = "lat", 
					value = "$(org.eclipse.sensinact.gateway.weather.latitude.@context[endpoint.id])"),
					@KeyValuePair(key = "lon", 
					value = "$(org.eclipse.sensinact.gateway.weather.longitude.@context[endpoint.id])"),
					@KeyValuePair(key = "APPID", 
...
{	
	...
	{	
		...
		String stationsList = (String)mediator.getProperty(
	 ...
	 */
	public HttpChainedTaskProcessingContextFactory
	getChainedTaskProcessingContextFactory()
	{
		return new DefaultHttpChainedTaskProcessingContextFactory(mediator)
		{
			 ...
			 */
			@Override
			public <CHAINED extends HttpChainedTask<?>> HttpTaskProcessingContext newInstance(
			     HttpTaskConfigurator httpTaskConfigurator, String endpointId, 
			     HttpChainedTasks<?, CHAINED> tasks, CHAINED task)
			{
				return new OpenWeatherTaskProcessingContext(
				Activator.this.mediator, httpTaskConfigurator, endpointId, tasks, task);
			}
	...
	extends DefaultHttpChainedTaskProcessingContext
	{
		...
		OpenWeatherTaskProcessingContext(Mediator mediator,
			...
			final CHAINED task) 
		{
			...
			super.properties.put("weather.icon", new Executable<Void,String>()
			{
				...
				public String execute(Void parameter) throws Exception
				{
					...
					String icon = new JSONArray(result
						).optJSONObject(0
						).optJSONObject("weather"
