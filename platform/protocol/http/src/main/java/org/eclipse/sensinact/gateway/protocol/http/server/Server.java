...
public class Server {
     ...
     * server
     *
     * @param running the running state of the HTTP
     *                server
     */
     ...
     * HTTP server
     *
     * @return the running state of the
     * HTTP server
     */
    boolean running() {
     ...
     *
     * @param path the path of the HttpContext for which to attach
     *             an new HttpHandler
     */
    protected void handle(String path) {
        ...
        context.setHandler(new HttpHandler() {
            ...
            public void handle(HttpExchange httpExchange) throws IOException {
                int responseCode = (!Server.this.running() ? HttpURLConnection.HTTP_NOT_FOUND : HttpURLConnection.HTTP_ACCEPTED);
                ...
                byte[] content = length > 0 ? IOUtils.read(httpExchange.getRequestBody(), length, false) : IOUtils.read(httpExchange.getRequestBody(), false);
    ...
    protected void bind() throws IOException {
        ...
        while (networkInterfaces.hasMoreElements()) {
            ...
            while (inetAddresses.hasMoreElements()) {
                ...
                this.bind(inetAddress.getHostAddress(), DEFAULT_PORT);
            }
        }
    }
    ...
    public void bind(String ipAddress, int port) throws IOException {
        ...
        if (LOG.isLoggable(Level.INFO)) {
            LOG.log(Level.INFO, new StringBuilder().append("server binded on ").append(ipAddress).append(" / port ").append(port).toString());
        }
    }
