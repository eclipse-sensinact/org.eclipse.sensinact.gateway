...
public class TestResourceBuilder<R extends ModelInstance> {
    ...
    public void testSessionMethods() throws Exception {
        ...
        final JSONObject changed = new JSONObject(new Changed(Thread.currentThread().getContextClassLoader(), true).getJSON());
        ...
        JSONAssert.assertEquals(session.get("serviceProvider", "testService", "TestProperty", null).getResponse(), session.get("serviceProvider", "testService", "LinkedProperty", DataResource.VALUE).getResponse(), false);

        session.set("serviceProvider", "testService", "LinkedProperty", DataResource.VALUE, "testLink");

        JSONAssert.assertEquals(session.get("serviceProvider", "testService", "TestProperty", DataResource.VALUE).getResponse(), session.get("serviceProvider", "testService", "LinkedProperty", null).getResponse(), false);

        service.addLinkedResource(LocationResource.LOCATION, this.testContext.getModelInstance().getRootElement().getAdminService().getResource(LocationResource.LOCATION));

        ResourceImpl r4impl = service.getResource("location");

        r4impl.registerExecutor(AccessMethod.Type.valueOf(AccessMethod.GET), new Class<?>[0], new String[0], new AccessMethodExecutor() {
            @Override
        ...
        JSONObject message = new JSONObject(session.get("serviceProvider", "testService", "location", DataResource.VALUE).getJSON());
        ...
        org.junit.Assert.assertEquals(1, ((MyModelInstance) this.testContext.getModelInstance()).getHandler().count(filter));

        session.unsubscribe("serviceProvider", "testService", "TestProperty2", subId);

        org.junit.Assert.assertEquals(0, ((MyModelInstance) this.testContext.getModelInstance()).getHandler().count(filter));
    ...
    public void testResourceModel() throws Exception {
        ...
        Assert.assertTrue("the message should have been processed even if the value has not changed", 1 == this.testContext.getExtraCallbackCount());
    ...
    public void testAgent() throws Exception {
        ...
        Mockito.when(this.testContext.getMediator().getContext().getProperty(Mockito.anyString())).thenAnswer(new Answer<Object>() {
            @Override
...
//     {"notification":{"lifecycle":"PROVIDER_DISAPPEARING"},"uri":"/serviceProvider"}
    ...
    public void testConstrained() throws Exception {
        ...
        valueAttribute.addMetadata(new Metadata(this.testContext.getMediator(), Metadata.CONSTRAINTS, Constraint[].class, new Constraint[]{new MinLength(this.testContext.getMediator().getClassLoader(), 5, false), new MaxLength(this.testContext.getMediator().getClassLoader(), 10, false)
    ...
    public void testLinkedAction() throws Exception {
        ...
        action.registerExecutor(new Signature(this.testContext.getMediator(), AccessMethod.Type.valueOf(AccessMethod.ACT), new Class<?>[]{int.class}, new String[]{"count"}), new AccessMethodExecutor() {
        ...
        action.registerExecutor(new Signature(this.testContext.getMediator(), AccessMethod.Type.valueOf(AccessMethod.ACT), new Class<?>[]{int.class, boolean.class}, new String[]{"count", "plus"}), new AccessMethodExecutor() {
        ...
        LinkedActMethod linkedActMethod = (LinkedActMethod) method.invoke(emptyLinkedAction, AccessMethod.Type.valueOf(AccessMethod.ACT));

        method = LinkedActMethod.class.getDeclaredMethod("createShortcut", new Class<?>[]{Signature.class, Shortcut.class});
        method.setAccessible(true);

        method.invoke(linkedActMethod, new Object[]{new Signature(this.testContext.getMediator(), AccessMethod.Type.valueOf(AccessMethod.ACT), new Class<?>[]{int.class}, new String[]{"count"}), new Shortcut(this.testContext.getMediator(), AccessMethod.Type.valueOf(AccessMethod.ACT), new Class<?>[0], new String[0], new HashMap<Integer, Parameter>() {{
            this.put(0, new Parameter(TestResourceBuilder.this.testContext.getMediator(), "count", int.class, 4));
        }})});

        //securedAccess.getAnonymousSession().registered(instance);
        //instance.getServiceProvider().start();

        ServiceProvider proxy = (ServiceProvider) this.testContext.getModelInstance().getRootElement().getProxy(this.tree);
    ...
    public void testTrigger() throws Throwable {
        ...
        service.addActionTrigger("TestAction", "TestVariable", new Signature(this.testContext.getMediator(), AccessMethod.Type.valueOf(AccessMethod.ACT), null, null), new Constant("triggered", false), AccessMethodExecutor.ExecutionPolicy.AFTER);
    ...
    public void testFiltering() throws Throwable {
        ...
        Thread.sleep(1000);
        String obj = s.getAll(new FilteringCollection(this.testContext.getMediator(), false, new FilteringDefinition("xfilter", "a"))).getJSON();
