...
{	
	 ...
	 */
	public  LinkedResourceImpl(
			ModelInstance<?> modelInstance,
			ResourceConfig resourceConfig, 
			ResourceImpl targetedResource,
			ServiceImpl service) 
			throws InvalidResourceException 
	{			
	...
	public void buildAttributes(ResourceConfig resourceConfig)
	{
		String defaultAttributeName = resourceConfig.getTypeConfig(
				...
				Resource.ATTRIBUTE_DEFAULT_PROPERTY, false) ;
				
		this.setDefault(defaultAttributeName);
		try 
		{
			super.elements.add(new Attribute(
					super.modelInstance.mediator(), 
					this, Resource.NAME, String.class, 
					UriUtils.getLeaf(super.uri), Modifiable.FIXED, 
					true));
	...
	public List<AttributeDescription> getAllDescriptions()
	{
		List<AttributeDescription> descriptions 
		 = super.getAllDescriptions();
		
		if(this.targetedResource == null)
		{
			return descriptions;
		}		
		List<AttributeDescription> linkedDescriptions =
				this.targetedResource.getAllDescriptions();
		
		List<AttributeDescription> allDescriptions = 
    ...
    public void registerLink(String path)
    {
    	super.modelInstance.mediator().debug(
    			"Unable to register a link for a LinkedResourceImpl");
    }
}
