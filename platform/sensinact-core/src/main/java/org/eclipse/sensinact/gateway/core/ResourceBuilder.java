...
{	    
	 ...
	 * @param resourceType
	 * 		the extended {@link Resource} type of the 
	 * 		resource to create
	...
	public void configureName(String name)
	{
		RequirementBuilder builder = new RequirementBuilder(
				AttributeBuilder.Requirement.VALUE, Resource.NAME);
	...
	/**
	 * Configures the type of the 'value' attribute
	 * of the resource to build
	 * 
	 * @param name
	 * 		the name of the resource to build
	 */
	...
	{
		this.configureRequirement(DataResource.VALUE, 
				AttributeBuilder.Requirement.TYPE, 
				type);
	}	

	/**
	 * Configures the value of the 'value' attribute
	 * of the resource to build
	 * 
	 * @param name
	 * 		the name of the resource to build
	...
	public void configureValue(Object value)
	{
		...
		this.configureRequirement(DataResource.VALUE, 
				AttributeBuilder.Requirement.VALUE, 
				value);
	}
	
	/**
	 * Builds a new {@link RequirementBuilder} to apply on
	 * the set of {@link AttributeBuilder}s used to create
	 * the set of {@link Attribute}s of the resource to
	 * create
	 * 
	 * @param attributeName
	 * 		the name of the attribute on which the 
	 ...
	 */
	public void configureRequirement(String attributeName, 
			AttributeBuilder.Requirement requirement,
			Object value)
	{
		RequirementBuilder builder = new RequirementBuilder(
	 ...
	 * 
	 * @return
	 * 		this ResourceBuilder's {@link ResourceConfig}
	 */
	...
	/**
	 * Builds and returns a new {@link ResourceImpl}
	 * connected to the {@link ServiceImpl} passed
	 * as parameter
	 * 
	 * @param service
	 * 		the service to which the {@link ResourceImpl}
	 * 		to create is connected to 
	 * @return
	 * 		a new {@link ResourceImpl}
	 * 
	 * @throws InvalidResourceException 
	 */
	...
	{	
		...
		ResourceImpl resourceImpl = ReflectUtils.getInstance(
			typeConfig.getResourceBaseClass(), 
			typeConfig.getResourceImplementedClass(), 
			new Object[]{modelInstance, this.getResourceConfig(),
					service});
		
        if(resourceImpl == null)
        {
	        throw new InvalidResourceException(
	        	String.format("Unable to create the resource : %s",
	        		name));
        }
		if(!resourceImpl.isHidden())
		{
			...
			} catch (InvalidValueException e)
			{
		        ...
		        throw new InvalidResourceException(
		        		"Error while creating methods", e);
			}
		}
		return resourceImpl;
	}
	...
	/**
	 * Builds and returns a new {@link LinkedResourceImpl}
	 * connected to the {@link ServiceImpl} passed
	 * as parameter and linked to the {@link ResourceImpl}
	 * also passed as parameter
	 * 
	 * @param service
	 * 		the service to which the {@link LinkedResourceImpl}
	 * 		to create is connected to 
	 * @param target
	 * 		the {@link ResourceImpl} to which the 
	 * 		{@link LinkedResourceImpl} to create is
	 * 		linked to
	 * @param link
	 * 		the name of the {@link LinkedResourceImpl} 
	 * 		to create
	 * @return
	 * 		a new {@link LinkedResourceImpl}
	 * 
	 * @throws InvalidResourceException 
	 */
	public ResourceImpl	buildLinkedResource(ServiceImpl service, 
			ResourceImpl target) throws InvalidResourceException
	{
		if(target == null)
		{
			throw new InvalidResourceException(
					"Target resource is needed");
		}		
        LinkedResourceImpl linkedResourceImpl = new LinkedResourceImpl(
        	(ModelInstance<?>) target.getModelInstance(), 
	 ...
	 * @throws InvalidResourceException 
	 */
	public ResourceImpl buildLinkedActionResource( 
		ServiceImpl service, ResourceImpl target, 
			boolean copyActMethod ) 
				throws InvalidResourceException
	{
        LinkedResourceImpl linkedResourceImpl = (LinkedResourceImpl) 
        		this.buildLinkedResource(service, target);
	 ...
	 */
	protected final void buildMethods(final ResourceImpl 
	resource) throws InvalidResourceException, 
	...
	{		
		...
		subscribeMethod.addSignature(
			subscribeSignature, subscribeExecutor(resource), 
			AccessMethodExecutor.ExecutionPolicy.BEFORE);
		
		// Unsubscribe method
		unsubscribeMethod.addSignature(
			unsubscribeSignature, unsubscribeExecutor(resource), 
			AccessMethodExecutor.ExecutionPolicy.BEFORE);
		
		//Subscribe method - condition parameter shortcut  
		Parameter conditionsParameter = new Parameter(this.mediator,
				"conditions", Set.class);
		conditionsParameter.setValue(Collections.emptySet());
				
		Map<Integer, Parameter> fixedConditionsParameters = 
		...
		{
			Parameter nameParameter = new Parameter(this.mediator,
					"attributeName", String.class);
			nameParameter.setValue(resource.getDefault());	
			
			Map<Integer, Parameter> fixedNameParameter = 
			new HashMap<Integer,Parameter>();
	...
	/**
	 * Creates and returns the pre-processing {@link 
	 * AccessMethodExecutor} to register into the {@link 
	 * AccessMethod} whose type is passed as parameter
	 * 
	 * @return
	 * 		the pre-processing executor associated to the
    		...
    		final String type)
    {
	    return new AccessMethodExecutor()
	   {
	        		...
	        		throws Exception
	        {
				...
				type, resource.getPath(), parameter.getParameters());
    ...
    /**
	 * Creates and returns the specific post-processing {@link 
	 * AccessMethodExecutor} to register into the {@link 
	 * ActMethod} of the {@link ResourceImpl} passed 
	 * as parameter
	 * 
	 * @return the post-processing executor associated to the
	 * act method of the specified resource
	 */
    private AccessMethodExecutor getActPostProcessingExecutor(
    		final ResourceImpl resource)
    {
	    return new AccessMethodExecutor()
	   {
	        		...
	        		throws Exception
	        {
				SnaUpdateMessage message = SnaNotificationMessageImpl.Builder.<
				  SnaUpdateMessage>notification(mediator, 
				    SnaUpdateMessage.Update.ACTUATED, 
				     resource.getPath());
     ...
     * @return
     */
    final private AccessMethodExecutor getExecutor(
    		final ResourceImpl resource)
    {
    	return new AccessMethodExecutor()
		{
                    ...
                    throws Exception
            {
				...
				AttributeDescription description = resource.getDescription(
						(String) snaResult.getParameter(0));
     ...
     * @return
     */
    final private AccessMethodExecutor setExecutor(
    		final ResourceImpl resource)
    {
    	return new AccessMethodExecutor()
		{
     ...
     * @return
     */
    final private AccessMethodExecutor subscribeExecutor(
    		final ResourceImpl resource)
    {
    	return new AccessMethodExecutor()
		{
	                ...
	                throws Exception
	        {
				...
				Attribute attribute = (Attribute) resource.getAttribute(
						attributeName);
				if(attribute == null)
				{
				...
				if(callbackId != null)
				{
					...
					result.put("initial", new JSONObject(
     ...
     * @return
     */
    final private AccessMethodExecutor unsubscribeExecutor(
    		final ResourceImpl resource)
    {
    	return new AccessMethodExecutor()
		{
			...
			{  
				...
				result.setAccessMethodObjectResult(
						new JSONObject().put("message", "unsubscription done"));
				return null;
			}
		};
    }
