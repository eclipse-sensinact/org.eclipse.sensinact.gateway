...
public class ResourceBuilder {
     ...
     * @param resourceType the extended {@link Resource} type of the
     *                     resource to create
     ...
     * create
     *
     * @param attributeName the name of the attribute on which the
     ...
     * as parameter
     *
     * @param service the service to which the {@link ResourceImpl}
    ...
    public ResourceImpl build(ModelInstance<?> modelInstance, ServiceImpl service) throws InvalidResourceException {
        ...
        ResourceImpl resourceImpl = ReflectUtils.getInstance(typeConfig.getResourceBaseClass(), typeConfig.getResourceImplementedClass(), new Object[]{modelInstance, this.getResourceConfig(), service});
     ...
     *
     * @param service the service to which the {@link LinkedResourceImpl}
     *                to create is connected to
     * @param target  the {@link ResourceImpl} to which the
     *                {@link LinkedResourceImpl} to create is
     *                linked to
     * @param link    the name of the {@link LinkedResourceImpl}
     *                to create
     ...
     */
    protected final void buildMethods(final ResourceImpl resource) throws InvalidResourceException, InvalidValueException {
        if (resource == null) {
            return;
        }
        //Get method
        ...
        subscribeSignature = new Signature(this.mediator, SUBSCRIBE, new Class<?>[]{String.class, Recipient.class, Set.class}, new String[]{"attributeName", "listener", "conditions"});
        ...
        while (attrs.hasMoreElements()) {
            if (Modifiable.MODIFIABLE.equals(attrs.nextElement().getModifiable())) {
                // Set method
                setMethod.addSignature(setSignature, setExecutor(resource), AccessMethodExecutor.ExecutionPolicy.BEFORE);
                break;
            }
        }
        // Subscribe method
        subscribeMethod.addSignature(subscribeSignature, subscribeExecutor(resource), AccessMethodExecutor.ExecutionPolicy.BEFORE);

        // Unsubscribe method
        unsubscribeMethod.addSignature(unsubscribeSignature, unsubscribeExecutor(resource), AccessMethodExecutor.ExecutionPolicy.BEFORE);
        ...
        Shortcut subscribeConditionsShortcut = new Shortcut(this.mediator, SUBSCRIBE, new Class<?>[]{String.class, Recipient.class}, new String[]{"attributeName", "listener"}, fixedConditionsParameters);
        ...
        if (resource.getDefault() != null) {
            ...
            Shortcut subscribeNameConditionsShortcut = new Shortcut(this.mediator, SUBSCRIBE, new Class<?>[]{Recipient.class}, new String[]{"listener"}, fixedNameParameter);

            Shortcut subscribeNameShortcut = new Shortcut(this.mediator, SUBSCRIBE, new Class<?>[]{Recipient.class, Set.class}, new String[]{"listener", "conditions"}, fixedNameParameter);
     ...
     *
     * @return the pre-processing executor associated to the
    ...
    private AccessMethodExecutor getActPostProcessingExecutor(final ResourceImpl resource) {
        return new AccessMethodExecutor() {
            @Override
            public Void execute(AccessMethodResponseBuilder parameter) throws Exception {
                SnaUpdateMessage message = SnaNotificationMessageImpl.Builder.<SnaUpdateMessage>notification(mediator, SnaUpdateMessage.Update.ACTUATED, resource.getPath());
    ...
    final private AccessMethodExecutor setExecutor(final ResourceImpl resource) {
        return new AccessMethodExecutor() {
            ...
            @Override
            public Void execute(AccessMethodResponseBuilder snaResult) throws Exception {
                JSONObject result = new JSONObject(resource.set((String) snaResult.getParameter(0), snaResult.getParameter(1)).getJSON());
                snaResult.setAccessMethodObjectResult(result);
                return null;
            }
        };
    ...
    final private AccessMethodExecutor subscribeExecutor(final ResourceImpl resource) {
        return new AccessMethodExecutor() {
            ...
            public Void execute(AccessMethodResponseBuilder snaResult) throws Exception {
                ...
                if (attribute == null) {
                    throw new InvalidAttributeException(new StringBuilder().append("unknown attribute :").append(attributeName).toString());
                }
                ...
                String callbackId = resource.listen(attributeName, recipient, conditions, type, lifetime, buffer, delay);
