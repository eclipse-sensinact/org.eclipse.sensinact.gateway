...
{			
			...
			AccessMethodExecutor postProcessingExecutor, ErrorHandler errorHandler)
	{
		...
		this.shortcuts = new IdentityHashMap<Shortcut, 
				Signature>();
	...
	/**
	 * Returns the {@link Signature} of this {@link AccessMethod}
	...
	/**
	 * Returns a new {@link Signature} instance whose parameter 
	 * types are the same as the ones specified in the array 
	 * argument and whose type is the one of this {@link AccessMethod} 
	 * 
	 * @param parameterTypes
	 * 		the array of types of the {@link Signature} to create
	 * @return
	 * 		a new {@link Signature} instance
	 */
	private Signature createSignature(Class<?>[] parameterTypes)
	{
		Class<?>[] types = parameterTypes == null
				?new Class<?>[0]: parameterTypes;	
		...
		{
            names[index] = new StringBuilder().append("arg"
	...
	/**
	 * Returns a new {@link Signature} instance whose parameter 
	 * types are the same as the ones specified in the types array 
	 * argument, whose parameter names are the same as the ones 
	 * specified in the strings array argument and whose type is 
	 * the one of this {@link AccessMethod} 
	 * 
	 * @param parameterTypes
	 * 		the array of parameter types of the {@link 
	 * 		Signature} to create
	 * @param names
	 * 		the array of parameter string names of the 
	 * 		{@link Signature} to create
	 * @return
	 * 		a new {@link Signature} instance
	 */
	...
	/**
	 * Creates a {@link Signature} using the parameter 
	 * types array argument and maps it to the {@link 
	 * AccessMethodExecutor} passed as parameter
	 *  
	 * @param parameterTypes
	 * 		the parameter types array
	 * @param executor
	 * 		the {@link AccessMethodExecutor} to map 
	 * 		to the {@link Signature} to create
	 * @param policy
	 * 		 the {@link AccessMethodExecutor.Execu tionPolicy}
	 * 		 of the specified {@link AccessMethodExecutor}
	 */
	public void addSignature(Class<?>[] parameterTypes, 
	...
	{			
		...
		this.addSignature(this.createSignature(
				parameterTypes), executor, policy);		
	}
	
	/**
	 * Creates a {@link Signature} using the parameter types and 
	 * parameter names arrays arguments and maps it to the {@link 
	 * AccessMethodExecutor} passed as parameter
	 *  
	 ...
	 * 		the parameter types array
	 * @param executor
	 * 		 the {@link AccessMethodExecutor} to map to the 
	 * 		{@link Signature} to create
	 * @param policy
	 * 		 the {@link AccessMethodExecutor.ExecutionPolicy}
	 * 		 of the specified {@link AccessMethodExecutor}
	...
	{		
		if(signature==null 
			|| signature.getName().intern()
			!= this.type.name().intern())
		{
			return;
		}		
		Deque<AccessMethodExecutor> executors =
			this.map.get(signature);
		...
		if(executors == null)
		{
			...
			{
				fixedParameters.putAll(((Shortcut)shortcut
						).getFixedParameters());
				shortcut = reference;
			}
			if((executors = this.map.get(shortcut)) == null)
			{						
				if((shortcut = this.getSignature(
		...
		AccessMethodExecutor.ExecutionPolicy executionPolicy = 
		policy == null?AccessMethodExecutor.ExecutionPolicy.AFTER:policy;
		...
		if(fixedParameters!= null && !fixedParameters.isEmpty())
		{
			...
			Iterator<Map.Entry<Integer, Parameter>> iterator = 
					fixedParameters.entrySet().iterator();
			...
			while(iterator.hasNext())
			{
				...
				try
				{
					((AccessMethodExecutorWrapper)methodExecutor).put(
					entry.getKey(), new Fixed(this.mediator.getClassLoader(), 
	...
	/**
	 * Registers the specified {@link Shortcut} mapped 
	 * to the already registered {@link Signature}, passed as 
	 * parameter
	 *  
	 * @param shortcut
	 * 		the {@link Shortcut} to register
	 * @param signature
	 * 		the {@link Signature} already registered 
	 * 		to map to the specified {@link Shortcut}
	 */
    ...
    /**
     * Invokes this method using the specified {@link Signature}'s
     * parameter values to parameterize the call 
     * 
     * @param signature
     * 		the {@link Signature} of this method parameterizing
     * 		the invocation
     * @param accessLevel	the integer access level of the call
     * @return
    ...
    public synchronized AccessMethodResponse invoke(Signature signature)
    {
		...
		while(true)
		{
			...
			if(!Shortcut.class.isAssignableFrom(
					current.getClass()))
			{
				executors = this.map.get(current);   
				break;
			}
			((Shortcut)signature).push((Shortcut) 
					current);
		}
		if(executors == null)
		{
    	...
    	Iterator<AccessMethodExecutor> iterator = 
    			executors.iterator();
	...
	/**
	 * Deletes all registered {@link Signature}s and
	 * {@link Shortcut}s
	 */
    public void stop()
    {
