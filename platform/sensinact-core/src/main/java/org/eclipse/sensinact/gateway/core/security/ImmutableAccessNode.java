...
extends ImmutablePathNode<ImmutableAccessNode> implements AccessNode
{
	//********************************************************************//
	//						NESTED DECLARATIONS	     					  //
	//********************************************************************//
	 ...
	 * @param mediator
	 */
    public <A extends AccessNodeImpl<A>> ImmutableAccessNode(
    		ImmutableAccessNode parent, String nodeName, 
    		boolean isPattern, PathNodeList<A> children,
     ...
     */
	private Map<AccessLevelOption, List<MethodAccessibility>>
	withAccessProfile(AccessProfile profile)
    {
        Map<AccessLevelOption, List<MethodAccessibility>> accesses = 
            new EnumMap<AccessLevelOption, List<MethodAccessibility>>(
        ...
        while(iterator.hasNext())
        {
        	...
        	{	    
        		...
        		List<MethodAccessibility> accessibilities =
        				accesses.get(optionLevel);
        		...
        		MethodAccessibilityImpl methodAccessibilities = 
        			new MethodAccessibilityImpl(types[index], 
	 ...
	 * available at the {@link AccessLevel} passed as parameter; otherwise
	 * returns false
	 * 
	 * @param accessMethod the {@link AccessMethod.Type} to define the 
	 * availability of for the specified {@link AccessLevel}
	 * @param accessLevel the {@link AccessLevel} for which to define the 
	 * specified {@link AccessMethod.Type} availability
	 * @return 
	 * 		<ul>
	 * 			<li>true if the specified {@link AccessMethod.Type} is 
	 * 			accessible for the specified {@link AccessLevel}</li>
			...
			AccessLevelOption optionLevel)
	{
		...
		{			
			accessible =  super.parent==null?false:
				super.parent.isAccessibleMethod(
