...
public final class AttributeBuilder implements Nameable, Cloneable {
     ...
     *
     * @param resourceType the class for which retrieve all predefined {@link
     *                     AttributeBuilder}s
     * @return the merged array of predefined {@link
     * AttributeBuilder}s
     */
    public static final AttributeBuilder[] getAttributeBuilders(LinkedList<Class<Resource>> list) {
        ...
        while (iterator.hasNext()) {
            ...
            try {
                attributeBuilders = mergeAttributeBuilders((AttributeBuilder[]) resourceInterface.getDeclaredField(AttributeBuilder.ATTRIBUTES_PROPERTY).get(null), attributeBuilders);
     ...
     *
     * @param current   the first of the two arrays to merge
     * @param inherited the second of two arrays to merge
     * @return the @link AttributeBuilder}s array resulting of the merge
     ...
     *
     * @param builders the AttributeBuilders array to copy
     * @param length   the length of the returned copy. If the length is inferior
     ...
     * Constructor
     *
     * @param requirementsArray the array of required field which apply on this
     ...
     * Constructor
     *
     * @param requirements the array of required field which apply on this
     *                     AttributeBuilder to create the appropriate {@link Attribute}
     * @throws InvalidAttributeException
     */
     ...
     *
     * @param typeName the type of the {@link Attribute} to build
     ...
     *
     * @param value the initial value of the {@link Attribute} to build
     * @return this AttributeBuilder
     */
     ...
     * modified or not
     *
     * @param modifiable the {@link Modifiable} policy applying on the value of
     *                   the {@link Attribute}
     * @return this AttributeBuilder
     */
     ...
     * @param resource   the {@link ResourceImpl} to which the Attribute
     *                   to create will be attached
     * @param typeConfig the TypeConfig gathering configuration
     *                   informations
     * @return the new created {@link Attribute}
     * @throws InvalidAttributeException if an error occurred while creating the Attribute
     */
    public Attribute getAttribute(Mediator mediator, ResourceImpl resource, TypeConfig resourceTypeConfig) throws InvalidAttributeException {
        ...
        while (this.requirementsList.size() > 0) {
            ...
            String constantName = new StringBuilder().append(PROPERTY_DEFAULT_PREFIX).append(constantPrefix).append(requirement.name()).toString();
        ...
        throw new InvalidAttributeException(new StringBuilder().append(resource.getPath()).append(" Creating attribute '").append(this.name).append("' : missing requirements ").append(Arrays.toString(this.requirementsList.toArray())).toString());
    }
     ...
     *
     * @param requirement the Requirement that has been satisfied
     */
     ...
     * {@link Attribute}s built using this AttributeBuilder
     *
     * @param constraints the list of{@link Constraint}s to add
     */
