...
extends AbstractAccessMethod<T, DescribeResponse<T>>
{
	...
	public DescribeResponseBuilder<T> createAccessMethodResponseBuilder(
    		Object[] parameters)
    {
    	switch(this.describeType)
    	{
			...
			case RESOURCES_LIST:
			case SERVICES_LIST:
				return (DescribeResponseBuilder<T>) 
				new DescribeStringResponseBuilder(mediator, uri, 
						describeType, null);
			case PROVIDER:
			case SERVICE:
			case RESOURCE:
				return (DescribeResponseBuilder<T>) 
				new DescribeJSONResponseBuilder(mediator, uri, 
						describeType);
			default:
				break;
    	}
