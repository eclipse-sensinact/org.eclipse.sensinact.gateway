...
public class SnaMessageListener extends AbstractStackEngineHandler<SnaMessage<?>> implements MessageHandler {
     ...
     *
     * @param filter the {@link SnaFilter} to remove
     */
        ...
        synchronized (this.callbacks) {
            ...
            while (iterator.hasNext()) {
                ...
                while (callbackIterator.hasNext()) {
                    ...
                    if ((callback.getTimeout() != MidCallback.ENDLESS && System.currentTimeMillis() > callback.getTimeout()) || (AccessMethodResponse.Status.ERROR == callback.getStatus() && (handler = callback.getCallbackErrorHandler()) != null && ErrorHandler.ErrorHandlerPolicy.contains(handler.getPolicy(), ErrorHandler.ErrorHandlerPolicy.REMOVE))) {
    ...
    private void doHandleAgents(final SnaMessage<?> message, final String method) {
        ...
        mediator.callServices(SnaAgent.class, new Executable<SnaAgent, Void>() {
            ...
            public Void execute(SnaAgent agent) throws Exception {
                ...
                if ((index = methodAccessibilities.indexOf(new Name<MethodAccessibility>(method))) > -1 && methodAccessibilities.get(index).isAccessible()) {
