...
public class ResourceImpl extends ModelElement<ModelInstance<?>, ResourceProxy, ResourceProcessableContainer<?>, Attribute, AttributeDescription> implements Typable<Resource.Type> {
    ...
    /**
     * Path of registered {@link LinkedResourceImpl} referring to
     * this resource
     */
    ...
    /**
     * this ResourceImpl's update policy :
     * NONE : updates are made by the way of the client request
     * INIT : the first upate is made by the way of the client request
     * and is automatic after that
    ...
    protected ResourceImpl(ModelInstance<?> modelInstance, ResourceConfig resourceConfig, ServiceImpl service) {
        super(modelInstance, service, UriUtils.getUri(new String[]{service.getPath(), resourceConfig.getName(service.getName())}));
    ...
    /**
     * Updates this ResourceImpl instance. If the attributeName parameter
     * is null the default one is targeted. If the metadataName argument
     * is not null the {@link Metadata} with the specified name is updated
     *
     * @param attributeName the name of the attribute to update the value of
     * @param metadataName  the name of the metadata to update the value of
    ...
    protected void update(String attributeName, String metadataName, Object value, long timestamp) throws InvalidValueException {
        if (value == null) {
            super.modelInstance.mediator().warn(new StringBuilder().append("Null object value : unable to update resource '").append(getPath()).append("'").toString());
            return;
        }
        String name = attributeName == null ? this.getDefault() : attributeName;

        if (name == null) {
            super.modelInstance.mediator().warn(new StringBuilder().append("Null attribute name : unable to update resource '").append(getPath()).append("'").toString());
            return;
        }
        Attribute attribute = this.getAttribute(name);
        byte buildPolicy = super.getModelInstance().configuration().getResourceBuildPolicy();

        if (attribute == null && SensiNactResourceModelConfiguration.BuildPolicy.isBuildPolicy(buildPolicy, SensiNactResourceModelConfiguration.BuildPolicy.BUILD_NON_DESCRIBED)) {
            attribute = new Attribute(super.modelInstance.mediator(), this, name, String.class, null, Modifiable.UPDATABLE, false);

            if (!this.addAttribute(attribute)) {
                super.modelInstance.mediator().warn("Error when creating attribute '%s': unable to update resource '%s", name, getPath());
                return;
            }
        }
        if (attribute == null) {
            super.modelInstance.mediator().warn(new StringBuilder().append("Null attribute '").append(name).append("': unable to update resource '").append(getPath()).append("'").toString());
        ...
        if (metadataName != null) {
            ...
            if (metadata == null && SensiNactResourceModelConfiguration.BuildPolicy.isBuildPolicy(buildPolicy, SensiNactResourceModelConfiguration.BuildPolicy.BUILD_NON_DESCRIBED)) {
                metadata = new Metadata(super.modelInstance.mediator(), metadataName, String.class, null, Modifiable.UPDATABLE);
                attribute.addMetadata(metadata);
            }
            if (metadata == null) {
                super.modelInstance.mediator().warn(new StringBuilder().append("Null metadata ").append(metadataName).append(": unable to update resource '").append(getPath()).append("'").toString());
                return;
            }
    ...
    /**
     * Creates the set of {@link Attribute}s of this
     * {@link ResourceImpl} according to the configuration
     * of the {@link ResourceConfig} passed as parameter
     *
     * @param resourceConfig the {@link ResourceConfig} holding the set of
    ...
    public void buildAttributes(ResourceConfig resourceConfig) {
        ...
        String defaultAttributeName = resourceConfig.getTypeConfig().<String>getConstantValue(Resource.ATTRIBUTE_DEFAULT_PROPERTY, false);
        ...
        for (; index < length; index++) {
            ...
            try {
                attribute = builders.get(index).getAttribute(super.modelInstance.mediator(), this, resourceConfig.getTypeConfig());
            ...
            if (attribute != null) {
                if (attribute.getName().equals(defaultAttributeName)) {
                    try {
                        Metadata metadata = new Metadata(super.modelInstance.mediator(), Attribute.NICKNAME, String.class, this.getName(), Modifiable.FIXED);
    ...
    public AttributeDescription set(String name, Object value) throws InvalidValueException {
        ...
        if (attribute != null) {
            if (!Modifiable.MODIFIABLE.equals(attribute.getModifiable())) {
                throw new InvalidValueException(new StringBuilder().append(name).append(" attribute is not modifiable").toString());
            }
            if (attribute.getLocked()) {
                throw new InvalidValueException(new StringBuilder().append(name).append(" attribute has been locked by an action trigger").toString());
            }
    ...
    /**
     * Sets the value of the default {@link Attribute} of this
     * Resource if it is defined and return the extended
     * {@link Description} describing it
     *
     * @return the {@link AttributeDescription} describing this
     * Resource's default {@link Attribute}
     */
    ...
    /**
     * Adds the given {@link Attribute} to this Resource. The
     * {@link Attribute} will not be added if there exist one
     * with the same name. In order to update an existing
     * {@link Attribute} the set methods must be used.
     *
    ...
    /**
     * Removes the {@link Attribute} whose name is passed
     * as parameter
     *
     * @param primitive the name of the {@link Attribute} to remove
     */
    public boolean removeAttribute(String attributeName) {
        if (attributeName == null || Resource.NAME.intern() == attributeName.intern() || Resource.TYPE.intern() == attributeName.intern() || DataResource.VALUE.intern() == attributeName.intern()) {
            return false;
        }
    ...
    /**
     * Returns the {@link Attribute} of this
     * ResourceImpl, whose name is the same as
     * the specified one
     *
     * @param name the name of the searched {@link Attribute}
     * @return the  {@link Attribute} with the specified
     * name
     */
    ...
    /**
     * Returns the {@link AttributeDescription} of the
     * {@link Attribute} owned by this ResourceImpl
     * and whose name is passed as parameter
     *
     * @param name the name of the {@link Attribute} to return the
     *             description of
     * @return the {@link AttributeDescription} of the {@link Attribute}
     * whose name is passed as parameter or null if this ResourceImpl
     * does not own a {@link Attribute} with the specified name
     */
    ...
    /**
     * Returns the array of {@link Description}s of all
     * owned {@link Describable}s
     *
     * @return the {@link Description}s of all
     * owned {@link Describable}s
     */
    ...
    /**
     * Returns the name of the default {@link Attribute}
     * of this ResourceImpl
     *
     * @return the name of this ResourceImpl's default
     * {@link Attribute}
     */
    ...
    /**
     * Defines the name of the default {@link Attribute}
     * of this ResourceImpl
     *
    ...
    /**
     * This ResourceImpl is hidden if its default
     * attribute is
     *
     * @return <ul>
    ...
    /**
     * Returns the extended {@link Resource} type
     * of this resource
     *
     * @return the extended {@link Resource} type of
     * this resource
     */
    ...
    /**
     * Registers a listener for the attribute whose name
     * is passed as parameter
     *
     * @param attributeName the name of the attribute to listen
     * @param recipient     the {@link Recipient} to callback
     * @param conditions    the set of {@link Constraint}s applied
     *                      on the listened events
     * @return a registration identifier if the listener has
     * been registered properly; null otherwise
     */
    protected String listen(String attributeName, Recipient recipient, Set<Constraint> conditions, MidCallback.Type type, long lifetime, int buffer, int delay) {
        ...
        switch (callbackType) {
            case BUFFERERIZED_AND_SCHEDULED:
                callback = new ScheduledBufferMidCallback(super.modelInstance.mediator(), callbackId, null, recipient, timeout, schedulerDelay, bufferSize);
                break;
            case BUFFERIZED:
                callback = new BufferMidCallback(super.modelInstance.mediator(), callbackId, null, recipient, timeout, bufferSize);
                break;
            case SCHEDULED:
                callback = new ScheduledMidCallback(super.modelInstance.mediator(), callbackId, null, recipient, timeout, schedulerDelay);
                break;
            case UNARY:
        ...
        if (callback != null) {
            super.modelInstance.registerCallback(new SubscriptionFilter(super.modelInstance.mediator(), filter, conditions), callback);
    ...
    /**
     * Unregisters the listener whose callback's identifier
     * is passed as parameter
     *
     ...
     *
     * @param act the {@link AccessMethod.Type} of the {@link AccessMethod}
     *            to return
     * @return the {@link AccessMethod} of this resource of the
     * specified type
     */
    ...
    /**
     * Registers a {@link Signature} to create and to associate
     * with the specified {@link AccessMethodExecutor}
     *
     * @param type           the type of the {@link AccessMethod} associated
     *                       to the signature to create
     * @param parameterNames the array of parameter names of the signature
     *                       to create
     * @param parameterTypes the array of parameter types of the signature
     *                       to create
     * @param executor       the {@link AccessMethodExecutor} to associate to
    ...
    /**
     * Registers the {@link Signature} passed as parameter
     * and associates it with the specified
     * {@link AccessMethodExecutor}
     *
     * @param signature the {@link Signature} to register
     * @param executor  the {@link AccessMethodExecutor} to associate
     *                  to the {@link Signature} to register
     * @param policy    the execution policy of the specified
     *                  {@link AccessMethodExecutor}
     * @throws InvalidConstraintDefinitionException
     * @throws InvalidValueException
     */
    ...
    /**
     * Returns the {@link Enumeration} of the set
     * of {@link Attribute}s of this ResourceImpl
     *
     * @return the the {@link Enumeration} of this
     * ResourceImpl's {@link Attribute}s
     */
    ...
    protected void updated(Attribute attribute, Object value, boolean hasChanged) {
        ...
        for (; index < length; index++) {
            ...
            SnaUpdateMessage message = SnaNotificationMessageImpl.Builder.<SnaUpdateMessage>notification(super.modelInstance.mediator(), SnaUpdateMessage.Update.ATTRIBUTE_VALUE_UPDATED, path);
    ...
    /**
     * Registers the path of a {@link LinkedResourceImpl} linked
     * to this ResouceImpl instance
     *
     * @param path the path of the {@link LinkedResourceImpl} linked
     *             to this ResouceImpl instance
     */
    ...
    public void start() {
        ...
        SnaLifecycleMessage notification = SnaNotificationMessageImpl.Builder.<SnaLifecycleMessage>notification(super.modelInstance.mediator(), SnaLifecycleMessage.Lifecycle.RESOURCE_APPEARING, path);
        ...
        if (this.defaultAttribute != null && (attribute = this.getAttribute(this.defaultAttribute)) != null) {
            ...
            for (; index < length; index++) {
                ...
                if (Modifiable.FIXED.equals(metadataDescription.getModifiable()) && Metadata.LOCKED.intern() != metadataName && Metadata.MODIFIABLE.intern() != metadataName && Metadata.HIDDEN.intern() != metadataName && Attribute.NICKNAME.intern() != metadataName) {
                    jsonAttribute.put(metadataDescription.getName(), PrimitiveDescription.toJson(metadataDescription.getType(), metadataDescription.getValue()));
                }
            }
    ...
    /**
     * Deletes all associated access method
     * and the link with its description
     * object
     */
    public void stop() {
        if (this.isHidden()) {
     ...
     *               to register
     * @param method the {@link AccessMethod} to register
     */
     ...
     *
     * @param updatePolicy this resource's update policy
     */
