...
public abstract class DescribeResponse<T> extends AccessMethodResponse<T> {
	...
	/**
	 * Returns the JSON formated String description of this DescribeResponse
	 * 
	 * @param raw
	 *            defines whether only the embedded response entry have to be output
	 * 
	...
	public String getJSON(boolean raw) {
		...
		while (iterator.hasNext()) {
			...
			if ("type".equals(typedKey.getName())) {
				builder.append(JSONUtils.toJSONFormat(this.describeType.getTypeName()));
			} else {
