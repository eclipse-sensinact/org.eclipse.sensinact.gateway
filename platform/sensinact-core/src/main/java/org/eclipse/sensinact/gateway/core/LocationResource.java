...
public interface LocationResource extends PropertyResource {
	/**
	 * Enumeration of types of LocationResources. The type of a LocationResource is
	 * useful for coordinates and distances interpretations and/or calculations
	 */
	public static enum LRType {
		/**
		 * Coordinates of this type of LocationResource are interpreted as being, in
		 * order, the latitude, the longitude and the altitude
		 */
		L_L_A_GEOCENTRIC,
		/**
		 * Coordinates of this type of LocationResource are interpreted as being, in
		 * order, the abscissa, the ordinate and the height in an orthonormal coordinate
		 * system
		 */
		X_Y_Z_ORTHONORMAL,
		/**
		 * Coordinates of this type of LocationResource are interpreted as being, in
		 * order, a linear distance from the origin of an orthonormal coordinate system,
		 * an angle defining the gradient, and the height.
		 */
	...
	static final AttributeBuilder[] ATTRIBUTES = new AttributeBuilder[] {
			...
			new AttributeBuilder(ABSOLUTE_ATTRIBUTE_NAME,
					new AttributeBuilder.Requirement[] { AttributeBuilder.Requirement.TYPE })
							.type(ABSOLUTE_ATTRIBUTE_CLASS).value(new Boolean(false)),

			new AttributeBuilder(LRTYPE_ATTRIBUTE_NAME,
					new AttributeBuilder.Requirement[] { AttributeBuilder.Requirement.TYPE })
							.type(LRTYPE_ATTRIBUTE_CLASS).value(LRType.X_Y_Z_ORTHONORMAL),

			new AttributeBuilder(COORDINATES_ATTRIBUTE_NAME,
					new AttributeBuilder.Requirement[] { AttributeBuilder.Requirement.TYPE })
							.type(COORDINATES_ATTRIBUTE_CLASS).value(new float[] { 0.0f, 0.0f, 0.0f }),

			new AttributeBuilder(LINEAR_MEASURE_UNIT_ATTRIBUTE_NAME,
					new AttributeBuilder.Requirement[] { AttributeBuilder.Requirement.TYPE })
							.type(LINEAR_MEASURE_UNIT_ATTRIBUTE_CLASS).value(DEFAULT_LINEAR_MEASUREMENT_UNIT),

			new AttributeBuilder(ANGULAR_MEASURE_UNIT_ATTRIBUTE_NAME,
					new AttributeBuilder.Requirement[] { AttributeBuilder.Requirement.TYPE })
							.type(ANGULAR_MEASURE_UNIT_ATTRIBUTE_CLASS).value(DEFAULT_ANGULAR_MEASUREMENT_UNIT),

			new AttributeBuilder(REFERENCE_POINT_ATTRIBUTE_NAME,
					new AttributeBuilder.Requirement[] { AttributeBuilder.Requirement.TYPE })
							.type(REFERENCE_POINT_ATTRIBUTE_CLASS) };
}
