...
{		
	 ...
	 * @param set 
	 */
    public SubscriptionFilter(Mediator mediator, 
    		String filter, Set<Constraint> conditions)
    {
	 ...
	 */
    public SubscriptionFilter(Mediator mediator, 
    		String filter, JSONArray conditions)
    {
	...
	{		
		...
		JSONObject jsonObject = (JSONObject) 
                    ((TypedProperties<SnaUpdateMessage.Update>)
			message).get(SnaConstants.NOTIFICATION_KEY);	
		...
		Object object = CastUtils.cast(super.mediator.getClassLoader(), 
				clazz, jsonObject.opt(DataResource.VALUE));
		
		Iterator<Constraint> iterator = 
				this.conditions.iterator();
