...
public class LinkedActMethod extends ActMethod {
	...
	/**
	 * Links all registered executors of the {@link ActMethod} passed as parameter
	 * to this LinkedActMethod
	 * 
	 * @param linked
	...
	private final void linkedSignatures() {
		...
		Iterator<Map.Entry<Signature, Deque<AccessMethodExecutor>>> signaturesIterator = linked.map.entrySet()
				.iterator();
	...
	public final Signature linkedSignature(Signature signature) {
		...
		if (linked == null || signature == null || signature.getName().intern() != super.getName().intern()
				|| (thatSignature = linked.getSignature(signature.getParameterTypes())) == null) {
			return null;
		}
