...
public class LinkedActMethod extends ActMethod {
    ...
    private final void linkedSignatures() {
        ...
        Iterator<Map.Entry<Signature, Deque<AccessMethodExecutor>>> signaturesIterator = linked.map.entrySet().iterator();

        while (signaturesIterator.hasNext()) {
            Map.Entry<Signature, Deque<AccessMethodExecutor>> entry = signaturesIterator.next();

            super.map.put((Signature) entry.getKey().clone(), new LinkedList<AccessMethodExecutor>(entry.getValue()));
        }
        Iterator<Map.Entry<Shortcut, Signature>> shortcutsIterator = linked.shortcuts.entrySet().iterator();

        while (shortcutsIterator.hasNext()) {
            Map.Entry<Shortcut, Signature> entry = shortcutsIterator.next();

            super.shortcuts.put((Shortcut) entry.getKey().clone(), (Shortcut) entry.getValue().clone());
        }
    }
     ...
     *
     * @param linked    the {@link ActMethod} to link with
     * @param Signature the {@Signature} of the {@link ActMethod}
     *                  to link with
     */
    public final Signature linkedSignature(Signature signature) {
        Signature thatSignature = null;

        if (linked == null || signature == null || signature.getName().intern() != super.getName().intern() || (thatSignature = linked.getSignature(signature.getParameterTypes())) == null) {
            return null;
        }
    ...
    public final void createShortcut(Signature signature, Shortcut shortcut) {
        ...
        super.map.put(new Shortcut(super.mediator, signature.getName(), signature.getResponseType(), parameters.toArray(new Parameter[parameters.size()]), fixedParameters), new LinkedList<AccessMethodExecutor>(executors));
    }
}
