...
import org.eclipse.sensinact.gateway.api.message.MessageFilterDefinition;
...
import org.eclipse.sensinact.gateway.api.message.SnaMessage.Type;
import org.eclipse.sensinact.gateway.api.message.annotation.Filter;
import org.eclipse.sensinact.gateway.api.message.whiteboard.AgentRelay;
...
/**
 * A AgentFactory is in charge of asking for the registration of an {@link SnaAgent}  
...
public class AgentFactory {
	...
	private final class AgentRelayWrapper implements AgentRelay, MessageFilterDefinition {
		...
		/* (non-Javadoc)
		 * @see org.eclipse.sensinact.gateway.api.message.AgentRelay#getPublicKey()
		 */
		@Override
		public String getPublicKey() {
			return agentRelay.getPublicKey();
		}
	...
	private final class AgentCallback extends AbstractMessageAgentCallback  {
     ...
     * Constructor
     *
     * @param mediator the {@link Mediator} allowing the AgentFactory
     * to be instantiated to interact with the OSGi host environment
     */
    public AgentFactory(Mediator mediator) {
     ...
     * registering a newly created {@link SnaAgent} based on it
     *
	...
	public final void attach(AgentRelay agentRelay) {
	    ...
	    Type[] handled = wrapper.handledTypes();
	    if(handled == null) {
	    	handled = Type.values();
	    } else {
	    	defined = true;
	    ...
	    if(defined) {
	    	filter = new MessageFilter(mediator, sender, isPattern, isComplement); 
     ...
     * unregistering the {@link SnaAgent} that is based on it
     *
