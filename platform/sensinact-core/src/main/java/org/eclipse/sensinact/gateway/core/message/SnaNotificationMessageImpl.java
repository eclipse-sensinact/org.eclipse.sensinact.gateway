...
public abstract class SnaNotificationMessageImpl<S extends Enum<S> & SnaMessageSubType & KeysCollection> extends AbstractSnaMessage<S> implements SnaNotificationMessage<S> {
        ...
        public static final <N extends SnaNotificationMessage<?>> N notification(Mediator mediator, SnaMessageSubType notificationType, String uri) {
            ...
            switch (notificationType.getSnaMessageType()) {
                case ERROR:
                    break;
                case LIFECYCLE:
                    notification = (N) new SnaLifecycleMessageImpl(mediator, uri, (SnaLifecycleMessage.Lifecycle) notificationType);
                    break;
                case RESPONSE:
                    break;
                case UPDATE:
     ...
     * message
     *
     * @param key the string key for which to retrieve
     *            the object value
     * @return the object value associated to the specified
     ...
     *
     * @param type the type into which to cast the object value mapped
     *             to the specified key
     * @param key  the string key for which to retrieve
     *             the object value
     * @return the casted object value associated to the specified
     * key in the notification {@link JSONObject}
     */
