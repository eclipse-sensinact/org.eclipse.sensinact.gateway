 ...
 */
public abstract class AccessMethodJSONResponse 
extends AccessMethodResponse<JSONObject>
{
	/**
	 * @param uri
	 * @param type
	 * @param status
	 */
    protected AccessMethodJSONResponse(Mediator mediator, 
    		String uri, Response type, Status status)
    {
    	this(mediator, uri,type, status,(status == Status.SUCCESS)
		?AccessMethodJSONResponse.SUCCESS_CODE: UNKNOWN_ERROR_CODE);
    }
