...
//		System.out.println("______________________________");
    ...
    public void updateContent(SnaLifecycleMessage.Lifecycle lifecycle, String uri, JSONObject initial, String type) {
        ...
        boolean added = !lifecycle.equals(Lifecycle.RESOURCE_DISAPPEARING) && !lifecycle.equals(Lifecycle.SERVICE_DISAPPEARING);
...
//					System.out.println("______________________________");
                    ...
                    properties.put(new StringBuilder().append(resourceKey).append(".").append(attribute).toString(), value);
                }
            }
        }
        ...
        for (; index < typesLength; index++) {
            ...
            properties.put(new StringBuilder().append(resourceKey).append(".").append(accessMethodTypes[index].name()).toString(), accessLevelOption.getAccessLevel().getLevel());
        }
    }
    ...
    private final void updateResourceDisappearing(String service, String resource, MutableAccessNode node, Dictionary<String, Object> properties) {
        ...
        int index = 0;
        for (; index < typesLength; index++) {
            properties.remove(new StringBuilder().append(resourceKey).append(".").append(accessMethodTypes[index].name()).toString());
        }
    }
    ...
    private final void updateService(String service, MutableAccessNode node, boolean added, Dictionary<String, Object> properties) {
        ...
        if (added) {
            ...
            for (; index < typesLength; index++) {
                ...
                properties.put(new StringBuilder().append(service).append(".").append(accessMethodTypes[index].name()).toString(), accessLevelOption.getAccessLevel().getLevel());
            }
...
//					System.out.println(obs);
                ...
                if (obs != null && !obs.isEmpty() && obs.contains(uriElements[3])) {
                    ...
                    this.updateObserved(new StringBuilder().append(key).append(".").append(uriElements[3]).toString(), value);
                }
                break;
            case LIFECYCLE:
