...
public class ServiceImpl extends ModelElement<ModelInstance<?>, ServiceProxy, ServiceProcessableData<?>, ResourceImpl, Resource> {
    class ServiceProxyWrapper extends ModelElementProxyWrapper implements ResourceCollection {
        ...
        public Object invoke(Object proxy, Method method, Object[] parameters) throws Throwable {
            ...
            if (this.proxy.getProxied().isAssignableFrom(method.getDeclaringClass())) {
                ...
                if (method.isVarArgs() && parameters != null && parameters.length == 1 && parameters[0].getClass().isArray()) {
     ...
     * @throws InvalidServiceException if an error occurred while instantiating the service
     */
    ...
    public void process(ServiceProcessableData<?> data) {
        ...
        if (resource == null) {
            ResourceDescriptor descriptor = super.getModelInstance().configuration().getResourceDescriptor().withServiceName(super.getName()).withResourceName(resourceId);

            ResourceBuilder builder = super.getModelInstance().getResourceBuilder(descriptor, this.modelInstance.configuration().getResourceBuildPolicy());
        ...
        if (resource == null) {
            super.modelInstance.mediator().warn("Resource '%s' not found for '%s' service", resourceId, super.getName());
            return;
        }
     ...
     * to this ServiceImpl
     *
     * @param resourceClass extended {@link DataResource} type to instantiate
     * @param name          the name of the {@link DataResource} to instantiate
     * @param type          the type of the 'value' {@link Attribute} of the
     *                      {@link DataResource} to instantiate
     * @param value         the value of the 'value' {@link Attribute} of the
     *                      {@link DataResource} to instantiate
     * @param attributes    extended set of attributes to add to the resource to
     *                      create
    ...
    public <D extends DataResource> ResourceImpl addDataResource(Class<D> resourceClass, String name, Class<?> type, Object value) throws InvalidResourceException {
        ResourceDescriptor descriptor = super.getModelInstance().configuration().getResourceDescriptor().withServiceName(this.getName()).withResourceName(name).withResourceType(resourceClass).withDataType(type).withDataValue(value);
     ...
     * service
     *
     * @param name          the name of the {@link ActionResource} to instantiate
     * @param resourceClass extended {@link ActionResource} type to instantiate
     * @param attributes    extended set of attributes to add to the resource to
     *                      create
    ...
    public <D extends ActionResource> ResourceImpl addActionResource(String name, Class<D> resourceClass) throws InvalidResourceException {
        ResourceDescriptor descriptor = super.getModelInstance().configuration().getResourceDescriptor().withServiceName(this.getName()).withResourceName(name).withResourceType(resourceClass);
     ...
     * this service
     *
     * @param resource the {@link ResourceImpl} to which to link the
     *                 {@link LinkedResourceImpl} to instantiate
     * @param name     the name of the {@link LinkedResourceImpl} to
     *                 instantiate
    ...
    public ResourceImpl addLinkedResource(String link, ResourceImpl targetedResource) throws InvalidResourceException {
        ...
        ResourceBuilder builder = super.getModelInstance().createResourceBuilder(super.getModelInstance().configuration().getResourceDescriptor().withResourceName(link).withResourceType(targetedResource.getResourceType()));
     ...
     *
     * @param resource      the {@link Resource.Type.ACTION} typed
    ...
    public ResourceImpl addLinkedActionResource(String link, ResourceImpl targetedResource, boolean copyActMethod) throws InvalidResourceException {
        if (targetedResource == null || targetedResource.getType() != Resource.Type.ACTION || this.getResource(link) != null) {
            super.modelInstance.mediator().debug(new StringBuilder().append("Unable to create the resource - Invalid Name :").append(link).toString());
            return null;
        }
        ResourceBuilder builder = super.getModelInstance().createResourceBuilder(super.getModelInstance().configuration().getResourceDescriptor().withResourceName(link).withResourceType(targetedResource.getResourceType()));
     ...
     *
     * @param listen    the name of the action resource
     * @param target    the name of the state variable resource to
     *                  link to the action resource
     * @param signature the signature to which attach the specified
     *                  {@link AccessMethodTrigger}
     * @param trigger   the {@link AccessMethodTrigger} to call when the
     *                  specified action resource is executed
     * @throws InvalidValueException
     */
    public <P> void addActionTrigger(String listen, final String target, Signature signature, final AccessMethodTrigger<P> trigger, AccessMethodExecutor.ExecutionPolicy policy) throws InvalidValueException {
        ...
        if (actuator == null || !ActionResource.class.isAssignableFrom(actuator.getResourceType()) || variable == null || !StateVariableResource.class.isAssignableFrom(variable.getResourceType())) {
     ...
     *
     * @param resourceName the name of the {@link ResourceImpl} for which
     ...
     * @param resource   the name of the targeted resource
     ...
     * parameter
     *
     * @param resource the name of the {@link ResourceImpl}
     * @return the {@link ResourceImpl} with the specified
     * name
     */
     ...
     *
     * @return the set of this ServiceImpl's
     * {@link ResourceImpl}s
     */
     ...
     *
     * @return the number of registered subscriptions for this
     * service
     */
