...
public class Attribute extends DescribablePrimitive
{
	...
	public Attribute(Mediator mediator, 
			...
			throws InvalidValueException
	{
		this(mediator, resource, name, type, null, 
	...
	public Attribute(Mediator mediator, 
			ResourceImpl resource,
			String name, Class<?> type, 
			Object value, Modifiable modifiable, 
			boolean hidden) 
			throws InvalidValueException
	{
		...
		Metadata modifiableMeta = new Metadata(
				super.mediator,
				Metadata.MODIFIABLE,
		        Modifiable.class, modifiable, 
		        Modifiable.FIXED);
		this.addMetadata(modifiableMeta);

		Metadata hiddenMeta = new Metadata(
				super.mediator,
				Metadata.HIDDEN, 
		...
		Metadata timestampMeta = new Metadata(
				super.mediator,
				Metadata.TIMESTAMP,  
				long.class, 
		...
		Metadata lockedMeta = new Metadata(
				super.mediator,
				Metadata.LOCKED,
		        boolean.class, 
		        false, 
	...
	{		
		super(mediator, 
			  attribute==null?null:attribute.optString(
					PrimitiveDescription.NAME_KEY),
			  attribute==null?null:attribute.optString(
						PrimitiveDescription.TYPE_KEY));	  
		
		this.resource = resource;
		this.metadata = new ArrayList<Metadata>();
		
		// set the value if defined in the JSONObject
		Object ovalue = attribute.opt(
				PrimitiveDescription.VALUE_KEY);
		
		if (ovalue != null)
		{
			this.setValue(CastUtils.getObjectFromJSON(
					super.mediator.getClassLoader(),
					this.getType(), ovalue));
		}
		JSONArray metadataArray = attribute.optJSONArray(
				"metadata");
	 ...
	 *     the value to set
	 * @return 
	 * 		the extended {@link Description} describing the 
	 * 		modified Metadata {@link Metadata}
	 * 
	 * @throws InvalidValueException
	 */
	public MetadataDescription setMetadataValue(String name, 
	...
	public void addMetadata(Metadata metadata)
	{
		...
		synchronized(this.metadata)
		{
			...
			} else
			{
				try 
				{
					this.setMetadataValue(metadata.getName(),
							metadata.getValue());
	...
	{
		if (metadata == null ||
				Metadata.HIDDEN.intern() == metadata.intern() ||
				Metadata.MODIFIABLE.intern() == metadata.intern() ||
				Metadata.TIMESTAMP.intern() == metadata.intern()||
	 ...
	 *       if the value cannot be set
	 */
    public Object setValue(Object value, long timestamp)
    throws InvalidValueException
	{	    	
		...
		{	
            boolean hasChanged = 
            		((valueObject==null && super.value!=null)
                    || (valueObject!=null 
                    && !valueObject.equals(super.value)));
	 ...
	 */
    @Override
    protected void beforeChange(Object value) 
    		throws InvalidValueException
    {
     ...
     * @throws InvalidValueException 
     */
    private void afterChange(Object value,boolean hasChanged) 
    		throws InvalidValueException
    {
	...
	{	
		...
		if((metadata=this.get(
				Metadata.MODIFIABLE))==null)
		{
			return modifiable;
		}
		try
	...
	{	
		...
		try
		{
			locked = (Boolean) this.get(Metadata.LOCKED
					).getValue();
	 ...
	 * @return
	 * 		the  {@link Metadata} with the specified 
	 * 		name 
	 */
	protected Metadata get(String name)
	{
		...
		synchronized(this.metadata)
		{
			if((index = this.metadata.indexOf(
					new Name<Attribute>(name))) != -1)
			{ 
	 ...
	 * 
	 * @return 
	 * 		the {@link MetadataDescription}s array of 
	 * 		this Attribute
	 */
	public MetadataDescription[] getAllDescriptions()
	{
		int index = 0;
		MetadataDescription[] descriptions = 
				new MetadataDescription[this.metadata.size()];
