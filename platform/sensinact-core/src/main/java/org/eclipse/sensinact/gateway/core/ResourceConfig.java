...
public class ResourceConfig implements Nameable {
     ...
     * ResourceConfig
     *
     * @param policy the {@link TypeConfig} which applies
     */
     ...
     * ResourceConfig
     *
     * @param updatePolicy the {@link UpdatePolicy} which applies
     */
     ...
     * returns false
     *
     * @return true if the service whose identifier is passed as
    ...
    public boolean isTargeted(String serviceId) {
        ...
        for (int index = 0; index < this.targets.length; index++) {
            if (serviceId.equals(targets[index]) || (!serviceId.equals(ServiceProvider.ADMINISTRATION_SERVICE_NAME) && targets[index].equals(ResourceConfig.ALL_TARGETS))) {
                return true;
            }
        }
        return false;
     ...
     *
     * @param policy specific profile for this XmlResourceConfig
     */
     ...
     *
     * @param requirementBuilder the {@link RequirementBuilder} to add
     */
