...
public class ResourceConfig implements Nameable
{
    ...
    public String getName(String service)
    {
		...
		String name = null;
		int index  = -1;
		if((index = this.requirementBuilders.indexOf(
			new Name<RequirementBuilder>(Resource.NAME)))>-1)
		{
			name = (String) this.requirementBuilders.get(
				index).get(service);
		}	
		return name;
    }
    ...
    /**
     * Returns the {@link TypeConfig} which applies
     * on {@link ResourceImpl} instances based on this
     * ResourceConfig
     * 
     * @return
    ...
    /**
     * Defines the {@link TypeConfig} which applies
     * on {@link ResourceImpl} instances based on this
     * ResourceConfig
     * 
     * @param policy
    ...
    /**
	 * Defines the {@link UpdatePolicy} which applies
     * on {@link ResourceImpl} instances based on this
     * ResourceConfig
     * 
     * @param updatePolicy
	...
	/**
	 * Returns the {@link UpdatePolicy} which applies
     * on {@link ResourceImpl} instances based on this
     * ResourceConfig
     * 
     * @return
    ...
    /**
     * Returns true if the service whose identifier is passed 
     * as parameter is targeted by this ResourceConfig; otherwise 
     * returns false 
     * 
     * @return 
     *      true if the service whose identifier is passed as 
     *      parameter is defined as targeted; <br/> false 
     *      otherwise
     */
    ...
    {    
        ...
        {
            if(serviceId.equals(targets[index]) ||
            (!serviceId.equals(ServiceProvider.ADMINISTRATION_SERVICE_NAME)
    ...
    /**
     * Defines specific {@link Service} targets for this 
     * XmlResourceConfig
     * 
     * @param target
    ...
    /**
     * Returns true if the profile whose identifier is passed 
     * as parameter is one for which this ResourceConfig; otherwise 
     * returns false 
     * 
     * @return 
     *      true if the service whose identifier is passed as 
    ...
    {    
        ...
        return (this.profile.equals(profileId) 
        	||this.profile.equals(ResourceConfig.ALL_PROFILES));
    }
	...
	/**
	 * Adds the {@link RequirementBuilder} passed as parameter,
	 * that will be applied on the set of {@link AttributeBuilder}s 
	 * used to create the set of {@link Attribute}s of the {@link 
	 * ResourceImpl} instances based on this ResourceConfig
	 * 
	...
	public List<AttributeBuilder> getAttributeBuilders(String service)
	{
		List<AttributeBuilder> builders = 
				this.typeConfig.getAttributeBuilders();
		
		Iterator<RequirementBuilder> iterator = 
				this.requirementBuilders.iterator();
