...
public class RemoteSensiNact implements RemoteCore {
    ...
    final class RemoteSensiNactCallback extends AbstractMidAgentCallback {
        ...
        @Override
        public void stop() {
            RemoteSensiNact.this.mediator.debug("RemoteSensiNactCallback '%s' stopped", super.identifier);
        }
    }
     ...
     */
    protected RemoteSensiNact(Mediator mediator, LocalEndpoint localEndpoint) {
    ...
    public void connect(final String namespace) {
        ...
        this.registration = AccessController.<ServiceRegistration<RemoteCore>>doPrivileged(new PrivilegedAction<ServiceRegistration<RemoteCore>>() {
            @Override
            public ServiceRegistration<RemoteCore> run() {
                Dictionary<String, Object> props = new Hashtable<String, Object>();
                props.put(NAMESPACE_PROP, namespace);

                ServiceRegistration<RemoteCore> reg = mediator.getContext().registerService(RemoteCore.class, RemoteSensiNact.this, props);
        ...
        if (this.registration != null) {
            ...
            if (!this.subscriptionIds.isEmpty()) {
                ...
                for (; index < length; index++) {
                    ...
                    JSONObject response = this.localEndpoint.unsubscribe(ref.publicKey, ref.serviceProviderId, ref.serviceId, ref.resourceId, keys[index]);
                }
            }
            if (this.onDisconnectedCallback != null) {
     ...
     */
    @Override
    public JSONObject subscribe(String publicKey, String serviceProviderId, String serviceId, String resourceId, JSONArray conditions) {
        JSONObject response = this.subscribe(publicKey, serviceProviderId, serviceId, resourceId, this.remoteEndpoint, conditions);
        return response;
    }
    ...
    public JSONObject subscribe(String publicKey, String serviceProviderId, String serviceId, String resourceId, Recipient recipient, JSONArray conditions) {
        JSONObject response = this.localEndpoint.subscribe(publicKey, serviceProviderId, serviceId, resourceId, recipient, conditions);

        if (response.optInt("statusCode") == 200) {
            try {
                String subscriptionId = response.getJSONObject("response").getString("subscriptionId");

                this.subscriptionIds.put(subscriptionId, new SubscriptionReference(publicKey, serviceProviderId, serviceId, resourceId));
    ...
    public JSONObject unsubscribe(String publicKey, String serviceProviderId, String serviceId, String resourceId, String subscriptionId) {
        ...
        JSONObject response = this.localEndpoint.unsubscribe(publicKey, serviceProviderId, serviceId, resourceId, subscriptionId);
        return response;
    }
    ...
    public void registerAgent(final String remoteAgentId, final SnaFilter filter, final String publicKey) {
        ...
        if (!JSONObject.NULL.equals(response) && (localAgentId = (String) response.opt("subscriptionId")) != null) {
    ...
    private void callAgent(final String agentId, final boolean local, final Executable<SnaAgent, Void> executable) {
        AccessController.<Void>doPrivileged(new PrivilegedAction<Void>() {
            @Override
            public Void run() {
                return RemoteSensiNact.this.mediator.callService(SnaAgent.class, new StringBuilder().append("(&(org.eclipse.sensinact.gateway.agent.id=").append(agentId).append(")(org.eclipse.sensinact.gateway.agent.local=").append(Boolean.toString(local)).append("))").toString(), executable);
            }
        });
    }
}
