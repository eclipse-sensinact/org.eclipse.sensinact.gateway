...
public interface SnaMessage<S extends Enum<S> & SnaMessageSubType & KeysCollection> extends Typable<S>, PathElement, JSONable {
    ...
    public static final TypedKey<?>[] PERMANENT_KEYS = new TypedKey<?>[]{new TypedKey<Integer>(SnaConstants.ACCESS_LEVEL, int.class, true), new TypedKey<Enum>(SnaConstants.TYPE_KEY, Enum.class, false), new TypedKey<String>(SnaConstants.URI_KEY, String.class, false)};
    ...
    static final class KeysBuilder {
        ...
        public TypedKey<?>[] keys() {
            ...
            LinkedList<Class<SnaMessage>> list = ReflectUtils.getOrderedImplementedInterfaces(SnaMessage.class, extended);

            Iterator<Class<SnaMessage>> iterator = list.descendingIterator();
            while (iterator.hasNext()) {
                TypedKey<?>[] tmpKeys = ReflectUtils.getConstantValue(iterator.next(), SnaMessage.PERMANENT_KEYS_PROPERTY);
