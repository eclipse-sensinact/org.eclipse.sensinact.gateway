...
public class Sessions {
    ...
    private static class Entry extends WeakReference<Object> implements Map.Entry<Session, SessionKey> {
         ...
         * Creates new entry.
         */
        Entry(Object key, SessionKey value, SessionObserver observer, ReferenceQueue<Object> queue, int hash, Entry next) {
            super(key, queue);
            this.value = value;
            this.hash = hash;
            this.next = next;
    ...
    public Sessions() {
        int capacity = 1;
        while (capacity < DEFAULT_INITIAL_CAPACITY) capacity <<= 1;
    ...
    public SessionKey get(Object key) {
        ...
        Entry e = tab[index];
        while (e != null) {
            if (e.hash == h && ((KeyExtractor.class.isAssignableFrom(key.getClass()) && eq(k, e.getValue())) || (Session.class.isAssignableFrom(key.getClass()) && eq(k, e.getKey())))) {
                return e.value;
            }
            e = e.next;
        }
     ...
     * is enqueued
     */
    public void put(SessionKey key, Session value, SessionObserver observer) {
        if (key == null) {
            return;
        }
