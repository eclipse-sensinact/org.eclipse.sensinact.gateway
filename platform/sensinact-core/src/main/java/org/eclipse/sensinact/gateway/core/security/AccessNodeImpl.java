 ...
 */
public class AccessNodeImpl<N extends AccessNodeImpl<N>> extends PathNode<N>
implements MutableAccessNode
{
			...
			AccessLevelOption optionLevel, boolean accessibility)
	{
		...
		MethodAccessibilityImpl methodAccessibilities = 
			new MethodAccessibilityImpl(accessMethod, optionLevel, 
					accessibility);
	...
	@Override
	public <P extends ImmutablePathNode<P>> P immutable(Class<P> ic,
			P parent)
	{
		return (P) new ImmutableAccessNode((ImmutableAccessNode) parent, 
			this.nodeName, this.isPattern, super.children, 
				this.profile);
	}
