...
/**
 * An SnaMessage provide information about a {@link ServiceProvider}, a
 * {@link Service}, or a {@link Resource} of sensiNact. It can be the response
 * to an access method call, relative to the lifecycle of an entity of the
 * system or an value update, or an error
 * 
 * @author <a href="mailto:cmunilla@cmssi.fr">Christophe Munilla</a>
 */
public interface SnaMessage<S extends Enum<S> & MessageSubType & KeysCollection>
	...
	static final class KeysBuilder {
		...
		public TypedKey<?>[] keys() {
			...
			LinkedList<Class<SnaMessage>> list = ReflectUtils.getOrderedImplementedInterfaces(SnaMessage.class,
					extended);

			Iterator<Class<SnaMessage>> iterator = list.descendingIterator();
			while (iterator.hasNext()) {
				TypedKey<?>[] tmpKeys = ReflectUtils.getConstantValue(iterator.next(),
						SnaMessage.PERMANENT_KEYS_PROPERTY);
