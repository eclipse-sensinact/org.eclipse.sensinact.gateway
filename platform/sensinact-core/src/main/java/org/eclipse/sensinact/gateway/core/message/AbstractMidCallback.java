...
{	
	...
	/**
	 * Transmits the {@link SnaMessage} passed as parameter to
	 * the final recipient of the callback process
	 *  
	 * @param message the {@link SnaMessage} to be sent
	 */
	 ...
	 * MidCallback} to be instantiated
	 *  
	 * @param buffer Defines whether the {@link MidCallback} 
	 * to be instantiated will use a fifo buffer to store 
	 * received messages before treating them
	 */
	protected AbstractMidCallback(boolean buffer)
	{
		this.setTimeout(MidCallback.ENDLESS);
		this.registerer = buffer?new MessageRegistererBufferDelegate()
				:new MessageRegistererDirectDelegate();
	}
	 ...
	 * 
	 * @param errorHandler the {@link ErrorHandler} to be set
	 */
	 ...
	 * 
	 * @param status the {@link AccessMethodResponse.Status} 
	 * of this callback
	 */
     ...
     * 
     * @param timeout the timeout long value of this
     * {@link MidCallback}
     */
    ...
    /**
     * Stops this {@link MidCallback} and all its associated
     * processes
     */
    public void stop()
    {
    	...
    	{
    		((MessageRegistererBufferDelegate)this.registerer
    				).stop();
    	}
    }
}