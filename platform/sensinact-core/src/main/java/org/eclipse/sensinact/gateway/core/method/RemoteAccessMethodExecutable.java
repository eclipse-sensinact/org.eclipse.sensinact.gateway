...
public class RemoteAccessMethodExecutable implements Executable<RemoteCore, JSONObject>
{
	...
	public static final TypedKey<String> SUBSCRIPTION_ID_TK = new TypedKey<String>(SUBSCRIPTION_ID,String.class,false);
	...
	public JSONObject execute(RemoteCore core) throws Exception
	{
		...
		{
			case "ACT":
				 json = endpoint.act(publicKey, provider, service, resource, 
					this.<Object[]>get(ARGUMENTS_TK));
				break;
			case "GET":
				json = endpoint.get(publicKey, provider, service, resource, attribute);
				break;
			case "SET":				
				json = endpoint.set(publicKey, provider, service, resource, attribute, 
					this.<Object>get(VALUE_TK));
				break;
			case "SUBSCRIBE":				
				json = endpoint.subscribe(publicKey, provider, service, resource, 
					this.<Recipient>get(RECIPIENT_TK), 
					this.<JSONArray>get(CONDITIONS_TK));
				break;
			case "UNSUBSCRIBE":				
				json =  endpoint.unsubscribe(publicKey, provider, service, resource, 
					this.<String>get(SUBSCRIPTION_ID_TK));
				break;
			default:
				break;
		}
