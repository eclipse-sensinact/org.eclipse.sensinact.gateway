...
/**
 * Allows to check whether an {@link SnaMessage} has to be 
 * transmitted or blocked, according to its type, the uri of 
 * the object sending it, or a set of registered conditions
 *  
...
{		
	...
	/**
	 * List of {@link Constraint}s applying on 
	 * received events
	 */
	...
	/**
	 * Is this filter's 'matches' method returns the
	 * the logical complement of the target and conditions
	 * evaluation result ? 
	 */
	protected final boolean isComplement;

	/**
	 * the changing contraint applying on the 
	 * evaluated value(s)
	 */
	 ...
	 */
	public SnaFilter(Mediator mediator, String sender, 
			boolean isPattern, boolean isComplement)
	{
     ...
     * @param constraints
     */
    public SnaFilter(Mediator mediator, 
    		String sender, JSONArray constraints)
    {
     ...
     */
    public SnaFilter(Mediator mediator, 
    		String sender, boolean isPattern, boolean isComplement,
	 ...
	 * 
	 * @param jsonCondition
	 * 		the JSON formated {@link Constraint} to
	 * 		add to this filter
	 */
	...
	{		
        ...
        {
			this.addCondition(
				ConstraintFactory.Loader.load(
				mediator.getClassLoader(), jsonCondition));
        }
	...
	/**
	 * Adds the {@link Constraint} passed as 
	 * parameter to this filter
	 * 
	 * @param jsonCondition
	...
	/**
	 * Adds the {@link SnaMessage.Type} passed as 
	 * parameter to this filter
	 * 
	 * @param type the {@link SnaMessage.Type} to add
	 */
	...
	/**
	 * Adds the {@link SnaMessage.Type}s from the array passed 
	 * as parameter to this filter
	 * 
	 * @param types the array of {@link SnaMessage.Type}s 
	 * to add
	 */
	...
	/**
	 * Returns true if the {@link SnaMessage} matches this
	 * filter, meaning : 
	 * 
	 * <ul>
	 *    <li> its type is handled by this SnaFilter;</li>
	 *    <li> its uri property (the sender) is equals to (or 
	 *    	   matches) this SnaFilter's one;</li>
	 *    <li> and it mets the set of {@link Constraint}s held 
	 *        by this SnaFilter</li> 
	 * </ul>
	 * If the complement field of this SnaFilter is set to true
	 * when the handled type is found, the logical complement is 
	 * returned for the sender and conditions evaluations
	 * 
	 * @param message
	 * 		the {@link SnaMessage} to check whether it
	 * 		matches this SnaFilter's or not
	 * @return
	 * 		<ul>
	 * 			<li>true if the {@link SnaMessage} matches 
	 * 				this SnaFilter</li>
	...
	/**
	 * Returns true if the {@link SnaMessage}'s uri field
	 * value matches this SnaFilter's target
	 * 
	 * @param SnaMessage
	 * 		the {@link SnaMessage} to check whether its
	 * 		specified uri value matches this SnaFilter's target
	 * @return
	 * 		<ul>
	 * 			<li>true if the {@link SnaMessage}'s uri field
	 * 				value matches this SnaFilter's target</li>
	 * 			<li>false otherwise</li>
	 * 		</ul>
	 */
	...
	/**
	 * Returns true if the {@link SnaMessage}'s "value"
	 * mets the set of {@link Constraint}s held by this 
	 * SnaFilter 
	 * 
	 * @param SnaMessage
	 * 		the {@link SnaMessage} to check whether its
	 * 		"value" complies this SnaFilter's condition(s)
	 * @return
	 * 		<ul>
	 * 			<li>true if the {@link SnaMessage}'s "value"
	 * 			complies this SnaFilter's condition(s)</li>
	 * 			<li>false otherwise</li>
	 * 		</ul>
	 */
	boolean matchesType(SnaMessage message)
	{
		return handledTypes.contains(((SnaMessageSubType)
				message.getType()).getSnaMessageType());
	}	

	/**
	 * Returns true if the {@link SnaMessage}'s type is 
	 * handled by this SnaFilter
	 * 
	 * @param SnaMessage
	 * 		the {@link SnaMessage} to check whether its
	 * 		type is handled by this SnaFilter
	 * @return
	 * 		<ul>
	 * 			<li>true if the {@link SnaMessage}'s type
	 * 				is handled by this SnaFilter</li>
	 * 			<li>false otherwise</li>
	 * 		</ul>
	 */
	boolean matchesConditions(SnaMessage message)
	{
		...
		switch(((SnaMessageSubType)
			message.getType()).getSnaMessageType())
		{
			case ERROR:
				break;
			case LIFECYCLE:
				switch((SnaLifecycleMessage.Lifecycle)
						message.getType())
				{
					case RESOURCE_APPEARING:						
						JSONObject initial = new JSONObject(
							message.getJSON()).optJSONObject(
									"initial");
			...
			case RESPONSE:					
				JSONObject response = new JSONObject(
					message.getJSON()).optJSONObject("response");
				if(!JSONObject.NULL.equals(response))
				{
					toValidate = response.opt("value");
				}						
				break;
			case UPDATE:switch((SnaUpdateMessage.Update)
					message.getType())
			{
				case ACTUATED:
					return false;
				...
				case METADATA_VALUE_UPDATED:										
					if(this.handleChangedStatus(((TypedProperties<?>)
						message).<Boolean>get("hasChanged")))
					{
	 ...
	 *@return
     *<ul>
     *	 <li>true if this filter accepts unchanged value
     * 		 notifications</li>
     *	 <li>false otherwise</li>
     *</ul>
     */
