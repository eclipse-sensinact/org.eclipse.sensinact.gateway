...
public class ServiceProxy extends ModelElementProxy
{
	 ...
	 */
	ServiceProxy(Mediator mediator, String path) 
			throws InvalidServiceException
	{
	 ...
	 */
	final void buildMethods(ErrorHandler handler,
	...
	{		
		...
		for(;index < length; index++)
		{
			...
			{
				case "ACT":
					actMethod = new ServiceAccessMethod(mediator, getPath(), 
						AccessMethod.Type.valueOf(AccessMethod.ACT), handler);
					break;
				case "DESCRIBE":
					break;
				case "GET":
					getMethod = new ServiceAccessMethod(mediator, getPath(), 
						AccessMethod.Type.valueOf(AccessMethod.GET), handler);
					break;
				case "SET":
					setMethod = new ServiceAccessMethod(mediator, getPath(), 
						AccessMethod.Type.valueOf(AccessMethod.SET), handler);
					break;
				case "SUBSCRIBE":
		...
		} else
		{
			AccessMethod snaMethod = new UnaccessibleAccessMethod(
				mediator, getPath(), AccessMethod.Type.valueOf(
					AccessMethod.GET));
