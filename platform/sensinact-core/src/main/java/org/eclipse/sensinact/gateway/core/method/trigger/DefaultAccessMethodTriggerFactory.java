...
{	
	...
	public <P> AccessMethodTrigger<P> newInstance(Mediator mediator, 
			JSONObject jsonTrigger) throws InvalidValueException
	{		
		...
		{			
			...
			{
				case CONDITIONAL:
					List<ConstraintConstantPair> constraints = 
					new ArrayList<ConstraintConstantPair>();
					index = jsonTrigger.optInt(AccessMethodTrigger.TRIGGER_INDEX_KEY);
					JSONArray constants = jsonTrigger.optJSONArray(
							Constant.TRIGGER_CONSTANTS_KEY);				
					int constantsIndex = 0;
					int length = constants==null?0:constants.length();
					
					for(;constantsIndex < length; constantsIndex++)
					{
						JSONObject constantObject = constants.getJSONObject(
								constantsIndex);
					...
					trigger = (AccessMethodTrigger<P>) new ConditionalConstant(mediator, 
							index, constraints, passOn);		
					break;
				case CONSTANT:
				...
				default:
					throw new InvalidValueException(new StringBuilder().append(
							"Unknown calculation identifier :").append(
									jsonType).toString());
			}			
		} catch(Exception e)
		{
