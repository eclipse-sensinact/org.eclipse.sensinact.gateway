...
/**
 * A FilteringCollection wraps a set of {@link 
 * FilteringAccessor}s, allowing on one hand to build 
 * the global LDAP formated String filter from the ones 
 * provided by each {@link FilteringAccessor}, on the 
 * other hand it can be used to apply all the filters, 
 * using the output of one as input of the next.
 * 
...
public class FilteringCollection 
{
	 ...
	 * the FilteringCollection to be instantiated to
	 * interact with the OSGi host environment
	 * @param hideFilter the boolean defining whether
	 * the JSON formated String describing the {@link 
	 * FilteringAccessor}s wrapped by the FilteringCollection 
	 * to be instantiated will defined as hidden
	 * @param filterDefinition the set of {@link 
	 * FilteringDefinition}s from which the 
	 * FilteringCollection to be instantiated will create
	 * the set of {@link FilteringAccessor}s
	 */
	public FilteringCollection(Mediator mediator,
		boolean hideFilter, FilteringDefinition... filterDefinition) 
	{
    	...
    	try
    	{
	    	for(;index < length;index++)
	    	{
    			...
    			jsonFormatedFilterBuilder.append(String.format(
    			"{\"type\":\"%s\",\"definition\":\"%s\"}",
	...
	/**
	 * Builds and returns the global LDAP formated String
	 * filter from the ones provided by each registered {@link 
	 * FilteringAccessor}
	 * 
	 * @param ldapFilter the initial LDAP formated String filter
	 * from which to create the global one
	 * 
	...
	/**
	 * Applies each {@link FilteringAccessor} in order, by 
	 * using the output of one as input of the next, the first
	 * input being the specified initial String value.
	 * 
	...
	/**
	 * Returns true if the the JSON formated String describing 
	 * the {@link FilteringAccessor}s wrapped by this FilteringCollection 
	 * is hidden; returns false otherwise
	 * 
	 * @return
	 * 	<ul>
	 * 		<li>true if the the JSON formated String describing 
	 * 		    the wrapped {@link FilteringAccessor}s is hidden</li>
	...
	/**
	 * Returns the global JSON formated String description
	 * of the filters provided by each registered {@link 
	 * FilteringAccessor}
	 * 
