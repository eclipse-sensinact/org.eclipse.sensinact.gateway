...
public class ServiceBuilder {
     ...
     *
     * @param name the name of the service to build
     */
     ...
     *
     * @param implementationClass the extended {@link ServiceImpl} type of the
     *                            service to build
     */
     ...
     * as parameter
     *
     * @param provider the service provider to which the {@link
     *                 ServiceImpl} to create is connected to
     * @return a new {@link ServiceImpl}
     * @throws InvalidServiceException
     */
    public final ServiceImpl build(ModelInstance<?> snaModelInstance, ServiceProviderImpl provider) throws InvalidServiceException {
        if (name == null) {
            throw new InvalidServiceException("Service's name is needed");
        }
        ServiceImpl serviceImpl = null;
        try {
            serviceImpl = ReflectUtils.getInstance(baseClass, implementationClass == null ? ServiceImpl.class : implementationClass, new Object[]{snaModelInstance, name, provider});
        } catch (Exception e) {
