...
public final class SignatureUtil
{
    ...
    public static boolean checkSign(String message, String sign, String keyPath )
    {
        ...
        try
        {
            ret = SignatureUtil.verify( message, sign, 
            		SignatureUtil.getKey( keyPath ) );
        }
        catch (Exception e )
        {
    		...
    		InvalidKeySpecException
    {
        final KeyFactory keyFactory = KeyFactory.getInstance( 
        		SignatureUtil.ENCRYPTION_ALGORITHM );
    		...
    		InvalidKeyException, SignatureException, IOException
    {
        final Signature sig = Signature.getInstance( 
        		SignatureUtil.HASH_ENCRYPTION_ALGORITHM );
        
        sig.initVerify( publicKey );
        sig.update( message.getBytes( SignatureUtil.CHARSET ) );

        byte[] bytes = Base64.decode(
        		URLDecoder.decode( sign, SignatureUtil.CHARSET ) );
