...
public class RestLikeMapper
{
	 ...
	 * @param hierarchies
	 * 		map of dependencies between initial configuration map entries
	 */
	public RestLikeMapper(BundleContext context, String[][] filters,
			String[][] hierarchies, String[] methods) throws RestLikeMapperException
	{
		...
		for(;index <filters.length;index++)
		{
			...
			this.filters[pos++][1] = mapping[2]==null?null:
				new StringBuilder().append(OPEN_BRACKET).append(
					OBJECTCLASS).append(EQUALITY).append(mapping[2]).append(
							CLOSE_BRACKET).toString();
			
			this.filters[pos][0] = mapping[0];
			
			this.filters[pos++][1] = mapping[1]==null?null:
				new StringBuilder().append(OPEN_BRACKET).append(
					mapping[1]).append(EQUALITY).append(VARIABLE).append(
							CLOSE_BRACKET).toString();
		}
		...
		for(String[] hierarchy : hierarchies)
		{
			if(hierarchy[0]!=null && hierarchy[1]!=null)
			{
				if(hierarchy[2] != null)
				{
					this.hierarchies[pos][0] = new StringBuilder().append(hierarchy[0]).append(
						LESS).append(hierarchy[1]).toString();
					
					this.hierarchies[pos++][1] = new StringBuilder().append(OPEN_BRACKET).append(
							hierarchy[2].length()==0?plural(hierarchy[1]):hierarchy[2]).append(
								EQUALITY).append(VARIABLE).append(CLOSE_BRACKET).toString();
				}
				if(hierarchy[3] != null)
				{
					this.hierarchies[pos][0] = new StringBuilder().append(hierarchy[1]).append(
						LESS).append(hierarchy[0]).toString();
					
					this.hierarchies[pos++][1] = new StringBuilder().append(OPEN_BRACKET).append(
							hierarchy[3].length()==0?plural(hierarchy[0]):hierarchy[3]).append(
								EQUALITY).append(VARIABLE).append(CLOSE_BRACKET).toString();
				}
			}
		}
	 ...
	 * 
	 * @param singular
	 * 		the word to convert to the plural form 
	 * @return
	 * 		the plural form of the singular word passed
	 * 		as parameter
	 */
	 ...
	 *  
	 * @param filterFst
	 * 		one of the two filters to concatenate 
	 * @param filterSnd
	 * 		one of the two filters to concatenate 
	 * @return
	 * 		the filter resulting of the concatenation of the
	...
	private String concat(String filterFst, String filterSnd)
	{
		...
		return new StringBuilder().append(OPEN_BRACKET).append(AND).append(
				(filterFst.charAt(0)==OPEN_BRACKET)?"":OPEN_BRACKET).append(filterFst).append(
				(filterFst.charAt(filterFst.length()-1)==CLOSE_BRACKET)?"":CLOSE_BRACKET).append(
				(filterSnd.charAt(0)==OPEN_BRACKET)?"":OPEN_BRACKET).append(filterSnd).append(
				(filterSnd.charAt(filterSnd.length()-1)==CLOSE_BRACKET)?"":CLOSE_BRACKET).append(
						CLOSE_BRACKET).toString();
	}
	 ...
	 * @return
	 * 		 the set of OSGi services mapped by the
	 * 		 URI argument
	 */
	public RestLikeMappingReport parseURI(String uri)
	{
		...
		for(int i = 0;i <uriElements.length;i+=2)
		{
			...
			String filter = concat(this.filters[j][1], this.filters[j+1][1] !=null 
				&& singular!= null?	this.filters[j+1][1].replace(VARIABLE,singular):null);		
			...
			if(filter != null)
			{
				...
				report.reportImplementation(this.filters[j][1]!=null?
					this.filters[j][1].substring(new StringBuilder().append(OPEN_BRACKET).append(
					OBJECTCLASS).append(EQUALITY).length(),this.filters[j][1].length()-1):null);
			}
			//else
			int k=0;
			...
			{
				String descendant = new StringBuilder(this.filters[last+1][0]).append(LESS).append(
						this.filters[j+1][0]).toString();
				
				String ascendant = new StringBuilder(this.filters[j+1][0]).append(LESS).append(
						this.filters[last+1][0]).toString();
				...
				{					
					...
					tmpFilter = concat(fstack.pollLast(),
							this.hierarchies[k][1].replace(VARIABLE, singular));
		...
		while(!fstack.isEmpty())
		{
			...
			{				
				...
				//filter
				ServiceReference[] current = context.getServiceReferences(
						(String)null, filter );
				
				if(current!=null)
				{
					List<ServiceReference> srs = new LinkedList<ServiceReference>(
					...
					{	
						...
						} else
						{
							...
							report.reportReferences(filter,intersection.size()==0
									?srs:intersection);
						}
					}
				} else
