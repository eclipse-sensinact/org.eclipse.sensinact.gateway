...
public class UriUtils
{
	 ...
	 * 
	 * @param uri
	 * 		the string uri to decompose
	 * @return
	 * 		the elements array of the specified 
	 * 		uri 
	 */
	public static String[] getUriElements(String uri)
	{
		...
		System.arraycopy(elements, 1, uriElements, 0, 
				elements.length-1);
		return uriElements;
	 ...
	 *   
	 * @param uri
	 * 		the uri to return the uri parent of
	 * @return
	 * 		the specified uri's parent uri
	 */
	public static String getRoot(String uri)
	{
		...
		for(;index < length; index++)
		{
			if(uriElements[index]==null || 
					uriElements[index].length()==0)
			{
				continue;
			}
	 ...
	 *  
	 * @param uri
	 * 		the uri from which to extract the leaf
	 * 		element
	 * @return
	 * 		the last part of the specified uri 
	 */
	...
	public static String formatUri(String uri)
	{
		...
		{
			formatedUri = new StringBuilder().append(
				PATH_SEPARATOR).append(
				formatedUri).toString();
		}
		if(formatedUri.length()>1 && 
				formatedUri.endsWith(PATH_SEPARATOR))
		{
			formatedUri = formatedUri.substring(0,
					formatedUri.length()-1);
		}
		if(formatedUri.length()==1)
		{
			return ROOT;
