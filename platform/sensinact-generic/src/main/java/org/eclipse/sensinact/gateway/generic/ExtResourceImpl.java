...
public class ExtResourceImpl extends ResourceImpl
{
	 ...
	 * @param service
	 */
	protected ExtResourceImpl(ExtModelInstance<?> modelInstance,
	...
	{		
		...
		{
			super.getAttribute(defaultAttribute).setValue(
		    		task.getResult(), task.getTimestamp());
		}
		if(task == null)
		{
		    return null;
     ...
     * 
     * @param type
     * 		{@link AccessMethod.Type} for which to retrieve the 
     * 		extended {@link Annotation} type
     * @return
     * 		the extended {@link Annotation} type mapped to the specified
	 ...
	 * 
	 * @param resourceConfig
	 * 		the {@link MethodDefinition} describing the {@link ActMethod}
	 * 		to build
	 * @param snaService 
	 */
	protected void buildMethod(ExtResourceConfig resourceConfig, 
			ExtServiceImpl service)
    {		
    	...
    	while(iterator.hasNext())
    	{
	    	...
	    	AbstractAccessMethod method = (AbstractAccessMethod) 
	    		super.getAccessMethod(methodDefinition.getType());
	    	
	    	SignatureDefinition definition = null;
	    	
	    	Iterator<SignatureDefinition> signatureIterator =
	    	...
	    	{		
                ...
                try
                {
	                signature = definition.getSignature(
	                	super.modelInstance.mediator(), service);
                }
	    		...
	    		if(annotationClass == null)
	    		{
	     		...
	     		Map<Method,? extends Annotation> methods = 
     					ReflectUtils.getAnnotatedMethods(
     	     					clazz, annotationClass);
	     		
     			Set<Method> methodSet = methods.keySet(); 
     			
     			Method javaMethod = ReflectUtils.getDeclaredMethod(
     				methodSet.toArray(new Method[0]), JSONObject.class, 
     				null, parameterTypes, true);
	    		...
	    		service.buildTriggers(this.getName(),
	    			signature, definition.getReferenceDefinitions());
	    	}
    	}
    }
}
