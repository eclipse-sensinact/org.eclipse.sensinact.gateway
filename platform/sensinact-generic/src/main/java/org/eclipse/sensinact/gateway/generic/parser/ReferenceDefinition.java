...
@XmlAttributes({
    ...
    @XmlAttribute(attribute = "passOn", field = "passOn"),
    @XmlAttribute(attribute = "index", field = "index"),
    @XmlAttribute(attribute = "calculated", field = "subType")})
public class ReferenceDefinition extends NameTypeValueDefinition 
implements JSONable, ConstrainableDefinition
{	
     ...
     * 
     * @param subType
     * 		the described CALCULATED {@link AccessMethodTrigger}s'
     * 		sub type
     */
     ...
     * 
     * @param reference
     * 		the name of the targeted {@link StateVariableResource}
     */
     ...
     * 
     * @param index
     * 		the index of the parameter on which applies 
     * 		the calculation
     */
    ...
    {
        return this.conditionalConstants.get(this.conditionalConstants.size()-1
        		).getTypeDefinition();
    }    
    ...
    {
    	 this.conditionalConstants.get(this.conditionalConstants.size()-1
    ...
    public String getJSON()
    {
	    ...
	    if(!this.conditionalConstants.isEmpty())
	    {
	    	...
	    	if(this.conditionalConstants.size() == 1 && 
	    			this.conditionalConstants.get(0
	    					).isUnconditional())
	    	{	    		
			    builder.append(JSONUtils.QUOTE);
