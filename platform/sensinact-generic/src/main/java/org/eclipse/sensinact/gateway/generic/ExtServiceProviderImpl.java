...
{    
     ...
     * Constructor
     * @param context
     *            the {@link BundleContext} where to register the service in
     *
     * @throws InvalidServiceProviderException 
     * @throws InvalidValueException 
     */
    protected ExtServiceProviderImpl(ExtModelInstance<?> modelInstance, 
    		String name) throws InvalidServiceProviderException
    {
     ...
     */
    protected ExtServiceProviderImpl(ExtModelInstance<?> modelInstance, 
    		String name, List<String> serviceNames) 
    		throws InvalidServiceProviderException
    {
    		...
    		Object[] parameters) throws Exception
	{
		...
		resourceConfig =  ((ExtModelInstance<?>)super.modelInstance
			).configuration().getResourceConfig(new ExtResourceDescriptor(
				).withServiceName(service).withResourceName(resource));		
			
		Task task = ((ExtModelInstance<?>)super.modelInstance).propagate(
				command, path, resourceConfig, parameters);
		...
		if (task != null)
		{
			...
			while (!task.isResultAvailable() && wait > 0)
			{
				...
				{
					Thread.interrupted();
					ExtServiceProviderImpl.super.modelInstance.mediator(
							).error(e);
					break;
				}
			}
	...
	{
        Task task = ((ExtModelInstance<?>)super.modelInstance).propagate(
        	Task.CommandType.SERVICES_ENUMERATION, this.getPath(), null, null);
		...
		new Executable<Task,Void>()
		{
                    ...
                    throws Exception
            {
				if(task.isResultAvailable())
		        {
	                ...
	                if(serviceIds!=null && 	(componentType = serviceIds.getClass(
	                	).getComponentType())!=null && componentType == String.class)
	                {
		        ...
		        } else
		        {
	                ...
	                throw new InvalidServiceProviderException(
	                		"services enumeration task timeout");
		        }
