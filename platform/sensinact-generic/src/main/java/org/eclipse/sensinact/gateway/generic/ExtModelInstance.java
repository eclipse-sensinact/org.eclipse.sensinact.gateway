...
public class ExtModelInstance<C extends ExtModelConfiguration>
extends ModelInstance<C>
{
	 ...
	 * @param path 
	 * 		the string path of the target ModelElement
     * @param resourceConfig
     *     the {@link ResourceConfig} associated to the 
     *     {@link Resource} for which the method is called
     * @param parameters
     *     array of objects used to parameterize the call
     * @return 
     * 	   The created and executed {@link Task}
     */
    public Task propagate(
			Task.CommandType command, String path,
			ResourceConfig resourceConfig, Object[] parameters)
    {
    	return this.taskManager.execute(command, path, 
    	  super.profileId, resourceConfig, parameters);
    }    
