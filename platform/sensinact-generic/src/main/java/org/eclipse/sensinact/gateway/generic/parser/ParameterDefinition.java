 ...
 */
public class ParameterDefinition extends NameTypeValueDefinition implements
...
{	
     ...
     * 
     * @return
     * 		the {@link TypeDefinition} of this
     * 		ParameterDefinition
     */
	 ...
	 * 
	 * @return
	 * 		true if a {@link Parameter} based on this 
     ...
     * 
     * @return
     *      the {@link Parameter} described by this 
     *      ParameterDefinition
    ...
    public Parameter getParameter(ServiceImpl service) throws InvalidValueException
    {
        ...
        try
        {
	        ...
	        { 
	            ...
	            {
	                super.mediator.error("no defined type : "
	        ...
	        if(builder !=null)
	        {
	        	...
	        	DynamicParameterValueFactory.Loader loader = 
	        			DynamicParameterValueFactory.LOADER.get();
	    		try
	            {
	    			DynamicParameterValueFactory factory = loader.load(
	    					mediator, builder.getName());  
	    			JSONObject builderDefinitionJSON = new JSONObject(builder.getJSON());
	    			JSONObject builderJSON = builderDefinitionJSON.getJSONObject(
	    		    			DynamicParameterValue.BUILDER_KEY);    			
	    		    dynamic = factory.newInstance(mediator,
	    		    	service.getResourceValueExtractor(builder.getReference()), 
	    		    		builderJSON);
	            }finally
	            {
	            		DynamicParameterValueFactory.LOADER.remove();            	
	            }
	        	parameter = new DynamicParameter(mediator, name,
	        			typeDefinition.getType(), dynamic);  
	    	...
	    	while(iterator.hasNext())
	    	{
	    		try
	            {
	                Constraint constraint = iterator.next().getConstraint(
	    	...
	    	if(fixedValue == null)
	    	{
	    		parameter = new Parameter(super.mediator, super.getName(), 
	    	        typeDefinition.getType(), (Set<Constraint>)constraints);
	    	} else
	    	{
	    		parameter = new Parameter(super.mediator, super.getName(), 
	    				typeDefinition.getType(), fixedValue);
	    	}
        } catch(Exception e)
        {
        	e.printStackTrace();
    ...
    {
    	if(constraint != null && Fixed.class.isAssignableFrom(
    			constraint.getClass()))
        {
        	fixed = true;
        } 
     ...
     * 
     * @param builder
     * 		the BuilderDefinition describing the {@link 
