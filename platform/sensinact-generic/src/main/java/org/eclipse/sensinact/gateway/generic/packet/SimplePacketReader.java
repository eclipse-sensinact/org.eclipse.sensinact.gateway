...
extends AbstractPacketReader<P>
{
     ...
     * 
     * @param resourceId
     * 		the targeted {@link Resource}'s identifier
     */
     ...
     * 
     * @param attributeId
     * 		the {@link Attribute}'s string identifier of 
     * 		the {@link PayloadResourceFragment} to create
     */
     ...
     * 
     * @param attributeId
     * 		the {@link Metadata}'s string identifier of 
     * 		the {@link PayloadResourceFragment} to create
     */
	 ...
	 * 
	 * @param		
	 * 		the parsed {@link Packet}'s timestamp
	 */
    ...
    protected void configure()
    {
		...
		int index = -1;
		if((index = super.subPackets.indexOf(new Name<PayloadFragment>(
    	...
    	if(name.length()>0)
		{
    		if((index = subPacket.payloadFragments.indexOf(
    				new Name<PayloadServiceFragment>(name)))!=-1)
    		{
    			payloadFragment = (PayloadServiceFragmentImpl) 
    					subPacket.payloadFragments.get(index);    			
    		} else
    		{
    	...
    	{    		
	    	...
	    	{
	    		PayloadResourceFragmentImpl payloadAttributeFragment = 
	    			newPayloadAttributeFragment(this.attributeId, 
	    					this.metadataId, this.data); 
	    		
	    		payloadAttributeFragment.setTimestamp(this.timestamp);	    		
	    		payloadFragment.addPayloadAttributeFragment(
	    				payloadAttributeFragment);
	    	}
    ...
    {
    	int index = 0;
    	int length = super.subPackets==null
    			?0:super.subPackets.size();
    	
    	for(;index < length && !super.subPackets.get(
    				index).treated(taskIdentifier); index++);
     ...
     * 
     * @return
     * 		 a new {@link PayloadFragment} instance
     */
     ...
     * 
     * @return
     * 		 a new {@link PayloadServiceFragment} instance
     */
     ...
     * 
	 * @param attributeId
	 * 		the attribute identifier of the {@link 
	 * 		PayloadResourceFragment} to create
	 * @param metadataId
	 * 		the metadata identifier of the {@link 
	 * 		PayloadResourceFragment} to create
	 * @param data
	 * 		the data object of the {@link 
	 * 		PayloadResourceFragment} to create
	 * 
     * @return
     * 		 a new {@link PayloadServiceFragment} instance
	 */
    private PayloadResourceFragmentImpl newPayloadAttributeFragment(
            String attributeId, String metadataId, Object data)
    {
