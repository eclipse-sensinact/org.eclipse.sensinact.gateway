...
public class PayloadServiceFragmentImpl implements PayloadServiceFragment
{
    ...
    public PayloadServiceFragmentImpl(Mediator mediator)
    {
        ...
        this.payloadAttributeFragments = 
        		new ArrayList<PayloadResourceFragment>();
    }
     ...
     * 
     * @param command 
     * 		the {@link Task.CommandType} of the Task this {@link PayloadServiceFragment}
     * 		responds to
     */
     ...
     * 
     * @param payloadAttributeFragment
     * 		the {@link PayloadResourceFragment} to add
     */
    public void addPayloadAttributeFragment(
    		PayloadResourceFragment payloadAttributeFragment)
    {
    	if(payloadAttributeFragment != null)
    	{
    		this.payloadAttributeFragments.add( 
    ...
    public Iterator<PayloadResourceFragment> iterator()
    {
    	return Collections.unmodifiableList(
    		this.payloadAttributeFragments).iterator();
    }
    ...
    public List<TaskIdValuePair> AsTaskIdValuePair(String serviceProviderIdentifier)
    {
    	...
    	List<TaskIdValuePair> taskIdValuePairs = 
    			new ArrayList<TaskIdValuePair>();
    	
    	int index = 0;
    	int length = this.payloadAttributeFragments==null
    			?0:this.payloadAttributeFragments.size();
    	
    	for(;index < length; index++)
    	{
    		TaskIdValuePair taskIdValuePair = this.payloadAttributeFragments.get(
    			index).AsTaskIdValuePair(this.taskIdentifier);
    ...
    public boolean treated(String taskIdentifier)
    {
    	...
    	for(;index < length; index++)
    	{
    		if(taskIdentifier.equals(this.payloadAttributeFragments.get(
    			index).getTaskIdentifier()))
    		{
