...
public class ExtResourceConfigCatalog implements ResourceConfigCatalog
{
			...
			Map<String,String> defaults)
	{
        ...
        Map<String, ExtResourceConfig> tmpDefaults = 
    			new HashMap<String,ExtResourceConfig>();
        
        if(handler != null)
        {
	        Iterator<ExtResourceConfig> resourceIterator = 
	        		handler.getXmlResourceConfigs();
	        ...
	        while(resourceIterator.hasNext())
	        {
	    		...
	    		while(iterator.hasNext())
	    		{
	    			...
	    			if((serviceId = entry.getKey()) == null || 
	    				(config = entry.getValue()) == null ||
	    				!config.equals(name))
	    			{
	    				continue;
	    			}
	 ...
	 */
	@Override
	public ExtResourceConfig getResourceConfig(
			ResourceDescriptor resourceConfigDescriptor) 
	{ 	
		...
		if(ExtResourceDescriptor.class.isAssignableFrom(
				resourceConfigDescriptor.getClass()))
		{
			identifier =((ExtResourceDescriptor)
					resourceConfigDescriptor).identifier();
		}
		int index = -1;
		...
		if(serviceName != null && resourceName != null)
		{
	        ...
	        while(iterator.hasNext())
	        {
	        	...
	        	if(resourceName.equalsIgnoreCase(
	        			resourceConfig.getName(serviceName)))
	        	{
	        		break;
	        	}
	        	resourceConfig = null;
	        }
		} else if(resourceName != null && (index = this.resourceConfigs.indexOf(
				new Name<ResourceConfig>(resourceName))) > -1)
		{
			resourceConfig = this.resourceConfigs.get(index);
			
		} else if(identifier != null)
		{
			Iterator<ExtResourceConfig> iterator = 
	        		this.resourceConfigs.iterator();
	        
	        while(iterator.hasNext())
	        {
	        	resourceConfig = iterator.next();
	        	if(ExtModelConfiguration.compareBytesArrays(identifier,
	        			resourceConfig.getIdentifier()))
	        	{
	        		break;
	        	}
	        	resourceConfig = null;
    ...
    public List<ExtResourceConfig> getResourceConfigs(String profile, String service)
    {
        ...
        while(enumeration.hasNext())
        {
            ...
            if(resourceConfig.isProfiled(profileId) 
            		&& resourceConfig.isTargeted(serviceId))
            {
            	if(ActionResource.class.isAssignableFrom(
            		resourceConfig.getTypeConfig().getResourceImplementedInterface()))
            	{
