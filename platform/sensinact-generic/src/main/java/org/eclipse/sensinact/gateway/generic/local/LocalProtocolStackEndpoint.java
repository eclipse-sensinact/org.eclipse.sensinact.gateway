...
{    
    public static final Task.RequestType REQUEST_TYPE = 
    		Task.RequestType.LOCAL;
     ...
     * @param injectable
     * 		the Object instance to make accessible to {@link 
     * 		TaskExecution} annotated types
     */
    public <T> void addInjectableInstance(Class<T> injectableType, 
    		T injectable)
    {
    	this.resolver.addInjectableInstance(injectableType, 
    		injectable);
    }
    ...
    {
    	this.addInjectableInstance(
    			(Class<T>)injectable.getClass(), injectable);
    }
	 ...
	 *  
	 * @param classes
	 * 		List of {@link TaskExecution} annotated types
	...
	private void buildExecutors()
	{
		...
		{ 
			...
			Map<Method, TaskCommand> instanceMap =  
					ReflectUtils.getAnnotatedMethods(
						instance.getClass(), TaskCommand.class);
			
			if(instanceMap.isEmpty())
			{
				continue;
			}
			Iterator<Map.Entry<Method, TaskCommand>> 
			    instanceIterator  = instanceMap.entrySet(
			    		).iterator();	
			
			while(instanceIterator.hasNext())
			{
				Map.Entry<Method, TaskCommand> methodEntry = 
				...
				TaskExecution taskInvoker = instance.getClass(
						).getAnnotation(TaskExecution.class);
				...
				{
					AnnotationExecutor executor = new AnnotationExecutor(
							target, command,sync, method.isVarArgs(),
							method.getParameterTypes(), profiles[index])
					{
						/**
						 * @inheritedDoc
						 *
						...
						{			
							...
							int length= taskParameters==null || (method.isVarArgs() 
								&& Array.getLength(taskParameters) == 0)
								?0:taskParameters.length;
	
							Object[] parameters = new Object[length+1];
							parameters[0] = task.getPath();
							if(length > 0)
							{
								System.arraycopy(taskParameters, 0, 
									parameters, 1, length);
							}		
							Object result = method.invoke(instance, method.isVarArgs()
	 ...
	 * 
	 * @param task
	 * 		the {@link TaskImpl} to execute
	 * @return
	 * 		the object result of the {@link AnnotationExecutor}
	 * 		invocation
	 * @throws Exception
	 * 		if a {@link AnnotationExecutor} cannot be retrieved 
	...
	{	
		...
		if(executor == null)
		{
			...
			Iterator<AnnotationExecutor> iterator = 
					this.executors.listIterator();
			
			while(iterator.hasNext())
			{			
				executor = iterator.next();
				
				if(!executor.equals(commandType)
						|| !executor.equals(parameterTypes)
						|| !executor.isProfile(profile))
				{
					executor = null;
					continue;
				}
		    	index = 0;			
		    	String[] thisPathElements = 
		    			UriUtils.getUriElements(path);        	
		    	String[] thatPathElements = 
		    			UriUtils.getUriElements(executor.getName());
		    	...
		    	for(;index < thatLength && 
		    		(thatPathElements[index].equals(UriUtils.WILDCARD)
		    		|| thatPathElements[index].equals(thisPathElements[index])); 
		    			index++);		    	
		    	if(index < thatLength)
		    	{
					executor = null;
			...
			if(executor == null)
			{
				executor = new EmptyAnnotationExecutor(
					path, commandType, parameterTypes, profile);
			}
	...
	public Task createTask(Mediator mediator, CommandType command, String path,
	        ...
	        Object[] parameters)
	{
		return new GenericLocalTask(mediator, command, this, path, 
	...
	private class PathCommandKey
	{
		...
		PathCommandKey(String path, String profileId, CommandType command)
		{
			...
			this.hash = new StringBuilder().append(path).append("_"
				).append(profileId).append("_").append(command
						).toString().hashCode();
		}		
	...
	private class EmptyAnnotationExecutor extends AnnotationExecutor
	{
		 ...
		 * @param profile
		 */
        EmptyAnnotationExecutor(String target, CommandType commandType,
                Class<?>[] parameterTypes, String profile)
        {
	        super(target, commandType, 
	        		TaskCommand.SynchronizationPolicy.SYNCHRONOUS,
	        		false, parameterTypes, profile);
        }
