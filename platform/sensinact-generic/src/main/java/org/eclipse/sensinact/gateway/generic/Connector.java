...
{	       
    ...
    {
    	super(mediator, endpoint, 
    		ExtModelConfiguration.isLockedAtInitializationTime(),
    		ExtModelConfiguration.isDesynchronized());
    ...
    {		       
    	try
    	{
	        ...
	        {	        
	        	this.customizer = new DefaultConnectorCustomizer<P>(
	        		mediator, this.ExtModelConfiguration);
	        }
    	}catch(Exception e)
    	{
    		mediator.error(e);
    ...
    {       
		if(!this.customizer.preProcessing(packet))
		{
	        if (super.mediator.isDebugLoggable())
			{
				 super.mediator.debug(
						 "Do not process the received packet : exiting");
			} 
		    return;
		}   
        ...
        if(reader == null)
        {
        	throw new InvalidPacketException(
        			"Unable to create an appropriate reader");
        }
        ...
        while(iterator.hasNext())
        {
        	...
        	while(taskIterator.hasNext())
        	{
    	        ...
    	        {      
    	           task.setResult(taskIdValuePair.getValue(), 
    	        		   taskIdValuePair.getTimestamp());
    	           treated = true;
    	        }
        	}
        	if(treated)
        ...
        while(subPacketIterator.hasNext())
        {
		    ...
		    if(serviceProviderName == null)
		    {
		    	...
		    	{
		    		this.mediator.debug(
		    			"Unable to identify the targeted service provider");
		    	}
		    	continue;
		    }
	    	int index = -1;
     ...
     * 		the {@link ServiceProvider} joining the network
     */
    protected void processHello(ExtServiceProviderImpl serviceProvider) 
    {    	
    	if (ServiceProvider.LifecycleStatus.INACTIVE.equals(
     ...
     * 
     * @param serviceProviderName
     *       a String key identifier of the 
     *       {@link ExtServiceProviderImpl} to instantiate
	 * @param services 
     * 		the array of 
     * @return 
     * 		a new {@link ExtServiceProviderImpl} instance
	 * @throws InvalidServiceProviderException 
     */
				...
				throws InvalidServiceProviderException 
	{
		...
		ExtModelInstance<?> instance = new ExtModelInstanceBuilder(
			this.mediator, this.ExtModelConfiguration.getPacketType()
			).withConnector(this
	 ...
	 * 
	 * @param instanceName
	 * 		the name of the {@link ExtModelInstance}
	 * 		to return
	 * @return
	 * 		the {@link ExtModelInstance} with the specified 
	 * 		name
	 */
