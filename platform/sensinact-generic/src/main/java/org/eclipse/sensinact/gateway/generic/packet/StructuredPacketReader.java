...
public class StructuredPacketReader<P extends Packet> extends SimplePacketReader<P> 
{
	...
	Arrays.asList(AttributeID.class.getCanonicalName(), CommandID.class.getCanonicalName(),
	Data.class.getCanonicalName(), Iteration.class.getCanonicalName(), MetadataID.class.getCanonicalName(),
	ProfileID.class.getCanonicalName(), ResourceID.class.getCanonicalName(), ServiceID.class.getCanonicalName(),
	ServiceProviderID.class.getCanonicalName(), Timestamp.class.getCanonicalName(), HelloMessage.class.getCanonicalName(),
	 ...
	 */
	public StructuredPacketReader(Mediator mediator, Class<P> packetType) 
			throws InvalidPacketTypeException
	{
		this(mediator);
		...
		AnnotatedElement[] elements =new AnnotatedElement[
			(fieldsLength)+(methodsLength)];
		
		if(fieldsLength > 0)
		{
			System.arraycopy(fields, 0, elements, 0, 
					fieldsLength);
		}
		if(methodsLength > 0)
		{
			System.arraycopy(methods, 0, elements, 
					fieldsLength, methodsLength);
		}
		int index = 0;
		...
		for(;index < length; index++)
		{
			...
			{				
				Class<? extends Annotation> annotationType = 
						annotations[annotationIndex].annotationType();
				...
				annotationName  = new StringBuilder().append(
					annotationName.substring(0,1).toLowerCase()).append(
							annotationName.substring(1)).append("Annotated"
									).toString();
				try
				{
	...
	public void parse(P packet) throws InvalidPacketException 
	{
		...
		if(StructuredPacket.class.isAssignableFrom(
				packet.getClass()))
		{
