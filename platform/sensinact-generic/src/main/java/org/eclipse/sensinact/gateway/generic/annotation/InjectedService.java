...
{	
				...
				
	{
		...
		String objectClassFilter = new StringBuilder().append("(").append(
				Constants.OBJECTCLASS).append("=").append(
						type.getCanonicalName()).append(")").toString();
		
		String propertiesFilter = filter==null?null:new StringBuilder(
				).append(filter.startsWith("(")?"":"(").append(filter).append(
						filter.endsWith(")")?"":")").toString();
		
		this.mediator.getContext().addServiceListener(this,
			new StringBuilder().append(propertiesFilter!=null?"(&":"").append(
				objectClassFilter).append(propertiesFilter!=null?propertiesFilter:""
					).append(propertiesFilter!=null?")":"").toString());
			...
			throws Throwable 
	{
		...
		if(this.reference != null && (service=this.mediator.getContext(
				).getService(this.reference))!= null)
		{
			try
			{
	...
	private void searchReference()
	{
		...
		try 
		{
		   references = this.mediator.getContext(
		...
		{
			if(this.mediator.getContext().getService(
