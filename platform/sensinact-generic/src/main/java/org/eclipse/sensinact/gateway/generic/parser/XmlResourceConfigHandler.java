 ...
 */
public class XmlResourceConfigHandler extends AbstractContentHandler<ExtResourceConfig> {
    ...
    private static final String RESOURCE_INFO_TYPE_PROPERTY = "resourceInfoProperty".intern();
    private static final String RESOURCE_INFO_TYPE_SENSOR = "resourceInfoSensor".intern();
    private static final String RESOURCE_INFO_TYPE_VARIABLE = "resourceInfoVariable".intern();
    private static final String RESOURCE_INFO_TYPE_ACTION = "resourceInfoAction".intern();
    ...
    public void policyStart(Attributes atts) {
        if (super.stack.isEmpty()) {
            ...
            this.policyDefinition = new PolicyDefinition(this.mediator, atts);
        }
    }
    ...
    public ExtResourceConfig policyEnd() {
        ...
        if (!super.stack.isEmpty()) {
            ...
            TypeConfig resourceTypeConfig = new TypeConfig(this.policyDefinition.getPolicy());

            Class<? extends ExtResourceImpl> implementationClass = this.policyDefinition.getPolicyImplementationClass();
            if (implementationClass == null) {
                implementationClass = registered.getPolicyImplementationClass();
            }
            Class<? extends Resource> implementationInterface = this.policyDefinition.getPolicyImplementationInterface();
    ...
    public void identifierStart(Attributes atts) {
        this.identifierDefinition = new IdentifierDefinition(this.mediator, atts);
    }
    ...
    public void commandStart(Attributes atts) {
        this.commands.push(new CommandDefinition(this.mediator, atts));
    }
     ...
     * @throws SAXException
     */
    public ExtResourceConfig resourceInfoStart(Attributes atts) throws SAXException {
        ExtResourceConfig resourceConfig = null;
        ...
        target = (target == null || "ANY_TARGET".equals(target)) ? ResourceConfig.ALL_TARGETS : target;

        String profile = atts.getValue("profile");
        profile = (profile == null || "ANY_PROFILE".equals(profile)) ? ResourceConfig.ALL_PROFILES : profile;
        ...
        this.registerProfile(profile, ResourceConfig.ALL_TARGETS.equals(target) ? null : target);
        resourceConfig.setUpdatePolicy(policyDefinition.getUpdatePolicy());

        TypeConfig resourceTypeConfig = new TypeConfig(policyDefinition.getPolicy());
        resourceTypeConfig.setImplementationClass((Class<? extends ExtResourceImpl>) policyDefinition.getPolicyImplementationClass());
    ...
    private void registerProfile(String profile, String target) {
        String prfl = profile;
        if (prfl == null || "ANY_PROFILE".equals(prfl) || (ResourceConfig.ALL_PROFILES.equals(prfl) && !this.handleNoProfile)) {
            return;
        }
        ...
        for (; targetIndex < targetLength; targetIndex++) {
            if (ServiceProvider.ADMINISTRATION_SERVICE_NAME.equals(targetArray[targetIndex]) || targets.contains(targetArray[targetIndex])) {
                continue;
            }
    ...
    public void valueStart(Attributes atts) throws InvalidXmlDefinitionException {
        ...
        this.valueDefinition = new ValueDefinition(this.mediator, atts, typeDefinition);
    }
    ...
    public void valueEnd() throws InvalidXmlDefinitionException {
        ...
        if (this.nameTypeValue != null) {
            ...
            this.attributeDefinition.setValueDefinition(this.valueDefinition);
        }
        this.valueDefinition = null;
    }
    ...
    public void parameterEnd() throws InvalidValueException {
        this.methodDefinition.addParameter(((ParameterDefinition) this.nameTypeValue));
        this.nameTypeValue = null;
    }
    ...
    public void referenceEnd() {
        this.methodDefinition.addReferenceDefinition((ReferenceDefinition) this.nameTypeValue);
        this.nameTypeValue = null;
    }
    ...
    public void builderStart(Attributes atts) {
        ParameterDefinition parameterDefinition = (ParameterDefinition) this.nameTypeValue;
        this.builder = new BuilderDefinition(mediator, parameterDefinition.getName(), atts);
    }
    ...
    private void startConstraint(Attributes atts, String name) throws InvalidXmlDefinitionException {
        ConstraintDefinition definition = new ConstraintDefinition(mediator, name, atts);
        if (this.builder != null) {
            this.builder.addConstraint(definition);
        }
        if (this.nameTypeValue != null) {
            ((ConstrainableDefinition) this.nameTypeValue).addConstraint(definition);
        } else if (this.attributeDefinition != null) {
            this.attributeDefinition.addConstraint(definition);
        } else {
            throw new InvalidXmlDefinitionException("No Attribute data structure found");
        }
    }
    ...
    public void metaEnd() throws InvalidXmlDefinitionException {
        this.attributeDefinition.addMetadataBuilder((MetadataDefinition) this.nameTypeValue);
        this.nameTypeValue = null;
    }
    ...
    public ExtResourceConfig end(String tag, String qname) {
        ...
        try {
            endMethod = getClass().getDeclaredMethod(qname.concat("End"));
     ...
     */
    @Override
    public ExtResourceConfig start(String tag, String qname, Attributes atts) throws SAXException {
        Method startMethod = null;
        try {
            startMethod = getClass().getDeclaredMethod(qname.concat("Start"), Attributes.class);
