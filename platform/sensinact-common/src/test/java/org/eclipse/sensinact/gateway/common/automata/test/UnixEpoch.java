...
public class UnixEpoch extends AbstractFrame
{
	...
	public Date getDate()
	{
		...
		{
			long unixEpoch =  ((frame[0] & MASK) << DECAL_BITS_24 | 
	    		 (frame[1] & MASK) << DECAL_BITS_16 |
	             (frame[2] & MASK) << DECAL_BITS_8 |
	             (frame[3] & MASK));
	 ...
	 * 
	 * @return
	 * 		the current date as an unix epoch formated 
	 * 		integer
	 * 
	...
	private int unixEpoch() throws FrameException
	{
		...
		if (unixTime < Integer.MIN_VALUE || unixTime > Integer.MAX_VALUE)
	    {
	        throw new FrameException
	            (unixTime + " cannot be cast to int without changing its value.");
	    }	
	 ...
	 * 
	 * @return
	 * 		the date as an unix epoch formated integer
	 * 
	 * @throws FrameException
	 */
