...
public class Mediator {
    ...
    private void loadBundleProperties() {
        ...
        if (!propertiesLoaded) {
            warn("%s bundle failed to load property file (either %s or %s respectively).", symbolicName, bundlePropertyFileName, bundlePropertyFileNameFallback);
        }
    ...
    public <S> String attachOnServiceAppearing(Class<S> serviceType, String filter, List<Executable<S, Void>> executables) {
        ...
        @SuppressWarnings("unchecked") TrackerCustomizer<S> customizer = (TrackerCustomizer<S>) this.customizers.get(trackingFilter);
        if (customizer == null) {
            ...
            ServiceTracker<S, S> tracker = new ServiceTracker<S, S>(context, serviceType.getCanonicalName(), customizer);
     ...
     * passed as parameter
     *
     * @param property the property key to return the value of
     * @return the value object of the property for the
     * specified key
     */
     ...
     *
     * @param property the property key to set the value of
     ...
     * and log level
     *
     * @param logLevel the log level of the message to display
     * @return true if the log level allow to display the message
     * false otherwise
     */
