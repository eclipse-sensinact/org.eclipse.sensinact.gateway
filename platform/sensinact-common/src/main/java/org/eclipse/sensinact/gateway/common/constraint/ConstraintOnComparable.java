 ...
 * 
 * @param <T>
 * 		the extended {@link Comparable} type
 * 
...
public abstract class ConstraintOnComparable<T> implements Constraint
{
	protected static final Logger LOGGER = Logger.getLogger(
			ConstraintOnComparable.class.getCanonicalName());
	/**
	 * Checks whether the value object passed as parameter
	 * complies to this constraint or not
	 * 
	 * @param castedValue
	 * 		the value object casted into the appropriate 
	 * 		type
	 * @return
	 * 		<ul>
	 ...
	 */
	public ConstraintOnComparable(ClassLoader classloader,
			String operator, Comparable<T> operand, 
			boolean complement)
	{
	...
	public ConstraintOnComparable(ClassLoader classloader,
			String operator, Class<?> operandClass,
	        ...
	        throws InvalidConstraintDefinitionException
	{
		...
		if (!Comparable.class.isAssignableFrom(comparableClass) && 
				(comparableClass = CastUtils.primitiveToComparable(operandClass))==null)
		{
			throw new InvalidConstraintDefinitionException(new StringBuilder()
			        .append(operand.getClass().getCanonicalName())
			        .append(" cannot be casted to Comparable").toString());
		}
		this.operand = (Comparable<T>) CastUtils.cast(this.classloader,
				comparableClass, operand);
		this.complement = complement;
	}
	...
	public boolean complies(Object value)
	{
		...
		try
		{
		  castedValue = (T)CastUtils.cast(
				  this.classloader,
				  this.operand.getClass(), value);
