...
public interface ConstraintFactory 
{
	...
	public static final class Loader
	{
		 ...
		 */
		public static Constraint load(ClassLoader classloader, 
				String constraint, Class<?> referenceClass, 
				Object reference, boolean complement) 
				throws InvalidConstraintDefinitionException
		{
			...
			if(!factory.handle(constraint))
			{
				...
				ServiceLoader serviceLoader = ServiceLoader.load(
					ConstraintFactory.class, classloader);
			...
			return factory.newInstance(classloader, 
					constraint, referenceClass, 
		 ...
		 * @return
		 * 		the {@link Constraint} described by the specified 
		 * 		constraint object  
		 * @throws InvalidConstraintDefinitionException
		 */
		public static Constraint load(ClassLoader classloader, Object constraint) 
				throws InvalidConstraintDefinitionException
		{
			if(constraint == null)
			{
				throw new InvalidConstraintDefinitionException(
						"Invalid constraint definition : null");
			}
			...
			throw new InvalidConstraintDefinitionException(
				new StringBuilder().append("Unable to cast ").append(
					constraint.getClass()).append(" into a constraint"
							).toString());
		}
		 ...
		 * @throws InvalidConstraintDefinitionException
		 */
		public static Constraint load(ClassLoader classloader, JSONObject constraint) 
				throws InvalidConstraintDefinitionException
		{
	        Object operand = null;
	        ...
	        if(type != null && type.length()>0)
	        {
	        	...
	        	} catch(ClassNotFoundException e)
	        	{
	        		...
	        		throw new InvalidConstraintDefinitionException(
		            new StringBuilder().append("Invalid type :").append(
		                type).toString());
	        	}
	        }
	        ...
	        boolean complement  = CastUtils.getObjectFromJSON(
	        		classloader,  boolean.class, constraint.opt(
	        				Constraint.COMPLEMENT_KEY) );
		 ...
		 * 		the appropriate ClassLoader
		 * @param constraint
		 * 		the JSON formated array of constraints 
		 * @return
		 * 		the {@link Expression} gathering the specified JSON 
		 * 		formated array of constraints
		 *  
		 * @throws InvalidConstraintDefinitionException
		 */
		public static Constraint load(ClassLoader classloader, JSONArray constraint) 
				throws InvalidConstraintDefinitionException
		{
			if(constraint == null)
			{
			...
			try
			{
				logicalOperator = Expression.LogicalOperator.valueOf(
						constraint.getString(0));
				constraint.remove(0);
		 ...
		 * 		the {@link Expression.LogicalOperator} between the 
		 * 		set of constraints
		 * @return
		 * 		{@link Expression} gathering the specified JSON 
		 * 		formated array of constraints 
		 * 
		 * @throws InvalidConstraintDefinitionException
		 */
		public static Constraint load(ClassLoader classloader,
			JSONArray constraint, Expression.LogicalOperator logicalOperator) 
				throws InvalidConstraintDefinitionException
		{			
			int index = 0;
			...
			for(;index < length;index++)
			{
				...
				if(JSONObject.class.isAssignableFrom(
						embeddedConstraint.getClass()))
				{
					expression.add(Loader.load(classloader, (JSONObject) 
							embeddedConstraint));
					
				} else if(JSONArray.class.isAssignableFrom(
						embeddedConstraint.getClass()))
				{
					expression.add(Loader.load(classloader, (JSONArray) 
							embeddedConstraint));
					
				} else
				{
					throw new InvalidConstraintDefinitionException(
						new StringBuilder().append("Invalid constraint definition : "
							).append(embeddedConstraint).toString());
				}
			}
			return expression;
		}
	 ...
	 * 
	 * @param constraint
	 * 		the name (operator) of the {@link Constraint}
	 * @return
	 * 		<ul>
	 ...
	 * 		to instantiate
	 * @param referenceType
	 * 		the expected type of the checked value
	 * @param reference
	 * 		the reference value
	 * @param complement
	 * 		defines whether to create the logical 
	 * 		complement constraint
	 * @return
	 * 		the new created {@link Constraint}
	 * 
	 * @throws InvalidConstraintDefinitionException
	 */
	Constraint newInstance(ClassLoader classloader, String constraint, 
			Class<?> referenceType, Object reference, 
			boolean complement)
			throws InvalidConstraintDefinitionException;
