...
public class DefaultConstraintFactory implements ConstraintFactory
{
	...
	public static enum Type
	{
		 ...
		 * 
		 * @param operator
		 * 		the string operator of the Restriction.Type
		 * 		to retrieve 
		 * @return
		 * 		the Restriction.Type whose string 
		 * 		operator is passed as parameter
		 */
        public static Type fromOperator(String operator)
        {        	
           DefaultConstraintFactory.Type[] values = 
        		   DefaultConstraintFactory.Type.values();
     ...
     */
    @Override
	public Constraint newInstance(ClassLoader classloader, String constraint, 
			Class<?> referenceType, Object reference, boolean complement) 
			throws InvalidConstraintDefinitionException 
	{
    	Type type = null;
        
        if ((type = valueOf(constraint.toUpperCase())) == null 
        		&& (type = Type.fromOperator(constraint)) == null)
        {
        		throw new InvalidConstraintDefinitionException(
        				new StringBuilder().append("Unknown constraint :"
            ...
            case ABSOLUTE:
                if(!reference.getClass().isArray() || Array.getLength(reference)!=2)
                {
                    throw new InvalidConstraintDefinitionException(
                            ...
                            "Invalid constraint definition :  ").append(
                                    "array of Numbers expected").toString());
                }
                instance = new Absolute(classloader, 
                		Array.get(reference, 0), 
                		Array.get(reference, 1), complement);
                break;
            case CHANGED:
            ...
            case DELTA:
                if(!reference.getClass().isArray() || Array.getLength(reference)!=2)
                {
                    throw new InvalidConstraintDefinitionException(
                    		new StringBuilder().append(
                            "Invalid constraint definition :  ").append(
                                    "array of Numbers expected"
                                    ).toString());
                }
                instance = new Delta(classloader,
                		Array.get(reference, 0),
                		Array.get(reference, 1), complement);
                break;
            case DIFFERENT:             
                instance = new Different(classloader,
                		referenceType, reference, complement);
                break;
            case EXISTS:             
                instance = new Exists(complement);
                break;
            case FIXED:
                instance = new Fixed(classloader,
                		referenceType, reference, complement);
                break;
            case MAXEXCLUSIVE:
                instance = new MaxExclusive(classloader,
                		referenceType, reference, complement);
                break;
            case MAXINCLUSIVE:
                instance = new MaxInclusive(classloader,
                		referenceType, reference, complement);
                break;
            case MINEXCLUSIVE:
                instance = new MinExclusive(classloader,
                		referenceType, reference, complement);
                break;
            case MININCLUSIVE:
                instance = new MinInclusive(classloader,
                		referenceType, reference, complement);
                break;
            case ENUMERATION:
                instance = new Contains(classloader,
                		referenceType, reference, complement);
                break;
            case LENGTH:
                referenceType = int.class;
                instance = new Length(classloader,
                	CastUtils.cast(classloader, int.class, reference), 
                	complement);
                break;
            case MAXLENGTH:
                referenceType = int.class;
                instance = new MaxLength(classloader, 
                	CastUtils.cast(classloader, int.class, reference), 
                	complement);
                break;
            case MINLENGTH:
                referenceType = int.class;
                instance = new MinLength(classloader,
                	CastUtils.cast(classloader, int.class, reference),
                	complement);
                break;
            case PATTERN:
                referenceType =  String.class;
                instance = new Pattern(classloader,
                		String.valueOf(reference),complement);
                break;
            case TRUE:
	...
	public boolean handle(String constraint) {
		...
		return (valueOf(constraint.toUpperCase()) != null 
        		|| Type.fromOperator(constraint) != null);
	}
