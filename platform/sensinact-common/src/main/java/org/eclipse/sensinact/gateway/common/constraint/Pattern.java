...
public class Pattern implements Constraint
{
	 ...
	 */
	public Pattern(ClassLoader classloader,
			String operand, boolean complement) 
			throws InvalidConstraintDefinitionException
	{
		...
		if ((operand == null) || (operand.length() == 0))
		{
			throw new InvalidConstraintDefinitionException(
			        "Unable to create a regular expression using an empty or null string");
		}
		try
		{
	...
	public boolean complies(Object value)
	{
		...
		try
		{
			sequence = CastUtils.cast(
				this.classloader ,String.class, value);
