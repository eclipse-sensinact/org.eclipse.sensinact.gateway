...
{		
	 ...
	 * 
	 * @param attributes
	 * 		the {@link Attributes} of the associated XML element
	 * 
	 * @return
	 * 		a new {@link FrameModel} implementation instance
	 */
	public static FrameModel newInstance(Mediator mediator, 
			Attributes attributes,FrameType type) 
	{
	...
	public void checkValid() throws FrameModelException
	{
		...
		for(;index<children.size();index++)
		{
			...
			{
				throw new FrameModelException(
						"a zero-length frame must be the last one declared in its scope");
			}
		}
	}
	...
	public FrameModel[] children()
	{
		...
		return this.children.toArray(
				new FrameModel[this.children.size()]);
	}
	...
	{		
		this.mediator.debug(
				"The name cannot be parameterized throught the model");
	}
	...
	{
		this.mediator.debug(
				"The length cannot be parameterized throught the model");
	}
	...
	{
		this.mediator.debug(
				"The type cannot be parameterized throught the model");
	}
}
