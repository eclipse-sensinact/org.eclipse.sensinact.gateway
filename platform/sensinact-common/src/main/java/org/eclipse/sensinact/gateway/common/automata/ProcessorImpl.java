...
public class ProcessorImpl implements Processor {
     ...
     */
    public ProcessorImpl(Mediator mediator, ProcessorListener listener) {
    ...
    public byte[] getDelimiters() {
        ...
        if (this.delimitation == Frame.START_DELIMITED || this.delimitation == Frame.START_END_DELIMITED || this.delimitation == Frame.SIZE_START_DELIMITED || this.delimitation == Frame.SIZE_START_END_DELIMITED) {
            delimitersStack.push(this.startDelimiter);
        }
        if (this.delimitation == Frame.END_DELIMITED || this.delimitation == Frame.START_END_DELIMITED || this.delimitation == Frame.SIZE_END_DELIMITED || this.delimitation == Frame.SIZE_START_END_DELIMITED) {
     ...
     *
     * @param xmlModel the path of the xml file describing the frame model
     * @throws FrameModelException if an error occurred during the model building
     */
    public void setXmlModel(String xmlModel) throws FrameModelException {
        ...
        try {
            ...
            this.automata = new Automata(this.startDelimiter, this.endDelimiter, this.escape);
     ...
     *
     * @return a {@link FrameModel} implementation instance
     */
    ...
    public Frame newFrame(FrameModel model) throws FrameProcessorException, FrameException {
        ...
        if ((this.model == null) || (this.listener == null) || ((factory = this.listener.getFrameFactory()) == null)) {
     ...
     */
    public FrameModelHandler newModel(String xmlModelPath) throws FrameModelException {
        FrameModelHandler handler = new FrameModelHandlerImpl(this.mediator);
        URL schemaLocation = ProcessorImpl.class.getClassLoader().getResource("frame.xsd");

        if (schemaLocation == null) {
            throw new FrameSchemaNotFoundException("'frames.xsd' schema cannot be found");
        }
        try {
    ...
    private class Automata {
         ...
         */
        private void run() throws FrameProcessorException, FrameException {
            this.frameIndex = 0;
            checkValid();
            try {
                byte currentByte = 0;
                ...
                synchronized (ProcessorImpl.this.sync) {
                    emptyBufferEvent = (this.frameIndex == ProcessorImpl.this.buffer.length && this.currentState == WAITING_EMPTY);
                }
                if (emptyBufferEvent) {
                    complete();
                }
        ...
        private void complete() {
            if (this.currentFrame != null && this.currentFrame.isComplete()) {
                ...
                ProcessorImpl.this.mediator.error("Maximum frame length exceeded : more than " + ProcessorImpl.MAX_FRAME_LENGTH + " bytes");
         ...
         *
         * @param ifNull   create a new {@link Frame} if the current one
         *                 is null
         * @param whatEver create a new {@link Frame} or not whatever is the
         *                 current one
         * @param factory  the {@link FrameFactory} use to create a new
         *                 {@link Frame} if required
         * @throws FrameException
         * @throws FrameProcessorException
         */
        private void newFrame(boolean ifNull, boolean whatEver) throws FrameException, FrameProcessorException {
            boolean tocreate = ifNull ? ((this.currentFrame == null) ? true : false) : whatEver;

            if (tocreate) {
                this.currentFrame = ProcessorImpl.this.newFrame(ProcessorImpl.this.model);
            }
        }
         ...
         *
         * @param currentByte the currentByte value
        ...
        private byte nextByte(byte currentByte) throws FrameProcessorException {
            if (this.frameIndex < ProcessorImpl.this.buffer.length) {
                ...
                this.isStartByte = (!this.startDelimiterExists) ? false : (this.startDelimiter == nextByte) ? true : false;
                this.isEndByte = (!this.endDelimiterExists) ? false : (this.endDelimiter == nextByte) ? true : false;

                this.isEscaped = (!this.escapeExists) ? false : (this.escape == currentByte && !this.isEscaped) ? true : false;

                return nextByte;
            }
            throw new FrameProcessorException("Empty buffer [" + this.frameIndex + "/" + ProcessorImpl.this.buffer.length + "]");
        }
        ...
        private void cleanBuffer() {
            ...
            if (length > 0) {
                System.arraycopy(ProcessorImpl.this.buffer, this.frameIndex, newBuffer, 0, length);
            }
         ...
         *
         * @throws FrameProcessorException if the configuration is not valid
         */
        private void checkValid() throws FrameProcessorException {
            if (this.currentState == UNKNOWN) {
                throw new FrameProcessorException("Unable to process : Invalid configuration");
            }
        }
    }
}
