 ...
 *
 * @param <T> the type of the handled data set
...
public abstract class ConstraintOnCollection<T> implements Constraint {
     ...
     * @param operandClass use the Enum class if the constraint applies on its constants
     *                     set use the component type or the array class if the
     *                     constraint applies on an array or a collection
     * @param operand      use null if this constraint applies on the constants set of an
    ...
    public ConstraintOnCollection(ClassLoader classloader, String operator, Class<T> operandClass, Object operand, boolean complement) throws InvalidConstraintDefinitionException {
        ...
        if (operand == null) {
            if (operandClass.isEnum()) {
                ...
                this.operand = (T[]) ((Class<? extends Enum<?>>) operandClass).getEnumConstants();
                return;
            }
        ...
        if (operandClass.isArray()) {
            ...
            for (Type genericInterface : interfaces) {
                try {
                    ...
                    if (Collection.class.isAssignableFrom(((Class<?>) parameterized.getRawType()))) {
