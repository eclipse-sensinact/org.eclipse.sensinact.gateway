...
public class Delta implements Constraint
{
	 ...
	 */
	public Delta(ClassLoader classloader, 
			Object delta, Object referenceValue, 
			boolean complement)
	        throws InvalidConstraintDefinitionException
	{
		...
		try
		{
			double deltaValue = Math.abs(CastUtils.cast(
				this.classloader, double.class, delta));			
			this.delta = deltaValue>100?deltaValue-100:deltaValue;
			
			this.referenceValue = CastUtils.cast(
				this.classloader, double.class, referenceValue);		
	...
	public boolean complies(Object value)
	{
		...
		try
		{
			doubleValue = CastUtils.cast(classloader,
					double.class, value);
		...
		complies = Math.abs(100 - Math.abs(
				(100d * doubleValue)/this.referenceValue)) 
				>= delta;
