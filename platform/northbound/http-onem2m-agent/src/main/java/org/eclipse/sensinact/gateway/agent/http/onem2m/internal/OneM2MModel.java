...
public class OneM2MModel {
	private static Logger LOG = LoggerFactory
			.getLogger(OneM2MModel.class.getCanonicalName());
	...
	public static OneM2MModel getInstance(String cseBase) {
		if (instance == null) {
			LOG.debug(
					"Creating OneM2M model singleton to manage created instance values");
			instance = new OneM2MModel(cseBase);
		}
		return instance;
	}

	private void createModel(String provider, String service, String resource,
			String value) {
		...
		if (!model.keySet().contains(provider)) {
			...
			try {
				Util.createRequest(cseBase, "POST",
						"CEA" + provider.toUpperCase(), null,
						"application/json;ty=2", m2mmodel);
			} catch (IOException e) {
				LOG.debug(
						"Failed to create application container in OneM2M server",
						e);
			}
		} else {
			LOG.warn(
					"Container for provider {} already exists, just integrating reading.",
					provider);
		}
	}

	public void integrateReading(String provider, String service,
			String resource, String value) {

		if (!model.containsKey(provider)) {
			createModel(provider, service, resource, value);
			OneM2MModelResource resourceModel = new OneM2MModelResource(
					provider, cseBase);
	...
	public void removeProvider(String provider) {
		...
		if (resourceModel != null) {
			...
			content.put("rn", provider);
			try {
				Util.createRequest(cseBase, "DELETE", provider, null,
			...
			LOG.warn(
					"Impossible to remove provider {}, it does not exist in managed list",
					provider);
		}
