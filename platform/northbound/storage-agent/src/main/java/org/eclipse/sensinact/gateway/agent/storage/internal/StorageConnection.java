...
import org.eclipse.sensinact.gateway.protocol.http.client.*;
...
class StorageConnection {
     ...
     * @param uri
     * 		the string URI of the storage server
     * @param login
     * 		the user login
     * @param password
     * 		the user password
     * @throws IOException
     *      Exception on connection problem
     */
    ...
    private void httpRequest(JSONObject object) {
        ...
        try {
            ...
            for(;iterator.hasNext(); entry = iterator.next()) {
                this.mediator.debug(entry.getKey() + " :: "
                        + (entry.getValue()==null?"null":Arrays.toString(entry.getValue().toArray(new String[0]))));
            }
        } catch(Exception e) {
            e.printStackTrace();
