...
public class ForwardingFactory {
			...
			ExtHttpService extHttpService) 
	{
		...
		this.filters = Collections.synchronizedMap(
				new HashMap<String,ForwardingFilter>());
	...
	public void start()
	{
		...
		attachAll();
		this.appearingKey = mediator.attachOnServiceAppearing(ForwardingService.class,
				(String)null, new Executable<ForwardingService,Void>()
		{
			@Override
		...
		this.disappearingKey = mediator.attachOnServiceDisappearing(ForwardingService.class, 
				(String)null, new Executable<ForwardingService,Void>()
		{
			@Override
			public Void execute(ForwardingService forwardingService) throws Exception 
	...
	public void stop() 
	{
		...
		mediator.detachOnServiceAppearing(ForwardingService.class, (String)
				null,appearingKey);
		mediator.detachOnServiceDisappearing(ForwardingService.class,(String)
				null, disappearingKey);
		detachAll();		
	}
	...
	public final void attach(ForwardingService forwardingService)
	{
		...
		ForwardingFilter forwardingFilter = new ForwardingFilter(mediator,
			forwardingService.getUriBuilder(), forwardingService.getQueryBuilder());
