...
import org.eclipse.sensinact.gateway.app.manager.component.*;
...
public class ApplicationFactory {
     ...
     */
    public static Application createApplication(AppServiceMediator mediator, AppContainer container, ServiceImpl service)
        ...
        Map<ResourceDataProvider, Collection<ResourceSubscription>> resourceSubscriptions =
                new HashMap<ResourceDataProvider, Collection<ResourceSubscription>>();
        ...
        for(AppComponent component : container.getComponents()) {
            ...
            try {
                ...
                ApplicationFactoryException exception = new ApplicationFactoryException(
                        "Unable to create application " + container.getApplicationName() + ". " + e.getMessage());

                if (mediator.isErrorLoggable()) {
                    mediator.error("Unable to create application " + container.getApplicationName() + ". "
            ...
            if(superType == null) {
                if(mediator.isErrorLoggable()) {
                    mediator.error("Instance creation error", new RuntimeException(
                            "Unable to instantiate: " + function.getClass()));
                }
            ...
            dataProvidersRegistration.add(mediator.registerService(DataProviderItf.class.getCanonicalName(),
                    resultData,
                    resultProperties));
            ...
            ComponentBuilder builder = new ComponentBuilder(mediator, component.getIdentifier(),
                    function, component.getFunction().getRunParameters(), componentDataProviderMap);
                    ...
                    if(!resourceSubscriptions.containsKey(dataProvider)) {
                        ...
                        for(ResourceSubscription resourceSubscription : resourceSubscriptions.get(dataProvider)) {
                            ...
                            if(event.getConditions().isEmpty() && resourceSubscription.getConditions().isEmpty()) {
                                ...
                                break;
                            } else if(event.getConditions().containsAll(resourceSubscription.getConditions())
                                    && resourceSubscription.getConditions().containsAll(event.getConditions())) {
                                constraintExist = true;
                                break;
                            }
                        }
                        ...
                        resourceSubscriptions.get(dataProvider).add(
                                new ResourceSubscription(resourceUri, event.getConditions()));
                    }
        ...
        for(AppComponent component : container.getComponents()) {
            String componentUri = AppJsonConstant.URI_SEPARATOR + container.getApplicationName() +
                    AppJsonConstant.URI_SEPARATOR + component.getIdentifier();
            ...
            if (component.getProperties().getRegister()) {
                try {
                    AbstractDataProvider dataProvider = allDataProvidersMap.get(componentUri
                            + AppJsonConstant.URI_SEPARATOR + ComponentConstant.RESULT_DATA);

                    RegisterPropertyBlock propertyBlock = new RegisterPropertyBlock(
                            service.addDataResource(SensorDataResource.class,
                                    componentUri,
        ...
        return new Application(mediator,
                ...
                components,
                new AppExceptionWatchDog(mediator, (ApplicationService) service));
    }
