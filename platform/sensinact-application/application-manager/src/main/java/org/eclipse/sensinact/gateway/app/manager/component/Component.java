...
public class Component implements DataListenerItf, FunctionUpdateListener {
     ...
     */
    public Component(AppServiceMediator mediator, String identifier,
                     Map<String, DataProviderSubscription> inputDataProviderMap, AbstractFunction function,
                     List<AppParameter> functionParameters, Map<String, AbstractPropertyBlock> propertyBlocks,
                     Map<String, DataProvider> outputDataProviderMap) {
        ...
        for(Map.Entry<String, DataProviderSubscription> map : inputDataProviderMap.entrySet()) {
            ...
            try {
                String filter = "(&(objectClass=" + DataProviderItf.class.getName() + ")" +
                        "(uri=" + subscription.getDataProviderUri() + "))";

                ServiceReference[] serviceReferences = mediator.getServiceReferences(filter);

                if(serviceReferences.length == 0) {
                    throw new LifeCycleException("Unable to find " + subscription.getDataProviderUri() + ". " +
                ...
                throw new LifeCycleException("Unable to instantiate the component " + identifier + " > "
                        + e.getMessage());
            }
        }
        ...
        for(Map.Entry<String, DataProviderSubscription> map : inputDataProviderMap.entrySet()) {
            ...
            if(!map.getKey().equals(event.getData().getSourceUri())) {
                ...
                for(List<String> listenerRoute : listenerRoutes) {
                    ...
                    if (listenerRoute.get(0).equals(firstProvider) &&
                            !listenerRoute.get(listenerRoute.size() - 1).equals(lastProvider)) {
