...
public class TestComponentFactory extends TestCase {
    ...
    public void init() throws Exception {
        ...
        Mockito.when(adminService.getName())
                .thenReturn(ServiceProvider.ADMINISTRATION_SERVICE_NAME);

        Mockito.when(adminService.getPath())
                .thenReturn(UriUtils.getUri(new String[]{ "TestAppDevice", ServiceProvider.ADMINISTRATION_SERVICE_NAME }));

        Mockito.when(mediator.getClassLoader())
                .thenReturn(Thread.currentThread().getContextClassLoader());
        Mockito.when(modelInstance.mediator())
                .thenReturn(mediator);
        
        Mockito.when(adminService.getModelInstance())
                .thenReturn(modelInstance);
        Mockito.when(device.getModelInstance())
                .thenReturn(modelInstance);
        ...
        Mockito.when(adminService.getResource(AppConstant.UNINSTALL))
        ...
        Mockito.when(device.getName())
                .thenReturn("TestAppDevice");
        Mockito.when(installer.getFunction(Mockito.any(AppFunction.class)))
                .thenReturn(new MockComponentIncrement(mediator));

        Mockito.when(mediator.getSecuredAccess()).thenReturn(securedAccess);
    ...
    public void testComponentCreation() {
        ...
        try {
            content = TestUtils.readFile(this.getClass().getResourceAsStream("/test_factory.json"),
