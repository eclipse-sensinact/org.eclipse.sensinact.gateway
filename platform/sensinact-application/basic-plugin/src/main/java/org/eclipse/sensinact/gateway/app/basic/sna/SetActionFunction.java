...
public class SetActionFunction extends ActionFunction {
    ...
    public void fireHook() {
        ...
        if (resource != null) {
            ...
            if (Modifiable.MODIFIABLE.equals(resource.element(DataResource.VALUE).getModifiable())) {
                ((PropertyResource) resource).set(params[0]);

            } else {
                throw new RuntimeException("Resource " + resourceVariable.getSourceUri() + " is not a modifiable PropertyResource");
            }
        } else {
