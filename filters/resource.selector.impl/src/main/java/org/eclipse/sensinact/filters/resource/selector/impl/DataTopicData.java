...
public class DataTopicData {

    private static final Logger LOG = LoggerFactory.getLogger(DataTopicData.class);
    ...
    public DataTopicData(ResourceSelection rs) {
        this.rs = rs;
        ...
        this.valueFilter = combineResourceCheck(this.resourceFilter, 
                rs.value().stream()
                    .map(DataTopicData::toValueFilter)
    ...
    private String exactSelection(Selection s) {
    ...
    private static Predicate<ResourceSnapshot> combineResourceCheck(Predicate<ResourceSnapshot> r, Predicate<TimedValue<?>> v) {
        if(v == null) {
            ...
            return r == null ? valueCheck : r.and(valueCheck);
        }
    }
    ...
    private static Predicate<ResourceSnapshot> combineServiceCheck(Predicate<ServiceSnapshot> s, Predicate<ResourceSnapshot> r) {
        if(s == null) {
            ...
            return r == null ? svcCheck : svcCheck.and(r);
        }
    }
    ...
    private static Predicate<TimedValue<?>> toValueFilter(ValueSelection vs) {
        ...
        if(vs == null) {
            ...
            Function<TimedValue<?>,Object> getterFunction = switch(ct) {
                case SIZE: yield DataTopicData::toSize;
