...
public class CasbinUtils {
    ...
    public static Model makeModel() {
        final List<String> snaFields = List.of("provider", "service", "resource", "level");
    ...
    public static List<Policy> defaultPolicies(final boolean allowByDefault) {
        ...
        policies.add(new Policy("*", "sensiNact", null, null, null, PolicyEffect.deny, -10000));
        policies.add(new Policy("*", "sensiNact", null, null, Stream.of(PermissionLevel.DESCRIBE, PermissionLevel.READ)
                .map(Enum::name).collect(Collectors.joining("|")), PolicyEffect.allow, -10001));

        // Admin has access to everything
        policies.add(new Policy("role:admin", null, null, null, null, PolicyEffect.allow, -1000));

        if (allowByDefault) {
            // Soft access denial to anonymous
            policies.add(new Policy(Constants.ROLE_ANONYMOUS, null, null, null, null, PolicyEffect.deny, 10000));

            // Soft access to users
            policies.add(new Policy("role:user", null, null, null, null, PolicyEffect.allow, 10000));
        } else {
            // Stronger denial for anonymous (models can't enable it)
            policies.add(new Policy(Constants.ROLE_ANONYMOUS, null, null, null, null, PolicyEffect.deny, 1));

            // Soft denial for users. Allow to describe resources
            policies.add(new Policy("role:user", null, null, null, null, PolicyEffect.deny, 10001));
            policies.add(new Policy("role:user", null, null, null, PermissionLevel.DESCRIBE.name(), PolicyEffect.allow,
                    10000));
        }
